...
summary: |
  Team channel for team-wide; DM individuals for direct asks; work
  items / pages for durable or cross-team work. Discover with
...
| Work that already has a durable home (work item / page) | Comment on the work item (or update the relevant page) — these outlive the conversation and don't require channel membership |
...
2. **Comment on the relevant work item.** A work item reaches everybody already on it — assignee, reporter, collaborators, watchers — without anybody having to be in a channel.
