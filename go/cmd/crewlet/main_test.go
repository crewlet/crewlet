...
func TestAWorkerOnlyNodeServesNoHTTPAndSaysSo(t *testing.T) {
	...
	surface, err := serveAPI(t.Context(), bootstrapFor(t, 0), nil, logging.Get("test"))
	if err != nil {
...
func TestAnUnbindablePortIsReportedRatherThanIgnored(t *testing.T) {
	...
	e := testEngine(t)
	surface, err := serveAPI(t.Context(), bootstrapFor(t, port), e, logging.Get("test"))
	if err == nil {
...
func TestAMergedNodeServesItsOwnHealth(t *testing.T) {
	...
	surface, err := serveAPI(t.Context(), boot, e, logging.Get("test"))
	if err != nil {
		t.Fatalf("serveAPI: %v", err)
	}
