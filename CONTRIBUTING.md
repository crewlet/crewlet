...
3. Add a `CHANGELOG.md` entry under the current `Unreleased` heading if the
   change is visible to someone running Crewlet — a feature, a config or CLI
   change, a behavior change, a notable fix. The GitHub Release body is cut
   from that section verbatim, so write it as release notes rather than as a
   commit log. Internal refactors with no observable effect need no entry.
4. Run the lint + test commands above.
