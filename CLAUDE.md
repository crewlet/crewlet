...
| `textcut` | Shortening a string without breaking it. It replaced THREE helpers named `truncate` — in `agent/extension`, `learning` and `providers/llm/cliagent` — which were three copies of one rule already disagreeing on the easy half: two appended `…` and one appended `...`. The hard half is that a plain `s[:n]` is invalid UTF-8 whenever a multi-byte rune straddles the cut, which a JSON encoder substitutes, a model reads as a replacement character and a vendor rejects. BYTES ONLY, because cutting is the last resort rather than the first: content a turn reasons over is passed whole, and a value with a vendor limit is REFUSED naming the field rather than silently cut to fit. It is NOT the tree's only shared cut — `agent/ledger.Elide` is the other, counting RUNES and reading a limit of 0 as unbounded where this one reads it as empty, and the two package docs point at each other rather than folding two contracts into one name |
