...
| `agent/runner` | Drives the three phases against real models and real tools — the wiring between the prompt builder, the tool registry, the provider chain and the turn loop's contract, none of which knows the others. Plumbing, plus the one rule plumbing cannot express: what happens when the model never gives a structured answer at all. What a valid answer IS lives here too, in the decoders; the SHAPE is `agent/structured` |
