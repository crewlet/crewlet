...
cmd/crewlet/          # The one binary. run / validate / schema / migrate /
                      #   budgets / secrets / config, and the three vendor
                      #   CLIs (gitlab, plane, mattermost). Every command the
...
| `coord` | Cross-process ownership: TTL leases with a monotonic fencing epoch, plus the shared counters a fleet coordinates through (`kv`). Every "do I hold this?" is three-valued — held / definitively not / **unknown** — and treating unknown as loss tears a healthy company down over a two-second store blip |
| `store` | The node's LOCAL database: the audit event log, learning memory, durable turn state. **One file, one process** — exclusively owned, so nothing here has to be safe against a peer, and the migrator's advisory-lock idiom simply disappears. Two certified drivers, one dialect: every statement must parse on both |
...
| `engine` | The ENTANGLEMENT POINT — which concrete thing satisfies which seam. Deliberately many small files rather than one large one; the two hardest passages (the inbox handler, the config apply) are packages of their own |
...
| `mattermost`, `plane`, `gitlab` | The three vendors. Each contributes only what is genuinely its own: a client, a parser, a transport, a prompt, a provisioning reconcile |
...
11. **Coordination** — TTL leases with a fencing epoch, plus the fleet's shared counters and ledgers
