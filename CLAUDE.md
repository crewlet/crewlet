...
- **OpenTelemetry** (`go.opentelemetry.io/otel` + the OTLP trace exporters) — tracing, and the one dependency here that `std` genuinely cannot supply: W3C trace context, a sampler, and a wire format three collectors already speak. Configured from the standard `OTEL_*` environment rather than Tier A, so an operator pastes their collector's own snippet — see `decisions/508`. The provider is always installed and only the exporter is conditional, because a trace id is a column in the event store before it is ever an exporter's concern
...
adrs/                 # Architecture decision records. Why the engine is
                      #   shaped the way it is, and what the obvious
                      #   alternative cost. A record of what was decided and
                      #   when — NOT a rule: revisit one by superseding it.
                      #   Cited from the code it governs. See adrs/README.md
...
**Every package states its own rationale in its package doc.** `go doc ./internal/coord` is the authority on coordination, not this file — what follows is a map, so you know which doc to read. Where a decision would be tempting to "fix" back to the obvious shape, it is written up under `adrs/` — as a record of the reasoning and what the alternative cost, not as a constraint. An ADR is superseded by writing the next one, never by being obeyed indefinitely; `internal/version` asserts every `adr-NNN` citation still resolves, so a removal cannot go quiet.
...
| `agent/turnctx` | What a turn IS, carried as an explicit argument rather than ambient state. Python threaded five contextvar channels through a turn; a contextvars context is COPIED into a task while a goroutine shares whatever it captured, so the pattern that merely obscured a dependency there is a live data race here. See `adrs/401` |
...
| `mcp` | MCP client and child supervision. A stdio server is a process TREE, not a process. See `adrs/602` |
