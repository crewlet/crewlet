...
| `agent/turnctx` | What a turn IS, carried as an explicit argument rather than ambient state. A turn has at least five values that want to travel ambiently, and a goroutine SHARES whatever context it captured — forever, including after the turn that created it finished — so what would elsewhere merely obscure a dependency is a live data race here. See `adrs/401` |
