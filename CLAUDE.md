...
- Tier A's `logging.level` / `logging.format` (and the `debug: true` shorthand) are what the file asks for; `crewlet run`'s flags override them ONLY where `isFlagSet` says the operator actually typed one. A flag typo resolves to the default, a FILE typo is refused with its field path — `debug: true` that nothing read is what that asymmetry exists to prevent. See `decisions/001`
