...
- **Every user-visible change gets a `CHANGELOG.md` entry** under the current `Unreleased` heading — new features, config or CLI changes, behavior changes, notable fixes. The release workflow cuts the GitHub Release body from that section verbatim, so write it as release notes for an operator, not as a commit log. Pure internal refactors with no observable effect do not need one.
- **The root-level files are canonical for process**, and this file must not contradict them: `CONTRIBUTING.md` (dev setup, contributor conventions, dependency surfaces), `RELEASING.md` (the release runbook), `SECURITY.md` (private vulnerability reporting, operator scope notes), `CODE_OF_CONDUCT.md`, `CHANGELOG.md`, `LICENSE`. When a rule here changes, change it there in the same commit — a contributor reading `CONTRIBUTING.md` and an agent reading this file must never get different answers.
...
- **A version bump means a `CHANGELOG.md` section** — the release workflow refuses to publish without one, and cuts the GitHub Release notes from it. The minor number moves for features, the patch number for fixes.
