...
| `agent/prefetch` | The turn-start context assembly: relevant knowledge, memory, the blocks a prompt gets. FROZEN before the runner exists, which is what stops a self_iterate loop moving the system prompt underneath a turn. Nothing re-fetches mid-turn any more — the executor asks, with `search_knowledge`, over this same seam |
