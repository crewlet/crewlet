...
// pointing at `src/crewlet/static/dashboard`. Thirty-five copies of one fact,
// which is the shape of thing this project collapses everywhere else it
// appears (the topic grammar, the ${VAR} grammar, the .env assignment
// grammar), and for the same reason: a location that is written down in many
// places is a location that can only ever be moved in most of them.
//
// It is being moved. The Go rewrite serves the dashboard from a tree embedded
// in the binary (`go/static/dashboard`), and the gate on that work is that
// these suites — the compatibility reference for the client's half of the wire
// protocol — pass against THAT tree, not against a second copy of it that
// happens to agree today. So the root is a parameter, and it has exactly one
// resolution point.
//
// CREWLET_DASHBOARD_ROOT names the tree to test. Unset, it is the tree this
// repository ships from Python, which is the living specification for the
// duration of the rewrite and the tree a contributor debugging one suite by
// hand wants. The Go runner sets it to the embedded copy; a drift guard in
// that same package holds the two byte-identical while both exist.
//
...
/** The tree Python serves, relative to this file. */
const IN_REPO = join(HERE, "../../../src/crewlet/static/dashboard");
