...
// A RUN THAT CANNOT REACH THE FLEET'S SECRET STORE IS REFUSED BEFORE THE
// THIRD-PARTY APP IS TOUCHED.
//
// With a keyring the company's credentials may be in that store, which every
...
func TestARunThatCanReachTheFleetIsNotRefused(t *testing.T) {
	...
	keyringless := filepath.Join(t.TempDir(), "config.yaml")
	...
	for name, args := range map[string][]string{
		...
		_, _, err := provisionCmd(t, append([]string{company}, args...)...)
		if errors.Is(err, errFleetUnread) || err == nil ||
			!strings.Contains(err.Error(), "no administrator token") {
			t.Errorf("%s: the run answered %v, want it to reach the "+
				"administrator-token check", name, err)
		}
	}
}
