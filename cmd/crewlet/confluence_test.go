...
// admit is guidance that never appears in an executor's prompt. This command
// runs the engine's own walk against a throwaway registry so an operator can
// see what the next boot will see, without restarting anything and without a
// running engine to ask.
//
