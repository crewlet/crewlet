...
// that could not be built, a refused budget, a seat handed to another node
// mid-call) proves nothing, keeps its NAK and keeps its retry.
//
...
func (d *Dispatcher) abandon(ctx context.Context, handle string, req Request, cause error, reason string) queue.Result {
	log.ErrorContext(ctx, "turn_abandoned", "seat", handle,
		"work_key", req.WorkKey, "reason", reason, "error", cause.Error(),
