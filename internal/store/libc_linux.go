...
//
// EVIDENCE, NOT A VERDICT, and it is only ever consulted after the native
// library has already failed to load — see [libcAdvice]. A distribution can
// carry musl alongside glibc as an ordinary package, so a true here does not
// prove that glibc is absent, and refusing to start on it would break a
// working deployment to prevent a hypothetical one. Used as a hint on an
// error path, a false positive costs a sentence.
