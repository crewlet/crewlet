...
// build pins.
//
// It exists because adrs/002 found the documentation and the code
...
// (adrs/003) took away the comparison this table used to draw, and it
...
// nullable-work_key design (adrs/002 §2).
//
...
func TestPartialIndexConflictTarget(t *testing.T) {
	...
	if _, err := db.SQL().ExecContext(ctx,
		...
		t.Fatal("a bare ON CONFLICT now resolves against a partial index — " +
			"re-read adrs/002 §2 before relying on it")
	}
