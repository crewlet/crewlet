...
// appRefusing is a GitHub stand-in that refuses every call with one body.
func appRefusing(t *testing.T, status int, contentType, body string) *github.AppClient {
	t.Helper()
	...
	t.Cleanup(srv.Close)
	client, err := github.NewAppClient(7, appRefusalKey(t), srv.URL, nil)
	if err != nil {
...
// appRefusalKey is one key for the whole file: generating a 2048-bit RSA key
// per case is the slowest thing these tests would do, and none of them
// asserts anything about the key.
func appRefusalKey(t *testing.T) string {
	t.Helper()
	appRefusalKeyOnce.Do(func() { _, appRefusalKeyPEM = testKey(t) })
	return appRefusalKeyPEM
}

// appRefusalFor runs one refused call and returns the typed error.
func appRefusalFor(t *testing.T, c *github.AppClient) *github.APIError {
	t.Helper()
...
//
// The invariant: an endpoint that answers more than this build reads must
...
// result is what this replaced, and it failed three ways at once: the quote
// was a prefix of a rendered document with nothing marking it as a prefix,
// the prefix was taken with a byte slice so a multi-byte rune straddling the
// boundary left invalid UTF-8, and the read error was discarded so a body
// that failed halfway arrived as an empty Detail.
func TestAnAppRefusalPastTheCeilingIsNamedRatherThanQuoted(t *testing.T) {
	t.Parallel()
	page := oversizedGatewayPage(t, httpx.RefusalBytes)
	detail := appRefusalFor(t,
		appRefusing(t, http.StatusBadGateway, "text/html; charset=utf-8", page)).Detail
...
// NOT. A gateway puts the reason in the document's <title>; the rest is
// layout, and pasting it into an error puts a rendered page in a log around a
// sentence nobody can find.
func TestAnAppRefusalWithinTheCeilingIsWordsRatherThanMarkup(t *testing.T) {
	t.Parallel()
	...
	if got := appRefusalFor(t,
		appRefusing(t, http.StatusForbidden, "text/html", page)).Detail; got != "403 Forbidden" {
		t.Errorf("detail = %q, want the page's own title", got)
	}
}

// GITHUB'S OWN JSON ENVELOPE IS NOT COLLATERAL. The fallback above must not
// eat the case it was written to leave alone: an API error is the one body
// that genuinely explains the refusal.
func TestAnAppRefusalKeepsGithubsOwnEnvelope(t *testing.T) {
	t.Parallel()
	const body = `{"message":"Bad credentials","documentation_url":"https://example.com"}`
	if got := appRefusalFor(t,
		appRefusing(t, http.StatusUnauthorized, "application/json", body)).Detail; !strings.Contains(got, "Bad credentials") {
...
// a refusal that needs more than this to be understood needs the endpoint's own
...
// severed sentence read as the endpoint's complete answer.
func TestAnAppRefusalLineIsBoundedMarkedAndRuneSafe(t *testing.T) {
	t.Parallel()
	...
	detail := appRefusalFor(t,
		appRefusing(t, http.StatusInternalServerError, "text/plain; charset=utf-8", body)).Detail
...
// oversizedGatewayPage is an HTML error page longer than ceiling, arranged so
...
// ASCII fixtures are why four separate truncation helpers in this tree shipped
// with the same bug for as long as they did.
...
// appRefusalKeyPEM is generated once for this file; see [appRefusalKey].
var (
	appRefusalKeyOnce sync.Once
	appRefusalKeyPEM  string
)
