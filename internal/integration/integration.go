// Package integration is the vendor-neutral half of keeping an external
// surface in line with the company.
//
// # Why this exists before any vendor sits on it
//
// Six vendor packages already know how to bring their surface into line:
...
// was built, before any vendor sat on it. A spine grown after its first
// vendor has that vendor welded into it, and the five bespoke classifiers
// this replaces are what that looks like: written once per vendor, they had
...
//     asking one question, and answering it in two vendors' words makes them
...
//     vendor is doing comes back quickly; work a person owes is re-checked on
...
// gesture on the vendor's own subcommand, where the operator types the flag
...
// up carrying another vendor's status.
//
