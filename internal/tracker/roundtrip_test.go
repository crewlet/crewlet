...
// declares and keeps it for the test's life, as a running node's applier does.
// Housekeeping that needs a pin of its own cannot get one on a live node, and
// a harness that left the pin free would pass it anyway.
