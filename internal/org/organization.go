...
type Organization struct {
	...
	// ConfluenceSpaces and PlaneProjects are the org-wide knowledge READ
	// scope — the only thing that narrows a knowledge search. Empty means
	// unscoped, bounded by whatever the backend's own ACLs allow.
	//
	// Deliberately org-wide rather than per-seat: a unit's Confluence space
	// or Plane project is an identity (where it writes, where its webhooks
	// route), and letting an identity double as a read scope is how an
	...
	PlaneProjects    []string `yaml:"plane_projects,omitempty" json:"plane_projects,omitempty"`
}
