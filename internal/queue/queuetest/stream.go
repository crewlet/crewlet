...
func (s *suite) runStream(t *testing.T) {
	...
	t.Run("stream_broadcasts_to_every_subscriber", func(t *testing.T) {
		...
		streamTo(t, q, "crewlet.events.>", a)
		streamTo(t, q, "crewlet.events.>", b)

		publish(t, q, "crewlet.events.task_created", newEvent("task_created"))
	...
	t.Run("stream_filters_by_topic_pattern", func(t *testing.T) {
		...
		q := s.start(t)
		j := newJournal()
		streamTo(t, q, "crewlet.events.>", j)

		publish(t, q, "crewlet.events.task_created", newEvent("task_created"))
		publish(t, q, "crewlet.notifications.inbound", newEvent("raw_webhook"))
	...
	t.Run("stream_star_matches_exactly_one_segment", func(t *testing.T) {
		...
		publish(t, q, "crewlet.events.task_created", newEvent("match"))
		...
		tryPublish(q, "crewlet.events.task.created", newEvent("too_deep"))
		tryPublish(q, "crewlet.events", newEvent("too_shallow"))
	...
	t.Run("stream_unsubscribe_stops_dispatch", func(t *testing.T) {
		...
		publish(t, q, "crewlet.events.task_failed", newEvent("task_failed"))
	...
	t.Run("stream_handler_failure_does_not_break_publish", func(t *testing.T) {
		...
		streamTo(t, q, "crewlet.events.>", healthy)
	...
	t.Run("stream_does_not_consume_from_durable_subscriptions", func(t *testing.T) {
		...
		streamTo(t, q, "crewlet.events.>", stream)
		subscribe(t, q, "crewlet.events.task_created", "grp", recordingHandler(durable))
...
func streamTo(t *testing.T, q queue.EventQueue, pattern string, j *journal) {
	t.Helper()
	if _, err := q.SubscribeStream(context.Background(), pattern,
		func(_ context.Context, topic string, ev *events.Event) {
			j.record(topic + "/" + labelOf(ev))
		}); err != nil {
