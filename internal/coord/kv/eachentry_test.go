...
func TestAListingThatEndsEarlyIsUnavailableRatherThanShort(t *testing.T) {
	...
	for _, delivered := range []int{0, 3} {
		t.Run(fmt.Sprintf("after_%d_entries", delivered), func(t *testing.T) {
			t.Parallel()
			var seen int
			err := eachEntry(context.Background(), truncatingKV{deliver: delivered},
...
// with it. eachEntry owns the watcher and stops it on every exit, so the walk
...
func TestAnAbandonedWalkLeavesNoConsumer(t *testing.T) {
	...
	const keys = 300
	// The undecodable record goes in FIRST, so the walk abandons with the
	// overwhelming majority of the bucket still undelivered.
	if _, err := store.budgets.Put(ctx, encodeKey("scope-bad"), []byte("{not json")); err != nil {
		t.Fatalf("plant the bad record: %v", err)
	}
	...
	if _, err := store.Usage(ctx); err == nil {
		t.Fatal("an undecodable budget record was not raised")
	}
