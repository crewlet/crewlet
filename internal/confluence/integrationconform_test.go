...
// nobody had run. This points the same seven cases at [confluence.Reconcile],
// over an instance the pass has already brought into line, and counts what
...
//     method-keyed counter is a habit that transfers wrongly.
...
// convergedWorld is a Confluence this pass has already converged, plus the
// pass itself as [integration.Reconciler] sees it.
...
func (*convergedWorld) Kind() integration.Kind { return integration.KindConfluence }
...
// sealed store alike.
func (w *convergedWorld) mutations() int { return w.site.mutations() + w.sink.writes() }
...
// the pass mint afresh and re-point all eight hooks — the single most likely
// way to make this harness fail for a reason that is about the harness.
//
...
func convergedConfluence(
	...
	var (
		...
		want     = len(confluence.WebhookEvents)
	)
	if deployment == confluence.Cloud {
		...
		variable, want = "CONFLUENCE_WEBHOOK_SECRET", 1
		cfg = &config.Confluence{
	...
	world := &convergedWorld{site: site, sink: sink, opts: confluence.Options{
		Client: client,
		...
		Sink: sink,
		// A NON-EMPTY BASE, because an empty one short-circuits before
		// either branch and registers nothing: every case below would
		// then be true of a pass that never looked at a hook.
	...
	if got := len(site.snapshot()); got != want {
		tb.Fatalf("the seeding pass left %d hook(s) registered, want %d", got, want)
	}
	if held, _, _ := sink.Value(context.Background(), variable); held == "" {
		tb.Fatalf("%s holds nothing after the seeding pass, so the next pass "+
			"would mint rather than keep", variable)
	}
	return world
}
...
func TestTheConfluenceReconcilerMeetsTheContract(t *testing.T) {
	...
	for _, deployment := range []confluence.Deployment{confluence.Cloud, confluence.DataCenter} {
		t.Run(string(deployment), func(t *testing.T) {
			...
			// calls in that order within one sequential case. A
			// Mutations bound to a world captured before the last New
			// would measure a world no case is running against.
			var world *convergedWorld
			integrationtest.Run(t, integrationtest.Reconciler{
				New: func(tb integrationtest.TB) integration.Reconciler {
					world = convergedConfluence(t, tb, deployment)
					return world
				},
