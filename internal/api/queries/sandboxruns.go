...
// one-member object keyed "…". A call that a server set below the ceiling
// refused is its least form: its name and outcome, each text no longer than
// its mark as it was and each longer one set aside for its mark, the
// arguments' object then saying they were set aside because a server refused
// the record. A cut call whose whole is kept in parts carries `whole_bytes`
// and `whole_parts`, the length of its whole and how many parts hold it under
// the call's record, which is what the run's resume reads it back whole from.
// One that carries neither says its whole could not be kept in the field that
// was cut: the arguments' object says they were not kept, and a cut output
// ends in a note after its "…". The answer never carries a part, nor counts
// one.
