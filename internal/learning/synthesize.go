...
func parseSkillDraft(c *llm.Completion) (skillDraft, bool) {
	...
	// Fenced JSON is what a model returns when it has been told to answer
	// JSON and cannot help itself.
	body = strings.TrimPrefix(body, "```json")
	body = strings.TrimPrefix(body, "```")
	body = strings.TrimSuffix(strings.TrimSpace(body), "```")

	var draft skillDraft
	if err := json.Unmarshal([]byte(strings.TrimSpace(body)), &draft); err != nil {
