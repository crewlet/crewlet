...
func (s *Skills) Update(ctx context.Context, skillID string, rev Revision, r Refinement) (Skill, error) {
	...
	// holds the database's write lock from its BEGIN, so the second refine
	// reads only after the first has committed, and the store's own retry
	// covers the transient failures that remain. The loop that used to sit
	// here retried EVERY error, because telling a write conflict from a
	// dead store once meant reading driver error strings that two drivers
	// worded differently. There is one driver, the discrimination is written
	// once in the store, and a second budget on top of it only multiplied
	// the attempts a genuinely-down store gets hammered with.
	var updated Skill
...
func (s *Skills) MarkUsed(ctx context.Context, skillID string, at time.Time) Use {
	...
	// [store.DB.Tx] holds the write lock from its BEGIN, so there is none to
	// survive. What the loop added on top was a second budget over every
	// OTHER error too (an outage included), which is the case that cannot
	// be helped by trying again.
	var use Use
	err := s.db.Tx(ctx, func(tx *sql.Tx) error {
