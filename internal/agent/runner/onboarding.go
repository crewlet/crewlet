...
func (r *Runner) onboardingPass(ctx context.Context, chain string) (bool, error) {
	// ONE REGISTRY FOR THE SURFACE AND THE PROMPT: the catalogue the pass is
	// shown is rendered from the tools it may call, so it never lists one
	// whose every call would fail.
	pass := unsequenced(r.cfg.Registry)
	surface, err := r.surfaceWith(ctx, phase.Onboarding, 0, nil, nil, pass.Snapshot(), nil, onboardingAlwaysOn)
	if err != nil {
		return false, err
	}
	system := prompts.BuildOnboarding(r.cfg.Seat, prompts.OnboardingInput{
		Hint:          learning.Hint(r.cfg.Seat.Org, r.cfg.Seat.Role),
		ToolCatalogue: pass.Catalogue(),
	})
...
// unsequenced is a copy of the registry without any tool that names its writes
// after the calls before it in the turn ([tools.Sequenced]) — the native
// tracker's writes — which is the universe the onboarding pass may discover
// and call, and the catalogue its prompt lists.
//
...
// do is lost.
//
// A COPY, because the registry is the seat's own and every later phase of the
// turn reads it whole.
func unsequenced(reg *tools.Registry) *tools.Registry {
	out := reg.Clone()
	for _, entry := range out.List() {
		if _, sequenced := entry.Tool.(tools.Sequenced); sequenced {
			out.Unregister(entry.Name())
		}
	}
	return out
}
