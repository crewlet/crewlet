...
func companyRules() []any {
	...
	confluencePresent := map[string]any{
		"properties": map[string]any{"confluence": map[string]any{"type": "object"}},
		"required":   []any{"confluence"},
	}

	// The knowledge backend is single-homed. Selection keys on block
	// PRESENCE, because the scope lists default to empty (unscoped) and so
	// cannot be the signal.
	exclusive := map[string]any{
		"$comment": "integrations.confluence and an enabled integrations.plane are mutually exclusive.",
		"not": has("integrations", map[string]any{
			"allOf": []any{confluencePresent, planeEnabled},
		}),
	}
	...
	confluenceScopeRejectsPlane := map[string]any{
		"$comment": "knowledge.confluence_spaces is a Confluence scope; it is refused when Plane is the backend.",
		"if":       has("knowledge", has("confluence_spaces", map[string]any{"minItems": 1})),
		"then":     map[string]any{"not": has("integrations", planeEnabled)},
	}
	return []any{exclusive, planeScopeNeedsPlane, confluenceScopeRejectsPlane}
}
