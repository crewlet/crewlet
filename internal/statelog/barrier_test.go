...
func TestBarrierNeverAcceptsADuplicateAck(t *testing.T) {
	t.Parallel()
	h := newHarness(t)
	idx, err := statelog.NewReadIndex(probeDomain{}, h.log, testSigner(t, probeDomain{}), probeEncode, h.gen.Load, nil)
	if err != nil {
...
func TestABarrierServedFromTheDedupeWindowIsRefused(t *testing.T) {
	t.Parallel()
	h := newHarness(t)
	idx, err := statelog.NewReadIndex(probeDomain{}, duplicating{h.log}, testSigner(t, probeDomain{}), probeEncode,
		h.gen.Load, nil)
	if err != nil {
		t.Fatalf("NewReadIndex: %v", err)
	}
...
func TestAReaderNeverJoinsABarrierThatWasAlreadyInFlight(t *testing.T) {
	...
	idx, err := statelog.NewReadIndex(probeDomain{}, gate, testSigner(t, probeDomain{}), probeEncode, h.gen.Load, nil)
	if err != nil {
		t.Fatalf("NewReadIndex: %v", err)
	}
