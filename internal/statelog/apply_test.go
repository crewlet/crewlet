...
// run twice on the operator's own hardware, and it reads zero by construction:
// the store's write transactions hold the lock from their BEGIN, so nothing
// committing elsewhere in the file can abort one. A non-zero count means the
// retry budget is being spent rather than held in reserve.
//
