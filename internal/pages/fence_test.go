...
// sequence trails a purge the floor already licensed. An unreadable bound refuses: failing open here is a
...
//
// An evicted node was refused with statelog.ErrConflict, which the page store
// translates into pages.ErrConflict and the page tools into "somebody else is
// editing this page — read it again", so a model on a node the fleet had
// removed was sent round a loop that could never land. A node below the floor
// was refused with ErrUnavailable wrapped in prose, so `below_floor` and `floor_unknown` — declared, documented reasons —
...
func TestEveryZeroFenceRefusalNamesItsReason(t *testing.T) {
	...
	wired := func(db *store.DB, floor func(context.Context, uint32) (uint64, error),
		ends func(context.Context) (statelog.LogEnds, error)) *pages.Fence {

		f := pages.NewFence(db, "node-a")
		f.Floor, f.Ends, f.Committed = floor, ends, standingAt(cursor)
		return f
	}

	for name, tc := range map[string]struct {
		build func(t *testing.T) *pages.Fence
		want  statelog.Reason
		cause error
	}{
...
func TestACheckpointAtTheLogsEndIsNotPastIt(t *testing.T) {
	t.Parallel()
	fence := pages.NewFence(openFenceStore(t), "node-a")
...
func TestAFenceMissingABoundRefusesZero(t *testing.T) {
	...
	onlyEnds := pages.NewFence(db, "node-a")
	...
	noCheckpoint := pages.NewFence(db, "node-a")
...
// evictInPages writes the row this node's applier writes when it applies its
// own eviction, which is the one source the fence reads.
func evictInPages(t *testing.T, db *store.DB, nodeID string) {
	t.Helper()
	if err := db.Replicated().Tx(t.Context(), func(tx *sql.Tx) error {
		_, err := tx.ExecContext(t.Context(), `
			INSERT INTO pages_evictions (node_id, at, by, from_position, version)
			VALUES (?, ?, 'operator', ?, ?)`,
			nodeID, store.EncodeTime(time.Now().UTC()), int64(50), int64(50))
		return err
	}); err != nil {
