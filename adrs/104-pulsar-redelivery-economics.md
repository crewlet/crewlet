# adr-104 — Redelivery economics on Pulsar, measured
...
- **The batch dispatch budget is deleted.** `_BATCH_DISPATCH_BUDGET_MS = 60 s`
  ...
  removes the republish that adr-101 §1 forbids anyway.
...
This is what makes `Unquiesce` reclaim the prefetch as adr-101 §3 requires.
...
opposite of adr-102 decision 1, and for the measured reason that decision gives:
...
adr-102 decision 4 puts the decision in the wrapper because JetStream ships no
...
not it. adr-102 decision 3 already removed the dependency.
