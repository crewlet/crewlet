# adr-406 — `-company` names a seed; the store is what a node serves

Status: **Accepted** · Applies to: `internal/engine/reconcile.go`,
`cmd/crewlet/reconcile.go`
...
## What `degraded` means here, and why it is not reachable
...
saying so is more useful than a hook with nothing behind it. Every step of an
apply either runs before the epoch is published or is undone by publishing the
previous one, so nothing in the path reaches the state the outcome names. It
becomes reachable the first time a subsystem that cannot be un-applied is
wired in, and such a subsystem is applied last, so the window in which
`degraded` is reachable stays as small as the ordering can make it.
