# adr-002 — The Turso SQL dialect we may actually use

Status: **Amended by [adr-003](003-turso-is-the-only-driver.md)** — §2 (NULL for
"unconstrained" instead of a partial-index `ON CONFLICT` target) is unaffected
and still load-bearing; the dual-dialect clauses are retired.

> **What adr-003 changed.** §1 (write in the intersection of two dialects) and §3's
...
1. ~~**Write in the intersection dialect.**~~ *Retired by adr-003.* All SQL had to
...
3. **Vector recall is brute-force, behind one query function** — one narrow function
   ...
   *Amended by adr-003.* The intent held and the outcome did not: the Go loop ran on
...
> *adr-003:* the middle clause did not survive contact. The fallback driver was real
