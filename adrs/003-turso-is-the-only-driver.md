# adr-003 — Turso is the only store driver

Status: **Accepted**

Applies to: `internal/store`, `internal/config` (Tier A `store:`), `.goreleaser.yaml`

Amends [adr-002](002-turso-sql-dialect.md), which decided to write in the
...
and second clauses; everything adr-002 measured is still true, and §2 (NULL for
"unconstrained" instead of a partial-index `ON CONFLICT` target) is unaffected
and still load-bearing.

## What changed since adr-002

Nothing about Turso's dialect — the matrix in adr-002 re-measures identically at
the pinned `v0.8.0-pre.7`, which is still the newest published version. What
changed is what the second driver was *for*.

adr-002 kept mainline SQLite as an escape hatch: a driver an operator could
...
2. **The knob is retired, not narrowed.** Tier A's `store.driver` and
   ...
   word, for the same reason adr-001 keyed it per tier: a `driver:` mistyped
...
4. **Keep the capability probe.** It stops being a comparison between two
   ...
   property adr-002 §5 asked for and it is worth more with one driver, not less.
...
- **`nextval` / `currval` / `setval`** — Turso sequences are per-database, and
  ...
  coordination KV precisely because N nodes must not keep N counters (adr-201,
...
- **NOT the static binary.** This is worth stating because it is the first
  thing the release-tooling note (adr-901) invites a reader to assume. Measured
  ...
  That is a real trade and it was made earlier, in adr-002, when Turso became the
  ...
  reopening adr-002, not this one.
...
- **adr-002 §2.** The nullable `work_key` and the plain unique index behind it
