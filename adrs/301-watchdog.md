# adr-301 — The watchdog watches the DUTY, not the loop

Status: **Accepted** · Spec: `docs/concepts/seat-ownership.md` § The wedged node

## "Is the runtime turning?" is the wrong question here

The obvious watchdog stamps a timestamp on a ticker and has a second thread
watch that stamp go stale. That design is complete only where **one scheduler
runs everything**: there, a stall stops the seat heartbeat, the queue consumers,
the turn engine and every hook at the same instant, so "is it turning?" is a
whole health question.
...
runtime**, and the runtime turning proves nothing about the duty. A watchdog
built the obvious way would therefore be green during the exact failure it
exists to catch: the heartbeat pass deadlocked on a mutex, blocked in
...
pulse alongside the duty check. It was dropped as strictly redundant: any stall broad enough to starve a plain ticker also freezes the
...
- **The threshold is the seat lease TTL and is not a config knob.** Past it the
  ...
  gets to be simultaneously "not the owner" and "still holding the mail".
  `NewWatchdog()` takes no threshold at all. The tight-threshold and no-exit seams are unexported, so only this
...
- **A GONE duty is not a WEDGED one.** From the watcher the two are
  ...
  That bug once killed this repo's own test suite at 63 %, exit code 75, with
  zero test failures. A watchdog whose duties have all stopped
...
On Pulsar the case is a measured one: a wedged-but-alive node keeps its broker
session open (the client answers keepalives from its own IO threads), so the
broker holds its **prefetch** of seats a peer now owns until that session dies —
there is no ack timeout to wait out — and exiting collapses it to 9 ms
(adr-104). On JetStream that specific hostage is smaller: pull consumers fetch
rather than receive, so a wedged node holds only what it had already fetched,
bounded by `AckWait` rather than by a 1000-message push queue (adr-102).
...
deadlocked mutex, a blocking syscall or a cgo call is not. So: **ending the
process is the only remedy available**, and it is the one that lets a supervisor restart into a
healthy node.
...
- It cannot stop the work — only the process, which is why correctness against
  zombies comes from epoch fencing rather than from the watchdog.
