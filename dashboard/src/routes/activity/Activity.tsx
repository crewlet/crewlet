 ...
 *
 * Mirrors `events.CategoryNames()`, which returns NINE, and two gates in
 * `internal/events` hold this list against it in both directions. It once
 * held ten when the engine had eight: `communication` and `knowledge` are
 * categories no event is registered under, so two of the chips could never
 * match a row and the reader was invited to filter a log down to nothing and
 * conclude the engine was quiet. A chip for a category with nothing in it is
 * still useful — it says the category exists and is quiet — but only where
 * the category exists. `auth` is the sign-ins, session ends, credential and
 * grant changes, coalesced failures and signature verdicts.
 */
const CATEGORIES = [
  "a2a",
  "auth",
