...
export function ItemsView({
  ...
  project?: string;
  /**
   * The page above has already said this company has filed nothing at all —
   * [NoWorkYet], which `Work.tsx` gates on the PROJECT list.
   *
   * TWO PANELS SAYING THE SAME THING IS ONE PANEL TOO MANY. This screen cannot
   * see the project list and that screen cannot see the rows, so neither one
   * can gate the other's sentence: the flag is how the page tells its list that
   * the question has already been answered, and the list then draws no empty
   * state of its own rather than stacking a second one under it.
   */
  firstRun?: boolean;
}) {
  const org = useOrg();
  ...
  const nothingShown = shown.length === 0 && answered.length === 0 && !inTrash;
  const foot = endNote({
    shown: shown.length,
 ...
 * that has no PROJECTS gets [NoWorkYet] from `Work.tsx`, which is gated on the
 * project list rather than on this answer — a read that failed must never be
 * rendered as a company that has filed nothing — and `firstRun` is how that
 * screen tells this one to stay quiet. At project scope the project's own
 * screen carries the container's empty state above this list, so (c) draws
 * nothing here at all.
 *
...
function EmptyList({
  ...
  if (scope === "all" || total === 0) {
    ...
    // one thing is one too many — the same rule `firstRun` keeps at workspace
    // scope.
