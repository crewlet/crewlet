...
- the tri-state (`unknown` ≠ `lost`) and each store's fail-open/fail-closed
  polarity — chosen per contract, deliberately, and never "fixed" to one
  uniform default;
...
- the measured constants and their provenance — a constant carries the
  ...
  re-derived against that broker rather than inherited;
- the non-promises — bounded duplication, not exactly-once.
