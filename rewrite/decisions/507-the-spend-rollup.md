...
That is the property worth having, and it is what "one implementation, never a
second one in JS" in the package layout means. This aggregation had three
copies once — the REST endpoint's, a re-implementation in the browser, and
whatever a reconnect left behind — and a refresh routinely disagreed with the
page it replaced.
...
`tests/test_dashboard/js/replay.mjs` now drives the captured frames through
