# d-405 — The event type system

Status: decided (built; this records it and closes the item)
Related: `103-payload-pointer-invariant.md`, `401`

## Settled

`internal/events` implements this decision item and `internal/events/types`
...
- **Trace context is passed at construction**, not read from an ambient span,
  matching d-401: callers that create events already hold the context that
  knows the span.
...
- **`Event.Data` is always the pointer form**, enforced by the `PayloadPtr`
  constraint at compile time. See d-103, which is where this was actually
...
Nothing in this item. The remaining event work is per-subsystem: each phase
that emits new events registers them, and `RegisteredTypes()` plus the test
asserting the registry matches the declared set is what keeps that honest.
