...
The flag survives a restart by BOTH halves of that rule, applied where each
one lives: the event-store writer stamps a `failed` tag from the payload's own
field, and the four types that *are* a failure carry no such field, so the type
half is re-applied when a row is read back (`EventLog.List`) rather than stored.
The projection seeds its feed from that answer at startup. The store's listing (`EventLog.List`) deliberately never
