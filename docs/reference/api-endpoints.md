...
| `GET` | `/agents/{id}/conversations` | The seat's own thread ledger: the threads it holds entries in, and one thread's turns when `conversation=` names it (the [`conversations`](#ws-wsstream) question) |
...
| `GET` | `/events/series` | The same rows **with a time axis** — a count per minute, hour or day under the listing's own filters (the [`event_series`](#ws-wsstream) question) |
| `GET` | `/events/trace/{trace_id}` | All events in one trace, oldest first, capped at 500 |
| `GET` | `/turns` | One row per **run** of a turn, newest first, keyset-paged (the [`turns`](#ws-wsstream) question) |
| `GET` | `/phases` | The company's phase records, newest first, **payloads included**, keyset-paged (the [`phases`](#ws-wsstream) question) |
| `GET` | `/phases/{id}` | One phase record **whole**: a record too large for one event was published cut, and this reassembles the whole it kept in parts; any other record answers with its own row (the [`phase_record`](#ws-wsstream) question) |
...
| `POST` | `/work/{id}/purge` | Destroy a task, on every node: its own rows, its comments and body revisions, and every reference another task holds to it. Its history rows stay, as the record that the work happened, emptied of their content — who, when and what kind of change are kept, and the activity feed marks each `content_purged` — beside the purge's own row, which carries its line whole; the inbox notices about it lose their excerpts the same way. Its subtasks move onto its own parent. The one operation with no inverse: `?confirm=` repeats the task's KEY, `?project=` names the container the record arbitrates under, `?reason=` is required and travels whole on the purge's notification — one too long for that line is refused with `400 reason_too_long`, whose `room` is how many bytes fit and `bytes` how many were sent, and nothing is purged — and `?op_id=` is how an `unknown` outcome is retried without appending a second purge. **Operator-only**, and absent rather than 503 on a build with no tracker |
...
| `GET` | `/work/workload` | Who is carrying how much open work, everybody at once (the [`work_workload`](#ws-wsstream) question) |
| `GET` | `/work/search` | The company's work ranked against a phrase (the [`work_search`](#ws-wsstream) question) |
| `GET` | `/work/routing/{record_id}` | Who one change woke, and under which reason (the [`work_routing`](#ws-wsstream) question) |
...
| `GET` | `/pages/activity` | What happened to a page, or to everything in a container (the [`page_activity`](#ws-wsstream) question) |
...
| `GET` | `/pages/{id}/revisions/{version}` | One revision's own body, message and author (the [`page_revision`](#ws-wsstream) question) |
| `GET` | `/containers` | Every knowledge container this node knows about, with how many pages each holds, trashed ones aside. The engine materialises one per `space:` the org chart names, plus the two reserved ones, on every config apply |
...
| `GET` | `/config/revisions` | One page of the history (newest first), metadata only, as `{revisions, truncated}` — `?limit=` (default 50, clamped to 500; zero or a negative number named no size and takes the default) and `?offset=` (a negative one is `0`). The bounds are ONE rule, applied by the function this route and the [`config_audit` query](#the-config_audit-query) both call, so the two answer one `limit` with one page. `truncated` is `true` when older revisions exist past the page, read as one revision past it rather than inferred from a page that filled; they are at `?offset=` plus the page's length, and `/config/revisions/{id}` answers any one directly. A non-numeric `limit` or `offset` is `400 invalid_pagination` |
...
Recent revision metadata for the dashboard's Configuration screen. **A query, not a REST route** — there is no `GET /config/audit` in this build; the screen asks the query channel for `config_audit` and gets the same answer `GET /config/revisions` serves — one page of revision records and whether older ones exist — from the same function, which applies the page bounds for both. Only PARSING differs: a `limit` or `offset` that is not a number is absent here, as it is for every integer parameter on the query channel, and `400 invalid_pagination` on the REST route.

```
query config_audit { "limit": <N>, "offset": <N> }
```

| Parameter | Default | Range | Description |
|-----------------|---------|-------|-------------|
| `limit` | `50` | `1..500` | Number of revisions to return, newest first. Above the range is CLAMPED to 500; zero, a negative number or a value that is not a number takes the default. |
| `offset` | `0` | `0..` | How many of the newest revisions to skip — where the next page starts. A negative number is `0`. |
...
`truncated` is `true` when older revisions exist past this page. It is read as
one revision past the page and never inferred from its length, because a
history of exactly `limit` revisions and one of a thousand return the same
rows; the older ones are at `offset` plus the page's length. A screen that
counts the rows it was given must say it is counting a page when this is set.
...
one row per `agent_phase_completed`, newest first, 50 to a page. A page with
older phases beneath it carries `next` — `{before_time, before_id}`, sent back
as those two parameters for that page — and the last page carries `null`,
which is the end of the record. Whether there is more is read as one phase
past the page, never inferred from a page that filled, so a seat with exactly
50 phases gets no cursor to an empty page. Half a cursor is `400`. The call
...
renderer, so a history row's record — nested as its `payload`, beside the
row's own `timestamp` and `failed` — names the fields a live call does:
...
A row may be a record **published cut**, because it was too large for one
event: its `whole_bytes` is then non-zero, and `GET /phases/{id}` (the
`phase_record` question) answers it whole — see *A record too large for one
event is published cut*, under [the live token meter](#the-live-token-meter).
...
the latest `agent_turn_progress` (phase, round, model, the last 4,000
bytes of the accumulated response *including the model's reasoning*, and
the phase's latest 48 tool calls and latest 48 narrated rounds, with how
many came before each counted in `tool_executions_earlier` and
...
the cap with nothing left behind keeps the window's own heading. The records
it does not hold are still in the store, and the `tokens` query with an
explicit `since` folds the window from there whole) and folds them with
...
24-hour spend window — read one record past the cap, so the store says whether
the window held more rather than the seed guessing it from a page that filled.
...
- A tool call's result and error, each round's reasoning and content, the
  round in flight and each attempt at it a provider abandoned
  (`partial_round`'s `abandoned`), and the joined `response` are sent
  **tail-bounded** — at most their last 4,000 bytes, behind a leading `…` when
  cut — because they are what a reader is watching the end of.
...
so a record it refuses goes out in two halves. First its **whole** — the
record exactly as it would have been stored — as `agent_phase_record_part`
events under ids derived from the record's own, which the event store keeps
and nothing lists (see `phase_record` below); a part that cannot be published
...
the tool results first, then the tool arguments, then the prose (the rounds,
the abandoned attempts, the response) and the prompts; failing that, each of
those texts reduced to its mark; failing that, its first rows carried and the
rest counted, the abandoned attempts given up first, then the tool calls, then
the rounds. The phase's own `error` is whole in every one of those forms: it
is cut only in a form with no row left that is still too large. A cut text
ends in `…`, a cut tool call, round or abandoned attempt carries its whole
length beside it as `<field>_bytes`, a text no longer than that mark is never
cut, and the record carries:

| Field | What it says |
|---|---|
| `whole_bytes` | How long the record's whole is. Non-zero exactly when the record was published cut |
| `whole_parts` | How many parts hold the whole, which `phase_record` reassembles. Zero when the record was published whole, or when its whole was not kept — its `notes` then say why |
| `abandoned_attempts_omitted`, `tool_executions_omitted`, `round_narration_omitted` | How many rows of each list the record does not carry, after the ones it does. Zero unless even the record's least form was too large |

Every form carries the scalars whole — the tokens, the cost, the model, the
decision, the phase — so a cut phase is still in every spend total. A record
...
| `event`    | Every engine event the activity feed files — each type with a [category](../guides/deployment.md#what-gets-stored-and-under-which-category) — as it is published to `crewlet.events.>`. A live-only type moves the projection without an `event` frame of its own (a round in flight, `agent_turn_progress`, reaches the client as the seat's `live_call` on the `agents` push), and the parts a phase record too large for one event keeps its whole in (`agent_phase_record_part`) are stored and never pushed. | `{ id, type, timestamp, source, actor, summary, category, trace_id, span_id, parent_span_id, topic, payload }` — the same shape as a `/events` row, plus the full event `payload` (from which the snapshot feed's `failed` flag is derived).  `agent_phase_completed` events carry the system prompt, response, and tool calls, so LLM invocations stream live. |
...
| `conversations` | `{handle, conversation, limit}` | `GET /agents/{id}/conversations`. The seat's own thread ledger — the engine's only account of what a seat said on a surface it does not own, and what stops it replying twice in one thread. TWO SHAPES IN ONE ANSWER, because a screen asks two questions with one navigation: `conversations` is the threads this seat holds entries in — one page of them, see `limit` below — and naming one in `conversation` adds that thread's turns as `entries`. Each turn's `reply` and `unsent` carry the same artifact and WHICH ONE HOLDS IT is the whole record of whether anybody received it — a turn can end with real work done and no way to say so. `limit` governs the THREAD ROSTER only (50 by default, 200 at the ceiling), newest thread first by its latest entry, and `truncated` says the seat holds threads past the page — read as one thread past it, because a seat that speaks on exactly `limit` surfaces is otherwise indistinguishable from one whose list was cut, and the thread somebody is looking for is the one that went missing. The roster takes NO CURSOR: past 200, the threads this seat spoke in least recently are listed by no answer, and `conversation=` reads a thread only for a caller that already holds its key. `entries` is everything the ledger holds for the named thread, with no page on it, because the bound there is the write-time trim (`turn_engine.conversation_session.max_entries`) rather than a page: one number served both reads and could only drop a conversation's OPENING on a company that raised the knob, since the store orders newest-first to make a `LIMIT` keep the recent turns. What the trim and the retention sweep removed is COUNTED rather than lost silently — each entry carries its `ordinal` in the conversation's whole record, so the oldest entry's `ordinal` minus one is how many earlier turns are no longer stored (zero claims nothing: an entry written before ordinals existed); those turns are readable only in the thread itself, on the surface it was held on. Same scope rule as `work_my_work` |
...
`/work/items/{id}`, `/work/views`, `/work/goals`, `/work/catalogue`,
...
| `config_audit` | `{limit, offset}` | One page of the revision history as `{revisions, truncated}` — see [the `config_audit` query](#the-config_audit-query); `GET /config/revisions` serves the same answer *(operator token required)* |
...
`task_activity`, `my_work`, `list_work_goals`,
...
**They are answered only while the company runs that backend.** A node that
booted on `tracker.backend: jira` registers no `work_items` question at all,
and one whose company moves to Jira by a live apply answers `unknown_query` /
`404` from the next call on, exactly as the first one does. That is
deliberate: an operator who wired Jira and then found a blank Crewlet board
would reasonably conclude their integration was broken. There is no native
record for this node to answer from, and an empty board would claim otherwise;
and a node that kept answering from the copy it held before the move would
show a board the company has left.
...
**The item surface is read-only, but for one gesture.** There is no
`POST /work`. An item is filed and moved by a seat's own tools, or by an
operator through the [MCP surface](../guides/tools-and-mcp.md), and both are
attributed to somebody — where a dashboard button would write as "the
dashboard", which is not a person and not a seat and cannot be asked why. The
one write about an item here is `POST /work/{id}/purge`, which has no inverse
and is refused without an operator identity to attribute it to. The `POST`
routes under `/work/retention/` below write too, and they are not about
items: they are operator gestures against the log's own history, attributed
to the token that made them.

### Paging and filters

Both listings take `limit` (default 50, max 500) and page the same way: by an
opaque cursor, returned as `next_cursor` on every answer that has more and sent
back as `cursor=` on the work listing and `after=` on the pages listing. Each
cursor is a keyset over the listing's own order, so the next page starts after
the last row read whatever moved in between — a work item is ordered on values
seats are changing while it is read, and a page by a title somebody can rename
— where an offset over a moving set skips rows and repeats rows with nothing to
say it did. A work board of several columns takes no cursor at all; one column
of it pages (see `work_items` above). The pages listing still takes `offset`
beside `after`, and it is a window at a numbered position rather than a way to
walk the listing.
  ...
  "runs": [
    {
      ...
      "bridge_calls_total": 1240,
      "bridge_calls": [
        {"seq": 1, "name": "read_page", "args": "{\"id\":\"ENG/Retry policy\"}",
         "output": "Idempotent calls are retried with backoff.", "at": "2026-06-08T07:13:02Z"},
        {"seq": 1240, "name": "read_page", "args": "{\"id\":\"ENG/Webhook client\"}",
         "output": "The client retries 5xx responses.", "at": "2026-06-08T07:29:51Z"}
      ],
      "bridge_calls_elided": 1040,
      "bridge_calls_elided_after": 100,
      "bridge_calls_next": 100
    }
  ]
}
```
...
**A bridged run's tool calls.** A run in agent mode calls its seat's tools over
...
- `bridge_calls_total` is how many calls the run's log holds, whatever the
  answer carries. A run that made no bridged calls has no `bridge_calls` at
  all.
- A first answer carries the log's **first** calls and then its **newest**:
  how the run set about the work and how it ended, which for a finished
  bridged run is its submission. Each end is at most `calls_limit` calls (100
  by default, at most 500) and at most one coordination record's worth of
  bytes, so an end made of heavy calls can be shorter.
- `bridge_calls_elided` counts the calls between the two ends, and
  `bridge_calls_elided_after` places them: they come after that many entries
  of `bridge_calls`. `bridge_calls_elided_after` is absent when nothing was
  left out.
- `bridge_calls_next`, present when the calls left out can be read, is the
  cursor to read them from: send it back as `calls_after`, with the run's
  `turn_id`. Each answer then carries the next page of the log, in order, and
  its own `bridge_calls_next` while the log goes on past it. The pages run on
  through the newest calls the first answer already showed, so a reader
  filling the gap stops after `bridge_calls_elided` calls. A page is bounded
  by bytes as well as by count, so it can carry fewer calls than asked for:
  the cursor, not the count, says whether there is more. And a `seq` can skip
  a number — an append that failed after taking one leaves it unused — so
  count calls rather than subtracting one `seq` from another.
- A run an older build recorded kept only its first and newest 100 calls, on
  the run's own row, and its calls may carry no `seq`. Its answer counts the
  calls that build dropped in `bridge_calls_elided` and places them with
...
The counts are **every delivery the event log holds** — its 30-day history
window back from now, counted by the store per source rather than folded from a
page of rows — and each response carries `traffic_since`, the timestamp of the
oldest delivery counted. "42 inbound" alone does not say over what span; "42
since Tuesday" is a measurement.
...
rather than from the inbound rows, counted over the same window as `inbound` —
the event log's whole history — so the three numbers on a row describe the
same span.
