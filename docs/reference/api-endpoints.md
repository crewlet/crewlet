...
Three questions answer about **one person** rather than about the company:
`work_my_work`, `work_inbox` and `work_person`. Every one of them takes a
`handle`, and the rule for whose is the same, in one place:
