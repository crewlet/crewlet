...
| `POST` | `/auth/bootstrap` | **The first person.** Redeems a one-time code a node wrote to a file beside its store, 0600, and creates an operator carrying the whole `max_grants` ceiling — the one stated exemption in the authority model. The code is checked **on the identity log**, so any node redeems a code any node wrote. The enrolment names the code as its authority, and the record honours it only while the code is live on the log and nobody else is enrolled, so a second caller racing the first is `409 bootstrap_closed`. A code that is real and no longer works — past its **24 hours**, withdrawn by a re-issue, or written by this node and never published — is `410 bootstrap_code_stale`, whose remedy is `crewlet iam bootstrap-code` (or a restart of the node that wrote it); a code nobody holds is the uniform `401`. **Unguarded**, and closed for good the moment anybody is enrolled |
| `GET` | `/auth/invite/{id}` | **Renders an invitation and never spends it** — a link is followed by mail clients prefetching, scanners and preview cards, and one spent by a GET is an account created for somebody who never saw it. **Unguarded**: holding the link is the credential. Answers the address it is for, who sent it, the password floor, and a `login` **proposed** from the address in the person grammar (`jane.doe@example.com` → `jane.doe`, `jane@example.com` → `jane.example`) for the form to pre-fill — and, where an identity provider is configured, `provider_start`: the provider sign-in carrying this invitation, to which the form appends `&login=` with the login the person chose, for somebody who would rather redeem it through the provider than set a password. Absent, redeemed and expired are one `410`, and every `410` on either invitation route is a **failed attempt counted against the caller's source** — the id in the link is the credential, so walking ids is guessing at one, and it is turned away at the same ceiling as a guessed password |
| `POST` | `/auth/invite/{id}` | Redeems it, conferring exactly the grants and reach whoever issued it decided — the enrolment names the invitation as its authority and the record refuses anything it does not cover, so a link spent or aged out between the form and the post is `410 invite_spent`. **Unguarded**. `{login, name, password}`, and the login is **required** — every person enrols with one, the name their changes are recorded under while they hold no seat. An absent login, or one outside a person's grammar (dotted, `jane.doe`), is `400` naming the rule and a login or address somebody already holds is `409` — without saying who, because a link is evidence of who the caller is and of nothing about anybody else. Only a record that could not land is `503`. **Retry until it lands**: the person a redemption creates is derived from the invitation, so every attempt names one person and a redeemer told their login is taken posts another. A link whose address somebody is already enrolled under is `410`, like a redeemed one. `POST /auth/bootstrap` takes `{code, login, email, name, password}`, accepts the code only while the log holds it live (not withdrawn, spent or past its 24 hours), derives the founder from the code the same way, and answers its enrolment the same way — except that a code that died between the form and the post is `410 bootstrap_code_stale` rather than `409`, because one command replaces it |
| `GET` | `/auth/oidc/start` `/auth/oidc/callback` | The identity-provider round trip, with PKCE and a sealed 10-minute flight cookie so a login begun on one node finishes on another. **Unguarded** — a browser following a redirect carries nothing this engine issued. **Absent** where no provider is configured. Linking is explicit: a subject nobody is **linked** to is refused, never provisioned or matched on an address. `?step_up=true` makes it a **confirmation** of the person already signed in (the session cookie must be presented, never a machine token): the provider is asked with `prompt=login` and `max_age`, the callback accepts only an `auth_time` inside the `step_up` window, and it replaces the session it was made from — see [Some gestures ask how recently you proved who you are](#some-gestures-ask-how-recently-you-proved-who-you-are). Every provider sign-in is proved at the ID token's `auth_time`, never at the instant it arrived. `?invite=<id>[&login=jane.doe]` makes the round trip a **redemption**: the start refuses a spent link (`410`, counted like every other) and a login outside the person grammar (`400`, and an omitted one is the proposal the invitation's page shows), seals both into the flight, and the callback enrols the person the invitation creates — its grants, its address, the login chosen, the provider's `name` as a display name and no password — and **links the subject the provider came back with** to them, then signs them in by redirect. The invitation redeemed is always the one sealed at the start, never one the way back names. A provider account already linked to somebody else, an invitation begun with a different provider account, and a subject a restore left linked to two people are `409 subject_conflict` — never `503`, because waiting clears none of them — and none spends the link. A provider whose discovery cannot be reached is `503` with a `Retry-After`; a start this node's own configuration cannot make — a provider block that does not validate, no keyring — is `500`, because waiting does not fix it |
...
| `bootstrap_code_stale` | Reachable only by presenting a code whose digest is on the identity log, or in the serving node's own file — so only by somebody holding a real one. Says it aged out, was withdrawn or never reached the log, and names `crewlet iam bootstrap-code`, because the alternative answers were both wrong: `sign_in_refused` sent a founder hunting for a typo in a code that was right, and `bootstrap_closed` told them a company still waiting for them had started. Counted against the source like every failed attempt |
...
`POST /auth/step-up` with a password and second factor, or, for somebody who
signs in through the identity provider, `GET /auth/oidc/start?step_up=true`:
...
**A provider's proof is dated by the provider.** A provider answers from its
own session whenever it can, so a sign-in through it is proved at the ID
token's `auth_time` — when the person last authenticated *there* — and never at
the instant the token arrived; a token asserting no `auth_time` proved nothing
this engine can date, which both windows read as stale. The provider step-up is
the provider's own round trip, asked with `prompt=login` and `max_age` set to
`step_up` (which makes `auth_time` required in the answer), and accepted only on
an `auth_time` inside that window — anything else is the sign-in refusal,
changing nothing. Like the password step-up it **replaces** the session it was
made from, ending it first and keeping its absolute deadline; the groups it
carries are the ones the provider just asserted.
...
the account whose `sub` claim that is, at the deployment's own provider
(`api.auth.oidc.issuer`) — one of the only **two** ways an account is ever
linked; the other is an invitation redeemed through the provider. An address
the provider asserts is never a link. `"oidc_subject": ""` unlinks them.

- It is a **claim** on the subject, like a login or a seat: a subject somebody
  else holds is `409 subject_conflict` carrying the `holder`, and never taken
  from them in passing — unlink them first.
...
- Refused **before the first record**: a deployment with no provider (`400`,
  ...
  asserts) and one past OpenID Connect's 255.
- Pinning the subject the person already holds publishes nothing.
...
so exactly one is live afterwards, and the new one lasts 24 hours. A re-issue
that could not read the outstanding codes, or could not land or confirm a
withdrawal or the new code's record, is `503` with a `Retry-After`; one that
...
| `config_entities` | `{kind, id}` | One addressable collection of the active revision: its ids, or one entity out of it. The read half of the Configuration screen, whose write half is `PUT /config/{kind}/{id}` *(needs `config:read`)* |
