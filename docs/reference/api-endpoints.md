...
which is what a reader opens the finished card for, with two exceptions. A
record too large for one event (`queue.MaxPayloadBytes`, 8 MiB) is published
with its longest texts cut to a common level until it fits, the tool results
first: each cut text ends in `…`, a cut tool call or round carries the whole
length beside it as `<field>_bytes`, and the record's `notes` say it was cut.
And a record refused as too large although it is within 8 MiB — by a NATS
server whose `max_payload` is set lower — is not published at all (see
...
`{seq, name, args, output, failed, at}`, in the order the run made them (the
...
- A run an older build recorded kept only its first and newest 100 calls, on
  ...
  copy of them exists.
- A call too large for one coordination record was cut when it was recorded:
  its `output` ends in `…`, and arguments that did not fit are replaced by a
  one-member object keyed `…` saying how many bytes were not kept. A call a
  NATS server set below the engine's ceiling refused is kept with its name
  and outcome only, both texts replaced by those marks.
