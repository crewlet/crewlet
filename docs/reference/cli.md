...
| `create` | Create somebody directly, with `-login` (required), `-email`, `-name` and `-kind` (`person` or `machine`). **Every principal enrols with a login** — it is the name their changes are recorded under while they hold no seat — and it follows the kind: a person's is dotted (`jane.doe`) and a machine's coloned (`ci:release`, or `token:<id>` to bind a Tier A token), and anything else is refused. A person also needs `-email`, which is how they sign in; a machine needs none. No password: a person arrives with one by redeeming an invitation, and a machine gets a token from `iam token` |
