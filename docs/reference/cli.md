...
| `crewlet jira provision <company.yaml>` | Report a [Jira](../integrations/jira.md) instance against the config: which account each seat's own credential authenticates as, whether every project the org chart names exists and agrees about its lead, and — on Data Center — register the inbound webhook with a minted secret. It mints no account credential of its own: a Data Center personal access token can only be minted for the calling user, and on Cloud the service accounts belong to [`crewlet atlassian provision`](#crewlet-atlassian-provision), so this run reports far more than it changes |
| `crewlet atlassian provision <company.yaml>` | Reconcile the config into an [Atlassian](../integrations/atlassian.md) Cloud organization: one service account per agent seat, a Jira and/or Confluence licence for each, and the account's API token and Atlassian-assigned address minted into the `${VAR}`s that seat's `mcp_env` already points at. A re-run leaves a working credential alone; `-rotate` mints fresh ones, `-handles a,b` narrows the run, `-decommission` deletes departed seats' accounts. A Data Center address is refused by name and sent to `crewlet jira provision`, and since Atlassian lets no API token place an agent in a project or a space, the run grants the licence and then reports the access each agent actually ended up with |
...
The Atlassian tracker's reconcile, and it is a different shape from its peers: **this command mints no account credential.** That refusal used to be stated of Atlassian as a whole, and it is half of one. It holds for a *user* account — a Cloud API token is created by the person it belongs to at Atlassian's own account site, and a Data Center personal access token can only be minted for the calling user — and it does not hold for a Cloud *service* account, which the organization admin API creates, gives a credential and licenses. So on Cloud the accounts are [`crewlet atlassian provision`](#crewlet-atlassian-provision)'s to make, and this command is the whole of what Data Center gets — and it is still the report a Cloud company wants beside the provisioner, because which account a seat authenticates as, and whether the projects the org chart names exist, are questions no provisioning run answers. What it does is the three things a Jira credential itself allows, each answering a question that is otherwise invisible until an issue reaches nobody:

1. **Which account each seat's credential authenticates as.** It calls `/myself` with every seat's own token, read from `mcp_env.atlassian`, `mcp_env.jira` or `mcp_env.confluence` — one Atlassian credential serves both products, so all three spellings are tried — and reports the seats that resolved and the seats that did not. A seat with no account id receives **no Jira events at all** — the routing gate drops every target that names it — and nothing else in the engine says so. On Cloud that finding is what [`crewlet atlassian provision`](#crewlet-atlassian-provision) repairs, by giving the seat an account; here it is reported and nothing more. Human seats are never probed: they hold no tool credential and are reached by `contact.atlassian_account_id` or by email.
...
| `-dry-run` | Read the instance and report; register nothing. The sink is not opened, so nothing is created, locked or written — `-env-file` creates its file on open, and `-secret-store` probes the store's lock. |
...
## `crewlet atlassian provision`

```
crewlet atlassian provision <company.yaml> [-secret-store | -env-file PATH | -print]
                                           [-admin-token KEY] [-handles a,b]
                                           [-rotate] [-decommission] [-dry-run]
```

The organization's own reconcile, and the third in this build that genuinely mints — beside GitLab's and Mattermost's — because the refusal it was written against is half of one: **Atlassian issues no credential for a *user* account and mints one for a *service* account.** With an organization API key created *without scopes*, the admin API creates the account, mints its token, and grants it a product licence. That is what lets an agent be an assignee, a watcher and an @mention target in its own right rather than acting through one shared company account.

**Cloud only, and it says so rather than degrading.** The account-management API does not exist on Data Center, where a personal access token can only be minted for the calling user — so a Data Center address in `integrations.jira.url` or `integrations.confluence.url` is refused by name, before anything is created, and pointed at [`crewlet jira provision`](#crewlet-jira-provision), which keeps the Data Center duties: the seat-identity report, the project and lead agreement check, and the inbound webhook. A product block naming no `cloud_id` at all is refused the same way and for the same reason. It is the one input that decides whether this command can do anything, and finding out from a 404 half way through a company leaves an operator working out which seats landed.

What one run does, in order:

1. **Resolves the organization and each product's site.** `integrations.atlassian.org_id` is the organization every account is created in; the site a licence is granted on comes from the product's own block — `integrations.jira.cloud_id` for Jira, `integrations.confluence.cloud_id` for Confluence. Naming a site per product looks redundant and is not: an organization that runs the two products on two sites licenses each agent where it will actually work rather than where the other product lives. The shape that cannot be true — one product with a cloud id and the other with a direct instance URL — is refused by [`crewlet validate`](#crewlet-validate) as half Cloud and half Data Center. The **`site_url`** the permission report's settings links are built from is a different field and a discovered one: `integrations.atlassian.site_url`, then whichever product block names a `site_url` or a browsable `url`, skipping any value on `api.atlassian.com` — a link built from the API gateway looks right and opens nothing — and only when nothing names one at all is the organization itself asked, in one request. That request is allowed to fail: an organization running two sites has no single answer, so a failure prints a note and the report names each container without a link rather than refusing over a field that changes no write. A **cloud id** is never discovered and has to be stated per product — it decides which site a billable licence lands on, and guessing licenses an agent into a site nobody chose.
2. **Walks the org chart for the seats it can finish.** An **agent** seat qualifies when its `mcp_env` names a whole `${VAR}` for **both** a token key and an address key. Cloud authenticates Basic `base64(email:token)`, so the address is half the credential: a seat naming a token variable and no address variable is reported and skipped rather than minted into, because a token it cannot present reads like a bad credential rather than an incomplete one. **Every** Atlassian block that holds a token is read — `atlassian`, `jira` and `confluence`, not the first of them — because a seat running a single-product `jira` server beside a `confluence` one has a variable in each, and minting into one of them leaves the other server starting with an unset credential: no error, no note, a seat half provisioned in silence. One account and one credential land in every variable both blocks name. A credential key that is **not** a whole reference makes the *whole seat* hand-managed rather than provisioning the rest of it: three referenced keys and one literal would otherwise leave the seat still authenticating through the fourth, which is the hardest stale credential there is to find, because everything else about the seat looks freshly rotated. The note names the offending key and the *shape* of what is in it, never the value — the report is something an operator pastes into a ticket. An `Authorization: Basic …` value is reported the same way for a different reason: its payload already carries the address and the token together, so there is no `${VAR}` under it to mint into and nothing to fix — point a named token key (`ATLASSIAN_API_TOKEN`) at a variable to provision the seat instead. Two seats pointing at one variable is refused at plan time, before either has been minted over. Human seats are never provisioned: they hold their own Atlassian account, and creating one would be a billable licence nobody authenticates as.
3. **Adopts or creates the service account.** Adoption matches on the display name — `<display_name_prefix> <role name>`, case and spacing ignored — because Atlassian assigns both the account id and its address, so the display name is the only field the org chart and the organization both control. Crewlet never chooses that address: it is read back off the account and recorded into the seat's address variables.
4. **Asks the credential the seat already holds who it is**, once per product it is enabled for (below), and that one answer decides both of the next two steps: whether the seat needs a new credential, and which of its licences are owed. An account this run has just created has neither, so it is not probed and everything is owed. `-rotate` does not skip it: the verdict is overridden so the seat is minted regardless — except **Other**, which still stops the run — while the owed list still governs the licences, so a rotation does not re-buy access the agent already holds.
5. **Grants a licence only where the probe found one owed**, waiting out a just-created account's directory delay (2s, 4s, 8s — Atlassian answers a grant against an account it cannot see yet with a 404 that reads exactly like "no such account"), and waiting only for such an account: the same answer from one that already existed is a real fault, not a directory that has not caught up. A grant is idempotent, so sending one per product per seat on every run looks free. It is not — it is a write per product per seat forever, and it makes every licence look freshly granted, which turns a container the agent genuinely cannot see into "still starting" for ever (step 7). A licence somebody revoked in the console is still repaired without anything being remembered: the agent's own credential is then refused on that product, and that refusal *is* the evidence a licence is owed. So the report's "licensed for" lines name only what this run actually granted, and a steady-state run prints none. 6. **Mints, records, retires.** The address is recorded before the token, because the other order leaves a run that died between the two with a seat holding a token it cannot present — which looks like a bad credential rather than an incomplete one. An account Atlassian gave no address at all fails the seat and rolls the run back rather than recording half a credential: with no address the token is sent as a bearer, Cloud refuses the scheme, and the seat reports as freshly rotated while authenticating as nobody. The superseded credentials **this tool** minted for that seat are revoked *after* the new one is recorded — never before, or a failed record leaves the seat with nothing. They are recognised by the label prefix `crewlet-<handle>-`; an administrator's own token on the account is left alone, because nothing here knows what is using it.
7. **Reads back, as the agent, what access it actually has** in every project and space the org chart names — plus `knowledge.confluence_spaces`, where every seat's Plan-phase search runs and where an unreadable space is indistinguishable from a company that has written nothing down. Each container comes back one of four ways: **missing** permissions the agent contract asks for — yours to grant, with the settings URL and whether that screen is a team-managed project, a company-managed one whose permission scheme is shared with other projects, or a space's permission grid; **holding** ones the contract forbids, attached by the site's own permission scheme, which Crewlet neither granted nor can revoke; **still starting**, where the licence was granted *in this run* and Atlassian applies one asynchronously, minutes at a time — and because the next run grants nothing it does not owe, the same container comes back as a plain finding on that run if the agent really cannot reach it; or **unreadable**, where the check could not run and the report says why. Only containers with a finding are listed and the clean ones counted — a line of "ok" per seat per project is a page an operator scrolls past, findings and all.

| Flag | Description |
|------|-------------|
| `-admin-token` | An Atlassian **organization API key**, created *without scopes* at admin.atlassian.com → Settings → API keys: the account-management API refuses a scoped key with 403 whatever scopes it holds, which is the same answer as no permission at all. Falls back to `$ATLASSIAN_ORG_API_KEY`, then `$ATLASSIAN_ADMIN_TOKEN`, and it is read from the environment **only** — never the secret store. It is the operator's own credential, it may create billable accounts across the whole organization, and the store is replicated to every node holding the keyring. The seats' credentials are what this run mints, so it cannot bootstrap itself from them. |
| `-secret-store` / `-env-file PATH` / `-print` | Where minted credentials go — exactly one, and there is no default: a run with nowhere to put what it mints creates live credentials at the vendor and prints none of them. Two values land per seat, the address and the token. A run that recorded something says what still has to happen for those values to reach a running engine — `-secret-store` needs the current revision re-activated ([`crewlet config activate`](#crewlet-config-activate)). See [the secret store](../concepts/secret-store.md). |
| `-handles a,b` | Provision only these seats. It narrows **provisioning** and nothing else: `-decommission` still reads the whole plan, because `-handles a -decommission` deleting every other seat's account would be the opposite of narrowing a run. |
| `-rotate` | Mint a fresh credential for every seat, including seats whose current one still works. **Restart the engine afterwards.** |
| `-decommission` | Delete the service accounts of seats that have left the config, scoped to display names beginning with `integrations.atlassian.display_name_prefix` — which is never empty (it defaults to `Crewlet`), and that default is the whole of what stops the sweep proposing the organization's people. What it keeps is **every agent seat the org chart has**, not the seats this run provisioned: a seat that opted out of every product, one whose credential is managed by hand, one naming no address variable is in none of them and may still hold an account an earlier run created. So narrowing a seat's `integrations.atlassian.products`, or taking its Atlassian credential out of `mcp_env`, keeps its account — deleting the seat from the org chart is what retires it. It runs after every minted credential is durable, because deleting an account is the one step no rollback can undo, and it is genuinely destructive: Atlassian has no disable verb, so the account that reported an issue stops existing and its history goes with it. |
| `-dry-run` | Print the plan and touch nothing. It is the **same** plan the run uses. The sink is not opened, so nothing is created, locked or written — `-env-file` creates its file on open, and `-secret-store` probes the store's lock. |

**A plain re-run does not rotate a working credential.** Atlassian returns an API token's value once, so a provisioner cannot check that what it recorded last time still matches — and minting every run is an outage, because the engine is running with the *old* value: an operator adding a tenth seat would revoke the nine credentials the other agents are authenticating with. So the run proves it instead. It reads back what the sink holds for the seat and asks Atlassian who that credential is, **once per product the seat is enabled for**: an answer naming this seat's own account keeps it and proves that product's licence, a refusal both mints and marks the licence owed, an answer naming a *different* account stops the run rather than minting a second identity onto a variable somebody else is also using, and a 5xx or a dropped connection leaves the credential exactly as it was with a note — re-minting on "cannot tell" destroys one that works. Asking per product is what catches a stale scope set: token scopes cannot be read back from Atlassian at all, so a seat that has gained Confluence holds a Jira-only token that looks healthy until its first real wiki call. Nothing is remembered between runs — no ledger, no local table — so a steady-state re-run performs no writes at all: the probe is a read, and a licence the credential already reaches is not re-granted. What it mints lasts `integrations.atlassian.token_expiry_days`, 1 to 365 and 300 by default, and nothing renews it on a schedule: a re-run after the expiry date mints a replacement, and `-rotate` before it is how you avoid the gap in between.

**`-print` is the exception, and it is not a small one.** That sink hands the values to a terminal and persists nothing, so it answers *not held* to every read — no seat's credential can be proved, so every run mints a replacement, re-grants every licence, and revokes what the previous run printed. A second `-print` run against a live fleet takes away the credential its engines are authenticating with. That is true of every provisioning command in this build, and it costs the most on this one, because the retire step is what removes the running credential. Use it for a deployment's first run, or to paste into a system this binary knows nothing about; a command you intend to re-run belongs on `-secret-store` or `-env-file`, which can answer the probe.

**Crewlet cannot place an agent** in a Jira project role or a Confluence space permission. Atlassian refuses that to an API token outright — only a Forge app on a paid plan may do it — so the run grants the organization-level licence and then reports what the agent ended up with, rather than issuing a write that silently does nothing. Everything the access table calls missing is granted by a person, on the screens it names.

A run that cannot record what it minted revokes it. An account this run created is deleted whole; an account it adopted loses exactly the one credential this run minted, because it owns issues and appears in history; and a licence granted on an account that already existed cannot be withdrawn — Atlassian offers no route — so it is named for the operator instead of being quietly kept. The cleanup runs on a detached context, because the failure it is undoing is often the cancellation itself.

See [Atlassian Integration — Provisioning](../integrations/atlassian.md#provisioning) for the permission contract and the full walkthrough.
...
| `-dry-run` | Read GitHub and report; register nothing. The sink is not opened, so nothing is created, locked or written — `-env-file` creates its file on open, and `-secret-store` probes the store's lock. |
