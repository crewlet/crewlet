...
is a message nobody ever answers. A third-party app that fires one webhook per changed
...
needs to know nothing about the third-party app, which keeps three routes from each
...
theory that a config naming a third-party app the build could not serve should fail
loudly rather than be silently ignored. That mechanism is **gone**, because the
premise stopped being true: all six third-party apps — Mattermost, Slack, GitLab,
GitHub, Jira and Confluence — route end to end, so there is nothing left to
refuse. The table it kept held four rows, and each was struck as that third-party app
shipped its parser.
...
the messages never arrived. If a third-party app is ever added to the schema ahead of
...
An operator wiring a collector should be able to paste the third-party app's snippet.
...
All external dependencies (LLM, embeddings, storage) are behind interfaces **defined by the package that calls them**, kept to what that caller needs — there is no `interfaces.go`, and a provider package exports a concrete type. No third-party app SDK lock-in. This enables:
