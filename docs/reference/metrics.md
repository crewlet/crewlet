...
| `crewlet.statelog.apply.records` | `1` | `domain`, `result` | Records consumed, by what happened to them: applied, retained, gated, skipped, or reprocessed by a build that could read what an earlier one retained. A node applying nothing while its position advances is healthy on lag alone. |
...
| `crewlet.statelog.records_tampered` | `1` | `domain` | Records whose signature failed under a key this node holds, which means something that is not this fleet wrote to the broker. The applier stops on the first one, so this counts at most one per domain per stop, and any value above zero is an incident rather than a rate. |
| `crewlet.statelog.records_unverifiable` | `1` | `domain` | Records signed under a key this node does not hold, retained until it does. Expected and transient during a keyring rotation; sustained, it means a key reached one node's secrets.keys and not this one's. |
| `crewlet.tracker.search.answers` | `1` | `coverage`, `semantic` | What each answer actually covered: whether every bucket of the corpus was scanned, and whether the semantic half ran. Both alarms below it are a FRACTION of this counter, and a short answer is indistinguishable from a short corpus without it. |
...
| `crewlet.auth.attempts.failed` | `1` | `method`, `outcome` | Every failed attempt to prove an identity, as it happens: a sign-in, a step-up, a founder code, a provider round trip that ended in nobody, or a bearer presented to a route that needs one and refused — a credential an unguarded route verifies itself, such as a webhook relay's own JWT, is never counted (`method`), and whether the attempt was verified and wrong or turned away at the throttle's ceiling (`outcome`). It is the ONLY per-attempt record there is, by design: an unauthenticated caller authors this rate, so the event log gets one coalesced `iam_login_failures` row per client per minute and the rate itself lives here, where a guessing run costs a counter increment rather than a row in every backup. |
