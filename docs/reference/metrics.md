...
| `crewlet.statelog.apply.tx.aborts` | `1` | `domain` | Apply transactions whose body the store ran more than once, because an attempt failed transiently after it began. It reads zero by construction: the database detects write conflicts per file, so every write transaction holds the file's lock from its BEGIN, and no commit elsewhere in the file can abort an apply. A non-zero count on the operator's own hardware means the retry budget is being spent rather than held in reserve. |
