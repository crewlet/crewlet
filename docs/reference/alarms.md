...
| `read_refusals` | Reads are being refused for something other than ordinary lag, and have been for longer than a heartbeat. | Read the refusal code in the logs. Anything other than `behind`, `too_stale` or `lag_unknown` is a fault rather than a wait. |
...
| `search_slow` | Interactive search is over its target. The corpus has outgrown one shard's index. | The corpus has outgrown one shard's index. See docs/guides/search.md for partitioning. |
...
| `search_scoped` | Searches are being answered over part of the corpus because a shard was unreachable. | A shard was unreachable. The answers were complete for what was searched and silent about what was not. |
