...
`knowledge.vectors` is on, reading the provider, the model and the switch
afresh on every tick — so a revision that turns vectors on or off, or changes
the model, takes effect at the next tick with no restart.
