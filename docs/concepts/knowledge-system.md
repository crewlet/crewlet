...
flowchart TD
    ...
    DIARY["agent_diary<br/>pgvector + agent_id + kind/ttl"]
...
- **`exclude_ancestors`** drops hits whose ancestor/parent chain matches any listed title. The prefetch defaults it to `["Auto-Drafted Skills"]` (`AUTO_DRAFTED_PARENT` in `knowledge/protocol.py`) so unreviewed [promotion drafts](agent-learning.md) never surface before a lead publishes them.
