...
| How search works | keyword (BM25 over the node's own lexical index), fused with semantic (two-stage 1-bit retrieval with an exact rerank) while `knowledge.vectors` is on | CQL against the site's search API, live at query time |
...
dashboard and a seat say which: the seat's turn-start block and its own
`search_knowledge` tell it the knowledge base is not searchable from this node
yet, and to ask a colleague rather than conclude nothing has been written down
([the exact sentences](#native-backend--the-engines-own-pages)) — because a
seat that read an empty result would act on it by writing a page that already
exists. A `search_knowledge` answer that did find pages on such a node keeps
them and says the list may be missing pages that exist.
...
document's first 600 bytes cannot centre on a match that is deeper in, so it
would show every hit on a long page as the page's preamble, and a snippet that
does not contain the search term reads as a wrong result even when the ranking
is right. The bodies are read per query for the ranked hits alone, never for
the corpus, and a read that cannot be taken falls back to the stored opening
rather than failing the search. That opening is at most 600 bytes
...
vector can only represent a few thousand words, so a long page is split into
overlapping 4 KB windows with the title repeated on each (a window that does
not say which document it is from matches a query about that document's
subject no better than anybody else's page four), and each window gets its own
vector. One vector over the page's opening would leave everything after it
unreachable by meaning — a handbook whose rate-limit section is on page four
would answer nothing to "how do we handle rate limits", from a corpus that
holds the answer.
...
that would otherwise not be searchable past their opening.
...
bill once and holds the answer everywhere. It embeds only while
`knowledge.vectors` is on, and takes the provider, the model and the switch
from the revision the node is serving, afresh on every tick — so a revision
that turns vectors on or off, or changes the model, takes effect at the next
tick with no restart, and a revision the node refused changes none of them. A
search embeds its query from the same revision's provider and model.
...
**A single node reads every bucket.** A fleet above 10 000 documents divides
them: each live node scans a contiguous range, returns its best candidates
*with scores*, and the asking node merges. There is no routing plan to compute and nothing to configure — the
...
Shared knowledge is read straight from the backend on demand, so there is no sync worker to run, no index to keep fresh, and no staleness window. It authenticates as the agent's own user where the seat has a credential of its own (the org token otherwise, [below](#confluence-backend--the-confluence-searcher)), which is what makes the backend's own permissions the ones that apply.
...
The two reads are independent: the diary is read by hybrid candidate selection (vector top-K ∪ recency top-K → aux-LLM relevance filter), scoped to the calling agent; the knowledge base is read through whichever backend the company wired — this node's own applied rows natively, a live query at the site on Confluence — scoped to the org-wide `knowledge.scope`, which is the same for every seat. Neither depends on the other, and each renders into its own block of the executor's prompt.
...
type Hit struct {
    ...
    Container      string   // a native container or a Confluence space key
    PageID         string
    ...
    AncestorsKnown bool     // Ancestors is the WHOLE chain, so an empty one
                            //   is the top of the container rather than a
                            //   chain that did not come back
}
...
type Query struct {
    ...
    Prefetch         bool              // a turn's own context assembly asking;
...
type Searcher interface {
    ...
    // first build, so a search here can miss pages that exist. Required
    // of every backend; one with no index of its own answers false. No I/O.
    Building(ctx context.Context) bool

    // Search returns up to Query.Limit ranked hits, and what the ranking
    // behind them is missing. It never reports an error: every failure path
    // is an empty answer marked Failed.
    Search(ctx context.Context, q Query) Answer
}

type Answer struct {
    Hits      []Hit
    Partial   *Partial // what the ranking did not cover; nil for a whole answer
    Truncated bool     // fewer hits than the limit while the backend ranks more than it read
    Failed    bool     // the search did not complete; Hits says nothing
}

type Partial struct {
    BucketsAnswered int      `json:"buckets_answered"`
    BucketsMissing  int      `json:"buckets_missing"`
    AbsentNodes     []string `json:"absent_nodes"`     // whose share went unscanned
    SemanticSkipped bool     `json:"semantic_skipped"` // matched on words alone
}
```
...
- **Unscoped-vs-nothing is enforced inside `Search`** on a backend whose seats carry credentials of their own — Confluence: empty scope + a self-authenticating role ⇒ unscoped search (the backend's own ACLs bound the hits); empty scope + a credential-less role ⇒ no results. The native backend has no per-seat credential and no second account to read through, so an empty scope there is the whole company.
...
- **`Building` separates "not indexed yet" from "nothing matched"**, also with no I/O. The native backend answers it from its index's first lap over the pages; Confluence keeps no index and answers false. It is a required method rather than an optional one, so an adapter between the engine and a tool cannot drop it without failing to compile.
- **Best-effort, and a failure says so**: `Search` never reports an error, and a failure is an empty answer marked `Failed` — which the turn-start block and `search_knowledge` render as a failed search rather than as nothing matching (the sentence is [below](#native-backend--the-engines-own-pages)). A seat told "nothing matched" tries other words against a search that is not running, or writes the page it could not find. Both searchers mark every failure path: natively a fan-out, a read-back or a parent-chain read that fails; on Confluence a query the site refused, a search with no credential to run as, and one the scope rule refuses. Empty query text is the one empty answer that is not marked, because nothing was asked.
- **A partial answer says so.** An answer ranked over part of what it was asked about carries `Partial`: how many of the corpus's buckets went unscanned and whose they were, and whether the half that matches by meaning did not run. Every reader renders it — the turn-start block, `search_knowledge`, and the dashboard's `knowledge` answer as a `partial` field — because a short list with nothing beside it reads as everything that matched. A live Confluence query has no buckets and no second half, so its answers are never partial.
- **A truncated answer says so.** An answer with fewer hits than its limit while its backend ranks more matches than it read carries `Truncated`: the backend read its ranking to a depth, the exclusions took places among what it read, and a page ranked below that depth is missing. The whole knowledge base was ranked, so it is not partial; it was read short. Only the Confluence searcher sets it, from the site's own answer — a search that links a next page (`_links.next`) ranks matches past the rows it returned, which no count of those rows can tell. Every reader renders it too, in one sentence — the turn-start block and `search_knowledge` set apart as their other notes are, and the dashboard's `knowledge` answer as a `truncated` flag with the sentence in its `note` (beside the partial one when an answer is both):

  > (this answer has fewer pages than were asked for because pages the search leaves out, such as unreviewed drafts, took places among the matches it read, and the knowledge base ranks more matches than it read. Before concluding a page does not exist, search again in more specific words)
- **`Query.ExcludeAncestors`** drops hits whose ancestor/parent chain matches any listed title. Left nil it takes the default, `"Auto-Drafted Skills"` (`knowledge.AutoDraftedParent`), so unreviewed [promotion drafts](agent-learning.md) never surface before a lead publishes them; an empty, non-nil list disables the exclusion. Every draft title also carries the `[Auto-draft] ` prefix (`knowledge.AutoDraftTitlePrefix`) as a fail-closed backstop: a hit whose chain the backend could not read whole (`Hit.AncestorsKnown` false) is dropped when its title carries the prefix and the auto-draft parent is among the exclusions. A hit whose whole chain was read is judged by the chain alone, an empty one included — so a draft a lead moved out from under the parent, prefix kept, is published wherever it landed, because moving is the gesture that means reviewed. Both backends apply this through one function, `knowledge.Excludes`, so a page is kept or dropped the same way on either.

**Selection is by `knowledge.backend`, and single-homed.** Every search is answered by the backend the revision a node is running names — the native searcher over this node's own page index, the Confluence searcher, or nothing — never by whichever searcher the node happens to hold. One knowledge home is what makes the turn-start prefetch, the `search_knowledge` builtin and the onboarding hints agree about what the company knows: two searchers would make an agent's answer depend on which was asked, and neither would be wrong. With `backend: none` nothing answers, and the `## Relevant knowledge` block is left out.

A live move takes effect at the apply, with one exception. The Confluence searcher is built for a revision on `backend: confluence`, at boot or at the apply that names it — a `backend: none` company may keep an `integrations.confluence` block for its page routing, and gets no searcher from it — and the apply that moves off Confluence stops it answering. The native searcher starts with the node: a node whose boot revision runs the native knowledge base holds it, and its copy of the pages, for as long as it runs. So a revision that moves a company off the native knowledge base stops a node answering from it at once, and one that moves the company back answers from it again — while a node that booted on another backend has no native searcher, and answers nothing for a native company until it restarts.

**When a search cannot run, `search_knowledge` says which of three states it is in**, in the words of whatever refused it, because each sends a reader to a different setting:

- the company runs **no knowledge base** (`knowledge.backend: none`). The tool is offered to nobody then — seat or operator — so a turn meets this only across the apply that turned it off;
- it runs one this node is **not serving**: a Confluence connection that did not build on this node, or whose org token (`integrations.confluence.token`) resolves to nothing — or the engine's own knowledge base on a node that started on another backend, which only a restart starts;
- this node serves it and the search has **nothing it may read**: no `knowledge.scope` is declared, and the search has no credential of its own to search unscoped with.

The dashboard's `knowledge` answer reports the same three as its `reason`: `no_backend`, `not_served` and `no_scope`.

An empty `backend` **derives** rather than defaulting blindly: a company that declares `integrations.confluence` gets `confluence`, and one that declares nothing gets `native`. So an Atlassian company that has not read this page keeps Confluence, and a quickstart company gets a wiki without asking for one.

**The seam has two implementations.** `knowledge.Searcher` belongs to the package its readers share — the turn-start prefetch, the `search_knowledge` builtin, the API's knowledge query — rather than to either backend, so each backend is one implementation of it rather than a second path through everything that searches. A seam collapsed into one backend is what makes the next one a rewrite.
...
**A search is hybrid whenever the company's search has a semantic half** — `knowledge.vectors` on, as it is when left unset on a company that configures `providers.embeddings`. Each search then embeds its query **once**, with the same provider and model the [embedding duty](#where-the-vectors-come-from) embeds the pages with, carries that vector to every node that scans a range of the corpus, and fuses the two ranked lists — so a page that shares no word with the query but means what it asks is found. The query and the corpus share one source because a query vector is only comparable with vectors of its own model and width. With `knowledge.vectors: false` nothing is embedded, pages or query, and every search is keyword-only. A query the provider will not embed is answered on its words alone, and the answer says so (below).
...
- **An index that is still building says so.** It is a different fact from an empty company, and a seat is told which, in one sentence wherever it asks — the turn-start `## Relevant knowledge` block, which skips its search, and the seat's own `search_knowledge`, when its search comes back empty:

  > (the knowledge base is not searchable from this node yet — it is still indexing. Pages that exist will not be found by a search right now, so ask a colleague who would know rather than concluding nothing has been written down)

  A seat that read an empty result would act on it, by writing a page that already exists. Where a fleet [divides a search](#every-document-carries-a-bucket), a building node's `search_knowledge` can still find pages, because its peers answer their own bucket ranges; it keeps them and ends the answer with a second sentence, since a list that is real and incomplete, read as whole, sends a seat to the same place:

  > (this node is still indexing the knowledge base, so this answer may be missing pages that exist — before concluding a page does not exist, search again shortly or ask a colleague who would know)

  A PARTIAL ANSWER and a FAILED ONE each say so too, in one sentence wherever a seat meets them. A partial answer keeps what it found and names what it did not search — buckets a node did not answer for, or a query the provider would not embed:

  > (this answer is partial: 22 of the knowledge base's 64 slices were not searched, because the node searching them did not answer in time or is still indexing — searching again shortly may find pages this answer does not name. Before concluding a page does not exist, search again or ask a colleague who would know)

  and a search that did not complete says that, rather than that nothing matched:

  > (the knowledge search failed, so it says nothing about whether a page exists — search again shortly, or ask a colleague who would know)

  The gate is this node's FIRST BUILD of the pages — one lap over them — and not "nothing is waiting to be indexed": a page saved a moment ago is ordinary staleness, and a gate read off a pending count would make every empty search on a company with people in it answer "still building" instead. The same index holds the work items, which finish their own first lap separately: a knowledge search does not wait for theirs, and a work-item search does not wait for the pages'. Each corpus's first lap is logged once, as `lexical_index_built` with the corpus (`source`) and how many of its documents the index still has no row for (`pending`), or why that count could not be read (`pending_error`). After the first lap a search is a true answer over slightly older rows, which is what a search always is — except that a page trashed, unpublished or purged since it was indexed is not returned, because the read that follows the ranking sees it has gone.
...
`Query.ExcludeAncestors` applies here too. The native searcher reads each hit's parent chain from this node's own page rows, as titles outermost first — the same chain a page's breadcrumb shows — and drops a hit whose chain carries an excluded title at any depth, compared without regard to case. So with the default, a page anywhere under a page titled `Auto-Drafted Skills` is not returned. A walk that reaches a page with no parent has read the chain whole, so a page at the top of its container has a known, empty chain; a walk that stops on a parent loop (two moves made at the same moment can close one, as above) names every page it can reach and counts as whole too. A draft moved out from under `Auto-Drafted Skills` is therefore returned wherever it landed — under another page or at the top of its container — with its `[Auto-draft] ` prefix still on. The prefix hides a page only when its walk runs into a parent this node holds no page for, where what was above that parent is unknown. The same read drops a hit whose page was trashed, unpublished or purged after it was indexed. A chain that cannot be read makes the whole search answer empty, marked failed, rather than answer without the exclusion.

### Confluence backend — the Confluence searcher

`internal/confluence` (`confluence.Searcher`). The query text is wrapped into a Confluence CQL `text ~ "..."` clause (`confluence.BuildCQL`), optionally narrowed by `space IN (...)` from the [read scope](#accessible-containers), and run against the Confluence REST API (`/rest/api/content/search`), so Confluence's own search backend does the matching and the relevance ranking. Authentication is **as the agent's own Atlassian user**, using the seat's Confluence credential from its `mcp_env` (the `atlassian` or `confluence` server entry, read by `atlassian.CredentialOf`, which accepts `CONFLUENCE_API_TOKEN`, `CONFLUENCE_PERSONAL_TOKEN`, `CONFLUENCE_TOKEN` or `ATLASSIAN_API_TOKEN`, and a `JIRA_API_TOKEN` on the shared `atlassian` entry). Confluence enforces its page permissions natively: a restricted page the agent's user cannot see simply doesn't come back, and there is no engine-side restricted-page handling. Seats without their own credential fall back to the **org token** (`integrations.confluence.token`); an agent on the org token sees whatever that account sees, subject to the empty-scope rule below. Each hit carries the ancestor-title chain the search's `ancestors` expand returned, so the auto-draft exclusion filters on any depth, by the same `knowledge.Excludes` the native backend uses. The client keeps whether the answer carried the `ancestors` key at all, because an empty list alone cannot say: `"ancestors": []` is a page at the top of its space and a known, empty chain, while an answer with no key, or a null one, is a chain that did not come back. So a draft a lead moved out of `Auto-Drafted Skills` — to the top of its space or under another page — is returned with its `[Auto-draft] ` prefix still on, and the prefix hides only a hit whose answer carried no chain.
...
knowledge.scope: []             # empty ⇒ the whole company natively; on
                                #   Confluence, unscoped and ACL-bound for a
                                #   self-authenticating agent
```

It is **role- and unit-independent** — every agent has the same read scope.

> **Read scope ≠ team identity.** A unit's own container — `space` (runtime `org.Unit.Space`) — is *integration identity*: it decides webhook routing (page activity → the unit lead) and is the team's write home (and where skill promotions are drafted, on either backend). It deliberately does **not** narrow reads. An Engineering agent isn't limited to the `ENG` container when searching: natively it searches the whole company, and on Confluence everything its own account can read. (See [Confluence § integration identity](../integrations/confluence.md).)

**The list is optional — and empty is the useful default.** When the scope list is empty, what a search covers depends on the backend. Natively it is the whole company: there is no per-seat account, so there is nothing to read through that a seat could not read itself. On Confluence it depends on how the search authenticates (per-agent token vs. engine/admin fallback):
...
So set `knowledge.scope` only to *narrow* reads to a curated floor (e.g. a company handbook); a fully per-agent-credentialled Confluence org leaves it unset and lets the backend's ACLs do the scoping, which remain the hard boundary there regardless.

**Two containers are reserved**, and each is named in config. `knowledge.skills_container` (`TS` by default) holds the [tool-skill](tool-skills.md) pages, and no search returns one: a tool skill is machinery the engine injects into a phase, and a seat told to read one as knowledge would follow it as an instruction. `knowledge.root_space` (`HOME` by default) holds the organisation's own pages, starting with the root `Onboarding` page, and is searched like any other container. Neither may be a unit's or a seat's own `space`. A seat's native `write_page` and `save_page` refuse both: a seat writing into the tool-skills container would be rewriting the guidance injected into its own phases with nobody reviewing it, and the root holds what the company publishes rather than what any one seat decides. An operator's own assistant over [`/operator/mcp`](../reference/api-endpoints.md#operatormcp--your-own-assistant) writes both — `write_page` is the only thing that creates a native page, so that surface is how a native company publishes its tool skills and its root `Onboarding` page.
...
Shared knowledge **is** the backend — the engine's own pages natively, the site on Confluence — and there is no second store filled beside it. The writers feeding the two reads:

| Writer | Read by | Reach |
|---|---|---|
| Native: seats via the page tools (`write_page` / `save_page` / `comment_on_page`), and an operator's own assistant through [`/operator/mcp`](../reference/api-endpoints.md#operatormcp--your-own-assistant) | `knowledge.Searcher` (this node's index) | Every seat |
| Confluence: humans + agents via the backend's MCP tools (`confluence_create_page` / `confluence_update_page` / `confluence_add_comment`) | `knowledge.Searcher` (live query) | Whoever the page's backend permissions allow |
| Confluence: `crewlet confluence import` ([below](#publishing-knowledge-docs)) | `knowledge.Searcher` (live query) | Same |
...
native page. So this is the path that publishes a native company's
[tool skills](tool-skills.md), whose pages live in the tool-skills container,
and the pages of its root container — `HOME/Onboarding.md` in the example below
included.

It serves what the company's **current** revision runs. A live apply that moves
the company to Confluence takes the page tools out of the catalogue your
assistant lists, and a call it sends from a list it read before the move is
refused naming `knowledge.backend`; the tracker's tools follow
`tracker.backend` the same way. `search_knowledge` stays wherever the company
runs a knowledge base, on either backend.
...
through the assistant path above, the `HOME` directory's page included. A company that moves to Confluence adds
...
- **The `knowledge.Searcher`** that answers is the one `knowledge.backend` names in the revision the node is running (see [the seam](#the-knowledgesearcher-seam)): the Confluence searcher, built for a `confluence` revision from the site connection and its org token, with nothing local; or the native one, built once when a node starts on a native revision, over this node's own store and its lexical index, which fuses the [semantic half](#semantic-search-two-stages-no-index-no-new-dependency) into every search while `knowledge.vectors` is on. Neither takes an LLM — writing the query text is the [prefetch's](#relevant-knowledge-prefetch) job, on the seat's auxiliary model, and `search_knowledge` has the executor's own words to search with. With `backend: none`, a `confluence` company whose org token did not resolve, or a native company on a node that booted on another backend, nothing answers and the `## Relevant knowledge` block is left out.
...
Beyond agents searching the knowledge base themselves, the executor's prompt carries a `## Relevant knowledge` block that pre-runs a knowledge-base search for the seat. Once per turn, the auxiliary LLM generates a short plain-text search query from the trigger context, the searcher runs it — over this node's own index natively, live against the site on Confluence — scoped to the org-wide [`knowledge.scope`](#accessible-containers), and the executor sees title + snippet bullets without having to think to call a tool. When the block is thin — a pointer trigger gates the search off — the executor searches the same seam itself with the `search_knowledge` builtin, once it knows what the task actually needs. The `CanSearch` pre-gate skips the aux-LLM call entirely when a search could not return anything, and so does a node whose index is still on its first build, which renders [the building sentence](#native-backend--the-engines-own-pages) instead.

What a seat can see is decided in two places. Who may read a page is the backend's: on Confluence the search runs as the seat's own Atlassian user (or the org token), so a restricted page that account cannot see never comes back and the engine keeps no restriction list of its own, while natively every seat reads every page. What a search leaves out is the engine's, on both backends: the tool-skills container, and whatever the query excludes by ancestor — the unreviewed drafts under `Auto-Drafted Skills` by default (see [`Query.ExcludeAncestors`](#the-knowledgesearcher-seam)). The ancestor exclusion is applied after the backend has answered, because it is judged on the parent chain that comes back with each hit, so each backend asks for three times the hits it will return. On Confluence the tool-skills space is excluded in the CQL itself, so the site never ranks a skill page into that headroom; natively the container is dropped from the ranked hits. An answer the drops still leave short of its limit is short rather than padded. On Confluence, one short of its limit while the site's answer links a next page — it ranks matches past the rows it returned — is marked truncated for every reader to say so ([`Answer.Truncated`](#the-knowledgesearcher-seam)) and logged as `confluence_search_short`; one short while the site links no next page holds every match the site ranked, and is whole.

A hit is a pointer. To read a page in full a seat opens it with its knowledge base's own page-read tool — `get_page` natively, by id or by `CONTAINER/Title`; the Confluence MCP server's `confluence_get_page` on [Confluence](../integrations/confluence.md). A further search is `search_knowledge` on either backend, or `confluence_search` on Confluence for a query the seat writes in CQL itself.
...
- **`integrations.confluence`** — required by `backend: confluence`, and refused beside `backend: native` because pages would then live in two places with nothing keeping them in step. The query-time search authenticates with each role's per-agent token (`mcp_env.atlassian`), falling back to the org-level token (`confluence.token`); a `confluence` company missing it has no searcher at all, so the `## Relevant knowledge` block is left out and only the agent's diary contributes. The native backend needs none of it — it searches as the engine, over this node's own applied rows.
- **`providers.embeddings`** — required for the diary's vector candidate path (the vector half of the `## Personal memory` prefetch's hybrid selection, plus the diary write-side embedding step), for `episodes` vector recall in the learning subsystem (`query_episodes` and the `## Similar prior work` prefetch), **and** for the [semantic half](#semantic-search-two-stages-no-index-no-new-dependency) of the native knowledge search. `knowledge.vectors` is the switch that fuses that half into the search: on, the embedding duty embeds the pages and work items with this provider and each search embeds its query with it; off, nothing is embedded for search at all. It **derives** from whether this block is configured — so a company already paying for embeddings for its diary gets the better search — and an explicit `vectors: true` with no provider is refused at validation rather than degrading quietly. Without an embeddings provider the native search is lexical only (BM25 over this node's own index), the diary degrades to its recency-only path (still functional, just without semantic candidate matching), and similarity recall of episodes is off. On `backend: confluence` the question does not arise: that search is a live CQL query against the site, which embeds nothing either way.
