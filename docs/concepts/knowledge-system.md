...
- **Best-effort**: `Search` never reports an error; every failure path returns no hits, which the turn-start block renders as the sentence that nothing surfaced.
