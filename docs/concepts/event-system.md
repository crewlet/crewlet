...
|---|---|
| **In this process** — `stream.type: embedded`, the default | A NATS JetStream server started in the engine's own process. No listener, no port, no service to operate: in the solo case it binds no socket at all, so the broker cannot be reached from outside the process. `stream.store_dir` makes its streams file-backed and restart-surviving, and it is **required**: left empty they live in memory, and the engine refuses to boot rather than lose them at the first restart. That holds for every company and on every node whatever its `node.roles`, because every node runs the engine and every company keeps its **org chart** on a state log whose records live on that stream — the native tracker and knowledge base put their items and pages there too (see [Configuration](../getting-started/configuration.md#stream)). |
