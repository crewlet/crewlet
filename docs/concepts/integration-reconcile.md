...
**A teardown is carried out for whoever asked for it.** The disconnect route records who asked on the fleet row beside the intent — the author, their kind and the credential they asked through — and every write the teardown makes is recorded under them: the revision that drops the block, each Slack seat's cleared credentials, the sealed values it deletes. The loop only carries it out, minutes later and on whichever node holds its duty, and those writes used to be recorded as its own (`reconcile loop`, of kind `system`) — while a *forced* disconnect, which drops the block inside the request, recorded the person. One gesture had two authors depending on a checkbox. A row a build that did not record the asker wrote is still finished as the loop's.
...
A seat is addressed through the **org chart**, one record per seat, because a merge patch over the settings replaces a list wholesale — and a seat is not in the settings at all: it lives on the chart's own log, where the handle IS the subject a change is arbitrated on, so two passes writing two seats never contend. What such a write carries is the seat's **runtime document** (`slack`, `github`), not the authored shape (`integrations.slack`), and it reports a log **position** rather than a revision and an epoch, because it makes neither. Left *unticked*, the apps go on working and every credential stays: the disconnect drops the company block, the transport retires, and nothing is deleted behind an app the operator kept.
...
**A pass also re-resolves this node's own seat identities.** A tracker or code-host webhook names people by *account*, and nothing in the org model says which account a seat holds — so the engine asks, with that seat's own credential, and registers whatever answers. That lookup runs whenever the node publishes a company — a config activation, a write to the org chart's own log, or boot — as the `seat_identities` step of [the convergence](configuration.md#what-follows-a-published-company); the lookups are keyed on the credential and cached, so a company whose credentials did not move spends no requests. A seat whose lookup FAILED is the case a publish cannot help with: nothing schedules one, so a failure that cleared itself a second later persisted until somebody edited something unrelated. Measured: a seat's token was created and sealed, the tracker checked it one second afterwards, the account was not grantable yet and answered `403` — and the surface held zero seat identities with every card reading Connected, still not having asked again two minutes later.
