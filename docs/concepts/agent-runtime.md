...
| `post_message` | the company runs [native chat](chat.md) | Say something new in a room, as a top-level message. `@handle` reaches one colleague; writing `@channel` in the text is what addresses the whole room, and the receipt says so back |
| `reply_in_thread` | the company runs native chat | Answer a message under its own thread, beside the question. Threads are one level deep — a reply to a reply carries the same root |
| `send_dm` | the company runs native chat | Say something privately to one colleague or a few, opening the conversation if it is not open. Never the place for an answer that was asked for in the open |
| `read_channel` | the company runs native chat | Read a room, newest first, or one thread in it, oldest first. The message a seat was woken about arrives in full; the conversation around it does not |
| `list_channels` | the company runs native chat | The rooms this seat is in, most recently active first — and where a room's id comes from when the seat only knows its name |
| `search_messages` | the node holds the chat index | Find a message by what it SAYS, ranked across every room this seat may read. `read_channel` is the other half: one room, in order, from a point you name |
| `react_to_message` | the company runs native chat | Put one emoji on a message, or take yours back. An acknowledgement and **not** an answer: it wakes nobody, and it does not discharge a reply somebody is waiting for |
| `join_channel` / `leave_channel` | the company runs native chat | Move this seat's own membership, and nobody else's. A private room cannot be joined this way, and a unit's own room can be neither joined nor left — its membership is the org chart's, and either gesture would be undone by the next apply |
...
Colleague outreach happens through whichever surface the company actually runs. Where that surface is the engine's own, the tools are first-party and registered exactly like the ones above: native chat's nine are in this table, and the [native tracker](../guides/work-tracker.md#what-a-seat-can-do)'s and the [knowledge base](knowledge-system.md)'s are documented with those subsystems. Where it is a vendor's, outreach goes through the upstream MCP tools directly (a chat server's post-message tool, the tracker's comment and update tools, the wiki's comment tool, the code host's review tools; these are examples, not engine-known names). `a2a_ask` is neither: it is the one colleague tool that reaches an agent rather than a surface, and it is narrowly scoped to tight-loop, mechanical sync between agents. Use whichever chat, issue-tracker, wiki or code-host tools your MCP servers expose for any collaboration a human teammate would reasonably want to see. The engine prompts name none of these: they describe the *capability* and the LLM picks the tool from its catalogue (see [Tool Capabilities](tool-capabilities.md)). See [Turn Engine: Colleague-surface tools](turn-engine.md#colleague-surface-tools) for when to use each.
