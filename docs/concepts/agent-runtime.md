...
| `spawn_subagent` | Run ephemeral bespoke sub-agents with a parent-chosen tool allowlist. **Execute only**, and built per turn rather than registered once: it carries that turn's grant — the parent's own live tool set, minus the control tools and anything that writes to a shared surface — so it cannot be a shared registry entry. Absent when the seat's remaining token allowance cannot be read, because a spawn with no readable ceiling is a spawn with no ceiling. See [Turn Engine](turn-engine.md) |
