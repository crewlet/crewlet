...
| `join_channel` / `leave_channel` | the company runs native chat | Move this seat's own membership, and nobody else's. A private room cannot be joined this way, and a unit's own room cannot be left — its membership is the org chart's |
