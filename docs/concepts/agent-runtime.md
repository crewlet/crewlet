...
| `recall_iteration` | always | Re-read one EARLIER round of this same turn in full — every tool call with the arguments exactly as they were sent, and everything the round produced. It is the floor under the [prior-work ledger's budgets](turn-engine.md#what-the-ledger-trims-and-why): that block shortens long arguments, long output and long read lists so it stays skimmable when re-sent every round, and this is where the whole value lives. Registered unconditionally because its corpus is the turn's own closed rounds, and because a tool that appeared only from round two would move the system+tools prefix mid-turn and cost a prompt-cache miss on every turn that iterates |
...
The reviewer's surface deliberately carries its submission and **nothing else** — not even `recall_iteration`. That is what makes forcing a tool call there the same instruction as "submit the review", and the duplicate-delivery question the reviewer reads the earlier-rounds block for is decided by the engine's own record of what ran rather than by the reviewer's reading of it.
