...
- **Trigger width** decides *which calls wait*. Prefer **exact tool names** — the tools the practices actually apply to. The bundled `skill:platform_mentions` triggers on the write tools only (posting broken mention markup is visible to humans; Plane/Slack *reads* never wait on it) — its Plane leaves name the write tools of the version-pinned `plane-mcp-server@0.2.10` release (`create_work_item_comment`, `create_page`, …), which is exactly why the example org pins the server version: an unpinned upgrade could rename a tool out from under the gate. `tool:refine_skill` triggers on the single `refine_skill` builtin (no other tool waits on skill-refinement conventions).
