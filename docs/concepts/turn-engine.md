...
| The resumed turn came back, and the run's record could not be READ to find out which box to settle | Indicator cleared. The store is asked to end the run while it is still the claim the tail took, so a job the resumed turn relaunched is left alone — and either way nothing is coming back for this turn. Staying quiet would leave the row in a claim no poll, redelivery, answer or reaper picks up, with the indicator over it |
...
| A resume that never reached its turn — a reply this build cannot read, a runner it could not build — or one whose turn broke without writing outside the engine | **Held where a box's completion drove it, cleared where a person's answer did.** The claim reverts to exactly where it was taken from, and the two routes came from opposite facts: a live run, so the retry finds a box still working and nothing could put a cleared indicator back; or a question still open, so the run is awaiting that same person again — and nothing is left claiming otherwise, because the offer that failed reports the failure rather than the delivery as handled, so the message that carried the answer falls through and runs as the ordinary chat message it looks like, raising a fresh indicator off its own trigger (or is requeued, and so redelivered, where a second run holds the seat). What it does *not* do is reach the coding run: that run waits for the conversation's next message. Where the revert itself cannot be written there is no retry on either route — the row is stuck in the claim — so the run is settled and the indicator cleared |
