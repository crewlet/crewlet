...
turn's shared budget explicitly — and the budget is read for room before
it is asked, as it is before every round: on a budget already at its
cap the judge is not called, and the phase is not extended, which is
the same outcome it would have paid a verdict to reach. A refusal there does not fail the
...
4. **Per-turn budget cascade.** Agent budget → phase budgets → the delegation slice (default 20% of the parent's remaining). Exhaustion publishes `budget_exhausted` and marks the turn failed. A round is charged once its model call has answered — its size is only known then — so the round a budget refuses has been billed all the same: the meter counts it, and the refusal stops the round before its tools run, which is why a phase's record and the budget's counter agree on what a refused round cost. Before each round's model call the budget is READ, counting nothing: a scope already at or past its cap stops the round before it is sent, so a spent budget refuses the next round rather than paying for it first. ONE slice covers a whole `delegate` call whatever its task count, so a fan-out cannot multiply the operator's fraction by N; the wrapper counts each round under a lock and decides its answer from that count before charging the parent, so concurrent workers can't both be admitted past the cap, and a round it refuses is on the slice and on the parent's counter like any other. A worker's round is read for room the same way, the parent's counter first and then the slice, so a refusal names the company or the seat whenever either is out. A call whose slice divided by its task count falls below `min_tokens_per_task` is refused UP FRONT — N workers that each die mid-round have spent the whole slice and produced nothing.
...
9. **A busy seat queues, it never drops.** A seat's inbox is one pull consumer that fetches again only after its handler returns, so a trigger that arrives during a minutes-long turn waits on the broker rather than erroring. Erroring instead would NAK it into bounded redelivery (25 deliveries, then the dead-letter topic) and spend that budget on events whose only problem is when they arrived. A seat parked on a detached sandbox job (potentially hours, `sandbox.Coordinator.AwaitingSandbox`) is handled differently: the dispatcher **parks** those deliveries, requeuing them onto the inbox and then acking (`inbox.ActionPark`), so nothing is held against a broker ack window. A run parked on a person's clarification question is the exception to both: it frees its seat, because the answer arrives on that seat's own inbox. So the dispatcher offers a delivery to a run waiting on a question once the completion ledger has been read — before the delivery can become a turn, or, on a seat still busy with another job, before it is parked — and a delivery that is the reply resumes the run instead. An answer handed over is recorded in the completion ledger, so a copy redelivered after a lost ack is never offered again, where it would be taken as the reply to the run's next question, and is dropped once the seat is free. So is an answer the seat's budget has no room for: the coordinator holds it on the run's record, and the turn resumes with it once the budget has room. One that could not be handed over or held — or a delivery for which the runs parked on its conversation could not be read — is redelivered, never run as a turn.
...
| `prompt.size` | The final size of one phase's prompt — `system_chars` and `user_chars`, plus a ~4-bytes-per-token approximation — so prompt growth is measurable across builds without reading every phase payload back. The figures are **bytes** whatever the keys say: that is what the engine can measure without a tokenizer and what a vendor bounds a request body in, and the two agree on ASCII while diverging on a prompt carrying non-Latin names, emoji or CJK. The keys are frozen by [ADR-0006](https://github.com/crewlet/crewlet/blob/main/adr/0006-event-evolution-is-additive-only.md) — renaming them would read back as a rendered `0` on every row already stored |
...
| `budget_exhausted` | The token budget stopped a seat. Two producers: the engine, beside `agent_turn_completed`, when a turn's phase error carries a `toolloop.BudgetError` — the budget refused a round's charge, or had no room for a round before its model call was sent; and the sandbox coordinator, once per answer it holds for a suspended turn it will not resume while the budget has no room — a finished coding job's result, or a person's reply to the run's question — naming the scope at its cap and the run's turn (see [Code Sandbox: Budgets](code-sandbox.md#budgets)). The completion poll that later signals room produces none |
