...
| `budget_exhausted` | The token budget ended the turn: it refused a round's charge, or had no room for a round before its model call was sent. Published by the engine beside `agent_turn_completed` when the phase's error carries a `toolloop.BudgetError` |
