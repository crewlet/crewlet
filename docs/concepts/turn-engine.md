...
`failed` is not only a reviewer's decision. The engine also ends a turn as `failed` when one of its runtime guards fires (stall, max-iter exhaustion, depth cap, a scheduled turn's wall-clock cap, an unhandled exception) and when a phase breaks outright (every provider in the chain unavailable, a refused budget, any other error a phase returns). A guard publishes `turn.guard_breach` with the specific `kind`; an exhausted chain publishes `llm_unavailable` and a refused charge `budget_exhausted`. Every such turn also closes with `AgentTurnCompleted` marked `failed`, carrying the one-line reason as `error` / `error_kind`.

**The phase that died publishes too.** A phase that returns an error still publishes its `AgentPhaseCompleted`, with `failed` set, the classified `error_kind`, and whatever the loop managed before it broke: the conversation, the tool calls that ran, the tokens already billed, the round it was on. Without it a failed phase left nothing behind but the `AgentPhaseStarted` that opened it, and the dashboard showed an in-flight model call whose response never arrived.

**A panic is an unhandled exception, not a crash and not a retry.** A panic in a phase (a provider SDK, an MCP client, a tool handler) is recovered at the turn loop, logged as `turn_phase_panicked` with its stack, and ends the turn as `failed` with `turn.guard_breach(kind="unhandled_exception")`; the turn's `AgentTurnCompleted` names that kind as its `error_kind`. The published detail carries the panic's value and never its stack, because the event store is readable by anyone the dashboard serves. A panic outside a phase, in the dispatcher's own stages or in a turn's set-up and tear-down, is recovered one frame further out (`dispatch_panicked`, or `sandbox_resume_panicked` on the resume path) and publishes the same breach. Either way the trigger is recorded and acknowledged rather than redelivered: a redelivery runs the same defect on the same input. Only what the delivery still holds is recorded: when a partition that would not merge has already requeued its tail, a panic in the head's turn records the head, and the tail's copies still run. See [A turn that broke halfway](seat-ownership.md#a-turn-that-broke-halfway).
...
The dashboard renders a failed phase record as a failed invocation, error first and partial work beneath it, and keeps it on screen. AFK is sticky until the agent does real work again: the failure event and the turn's own completion land microseconds apart, so a projection that took the newest event at face value showed a healthy idle seat immediately after the failure that stopped it.
...
`a2a_ask` is the only colleague call the engine itself mediates, so it is the only one that also records the *edge*: opening the channel publishes `a2a_channel_opened` (`requester`, `target`, `participants`) and wakes the target with an `a2a_request` carrying `delegation_depth + 1` and the requester appended to the chain; the answer publishes `a2a_message_sent` and wakes the requester with an `a2a_message`, and `a2a_channel_closed` ends it.
...
9. **A busy agent queues; it never drops.** A seat's attachment dispatches one partition at a time, and its pull loop fetches the next only after the running handler returns, so a seat's turns are strictly serialized without anything erroring. Erroring instead would NAK the triggering event into bounded redelivery (25 deliveries, then the dead-letter topic) and spend that budget on events whose only problem is that they arrived during a minutes-long turn. A seat parked on a detached sandbox job (potentially hours) is handled differently: the dispatcher **requeues and acks** those deliveries, so nothing is held against a broker ack window.

> **Known gap.** Nothing takes a pause hold on the seat's inbox while it is parked. The requeued copies therefore land back on a topic the seat is still consuming and are re-parked immediately, so a seat parked on a long run spins on republish-and-ack for the length of the run. The work is not lost (the same-id dedupe and the completion ledger hold), but the loop is real. Fixing it means a pause taken at the park AND released when the run settles; a pause without the release is strictly worse, because a seat that never resumes is deaf until the process restarts. `ResumeTopic` has no caller today, so both halves land together or neither does.
...
11. **A detached run holds its seat busy.** The coordinator marks the seat busy when it handles the run's `SandboxRunStarted` on the seat's control topic, and it takes the count from the fleet's run record (a run in `launching`, `running` or `resumed` holds the seat), so a redelivered start, a restart and a seat handoff all converge on the same answer. That handling is asynchronous, and nothing orders it before the suspended turn returns, so a delivery the inbox hands out in that window can start a turn beside the run. After an engine restart the coordinator re-derives the busy state as a step inside the seat acquire. On completion the agent stays busy through result collection and is freed immediately before the resume dispatch, whose failure un-claims the run row so a redelivery can retry (the suspended Execute loop is never lost), unless the resumed turn had already written outside the engine, or panicked, in which case the claim is kept so the completion is not redelivered into a conversation whose writes already landed, or into the same defect, and the run is settled like one that finished, so the seat is free for its next turn.
