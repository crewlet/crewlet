...
| `prompt.size` | The final size of one phase's prompt — `system_bytes` and `user_bytes`, plus a ~4-bytes-per-token approximation — so prompt growth is measurable across builds without reading every phase payload back. Bytes rather than characters because that is what the engine can measure without a tokenizer, and what a vendor bounds a request body in; the two agree on ASCII and diverge on a prompt carrying non-Latin names, emoji or CJK |
