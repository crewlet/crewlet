...
A prior round's **produced text** is kept whole in the record and elided at `RenderedArtifactLimit` (4000 runes) when *rendered* into the next round's block — from the TAIL, because `Execution.Text` is that round's whole tool loop concatenated (thinking included) and its deliverable is at the end. The block accumulates one of those per `self_iterate` and is re-sent on every round of all three phases that follow, so the product is what the bound answers.
...
5. **Sub-agent timeout.** A per-call deadline from `turn_engine.subagent_timeout_seconds` (default 120 s). A batched call additionally has an aggregate `subagent_batch_timeout_seconds` cap (default 600 s) and a `subagent_max_parallel` concurrency limit. The batch cap is the **parent** of every child's, so it must be at least `subagent_timeout_seconds` — validation refuses a smaller one, because with the two equal any batch wider than one wave has its tail time out before it calls a model. Hitting the aggregate cap does *not* discard the children that already finished — they come back with their real answers, and only the ones still running are reported as timed out. Their tokens were spent either way.
...
turn_engine:
  ...
  subagent_batch_timeout_seconds: 600    # aggregate cap for one batched call; must be >= subagent_timeout_seconds
