...
The team channel is configured on the OrgUnit, and it is integration-neutral: the same field serves whichever chat backend the company runs. A seat's transport identity is its own bot — a [Mattermost](../integrations/mattermost.md) bot token, or a [Slack](../integrations/slack.md) app:
