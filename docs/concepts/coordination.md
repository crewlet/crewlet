...
flowchart LR
    subgraph COORD["coordination store — shared by the fleet"]
        ...
        IA[("attempts<br/>failed authentications")]
...
| `attempts` | How many authentications a caller has failed recently, and the cap on how many of those one caller's record keeps. Company-wide because a guessing run reaches whichever ingress node the load balancer picks: a per-process count is one an attacker divides by the number of nodes without knowing it, and a successful login has to lift the lockout on every node rather than the one it landed on | [Deployment](../guides/deployment.md) |
...
**Every adopted bucket is read back, and a difference is named.** A retention that is not the one this node asked for is invisible in every other way — nothing about the write, the read or the caller says which horizon is actually in force — so the boot reads each bucket's live age back as it opens it and logs `bucket_retention_mismatch` naming both numbers and the remedy. That line is the only moment the two are ever in one place. `leases` reports its own instead, as `coord_kv_lease_ttl_differs`, because there the age is also the arithmetic every claim and heartbeat on the node is held to.

**There is exactly one bucket this engine still writes to after it exists, and it is `duties`.** Its age is a *ceiling* on the TTLs it can honour rather than a preference — a bucket made by a build with a shorter one reaps a live duty lease early, and the fleet then runs that duty on two nodes at once — so a booting node raises it to the longest duty's TTL and never lowers it. That is the whole exception: every other bucket, every other field, and the replica count of the duty bucket itself are adopted as they are found. An age *above* the ceiling is the healthy adopted state (a peer on a build with a longer one made it), so that bucket reports no mismatch.
...
| `setup states` | 15 minutes | The span of the task the state covers — a browser round trip to the vendor and two clicks — and under the one hour a GitHub manifest's one-time code lives, so a link that has expired says so rather than failing later on a dead code. It is a separate bucket from `claims` for the reason the whole table exists: a record cannot outlive its bucket, so a fifteen-minute claim written to the five-minute one lapsed after five and left the state replayable for the other ten |
| `attempts` | 15 minutes, **plus a cap of 16 records per caller** | The window a login throttle reasons over: long enough that a guessing run cannot wait it out between attempts and still make progress, short enough that an operator who fat-fingered a token is not locked out of the surface an incident is fixed from for the rest of the hour. The cap is the one explicit message limit in this estate, and its direction is the point — it discards the OLDEST attempt rather than refusing the newest. The other way round (which is what a KV bucket does by default) the record freezes at attempts that then age out one by one, and the caller stops being throttled in the middle of the flood that filled it |
