...
| `pages` | The knowledge base's containers, pages and revisions. **The one slot read by the thousand rather than by key** — so each node keeps a rebuildable [projection](architecture.md#5-where-state-lives) of it in its own database, and a page list is a local query rather than O(keys) message deliveries. Coordination holds it as opaque bytes and a version: every rule about what a revision means belongs to the package that owns the fields. *The tracker used to be here beside it and no longer is — it moved to an ordered log with a SQL copy on every node, which is [The Tracker](task-engine.md)* | [Architecture § Where state lives](architecture.md#5-where-state-lives) |
...
| `pages` · `kb vectors` | none | A page is a fact for the life of the deployment. A bucket age here would reap a person's work while they were still reading it — so removal is a **decision** under the maintenance duty (page revisions past 100, trashed records past 30) taken by a sweep that reads the record, never a horizon that reaps it |
