...
Coordination is never an estate of its own, and that is a construction rule rather than a convenience: **the coordination store rides the stream's own broker.** The engine starts or dials exactly one broker, and the leases and the shared records above are opened on it. Against an external cluster they are opened on the very connection the queue is already using, because a second dial would work and would be worse — two connections to one broker fail independently, so a node could go on renewing leases over the one that still works while the one carrying its inbox has dropped. Alive to its peers, deaf to its work, and holding every seat it owns while doing none of it. On the embedded broker the store takes a connection of its own to the server inside the same process: an in-memory pipe on a solo node, which listens on no socket, and the member's own client port on a clustered one.
