...
flowchart LR
    subgraph COORD["coordination store — shared by the fleet"]
        ...
        CR[("chat reads<br/>each person's cursors · mutes")]
...
| `chat reads` | Per person, per company: how far they have read in each room, which rooms they have muted, and whether they are in do-not-disturb. **Deliberately not on the chat log**, which is the one place in this engine where something durable and company-wide is kept OFF a domain's log on purpose. A cursor is a fact about one reader's attention: nobody replays it, no other row is derived from it, and the newest value is the only one that has ever been wanted — while a record would be applied by every node, kept for the log's whole replay window, and written far more often than messages are, because a person reading a busy room flushes a cursor per screen. The merge is a compare-and-set so two tabs cannot lose each other's progress, and a cursor never moves backwards | [Chat](chat.md#people) |
