...
| `work` · `pages` | The native tracker's items, comments and changes, and the knowledge base's containers, pages and revisions. **The only slots read by the thousand rather than by key** — so each node keeps a rebuildable [projection](architecture.md#5-where-state-lives) of them in its own database, and a board is a local query rather than O(keys) message deliveries. Coordination holds them as opaque bytes and a version, exactly as it holds a sandbox run: every rule about what a status transition means belongs to the package that owns the fields | [Architecture § Where state lives](architecture.md#5-where-state-lives) |
...
| `work` · `pages` · `kb vectors` | none | An item is a fact for the life of the deployment. A bucket age here would reap a person's work while they were still reading it — so removal is a **decision** under the maintenance duty (change keys past 365 days, page revisions past 100, trashed records past 30) taken by a sweep that reads the record, never a horizon that reaps it |
