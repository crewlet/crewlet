...
- **Clustered synthesis** (`scheduler_enabled`, `cluster_*`) — a periodic pass that greedy-clusters recent turns by tool-sequence Jaccard and drafts from clusters of size ≥`cluster_min_size` — is **not implemented in this build**. Its config fields validate and are read by nothing; `scheduler_enabled` defaults to off. Single-turn induction is what fills `synthesized_skills` today.
