...
- **`## The thread so far`** — the chat thread this turn was woken in, read at turn start on the seat's own chat credential and handed over rather than left for the agent to fetch. Rendered only for a chat *thread* reply; a top-level message has no earlier conversation. Bounded by whole messages (30 messages, 8000 characters, dropped oldest-first with the count reported; the root and the newest always survive), senders resolved through the party registry, and the seat's own replies marked. A thread that could not be read renders a different sentence from one that was read and was empty. It is **not** a learning prefetch and is **not** gated by the thin-trigger gate — it is the thing that makes a thin trigger thick. See [Agent Runtime](agent-runtime.md#the-thread-a-turn-was-woken-in).
