...
**A vector names the model that made it.** The note's vector is stored with the embedding model's id (`embedding_model`), and `learning.Diary.Recall` compares a query only with the notes its own model embedded: two models can share a width, and a cosine between one model's vector and another's is a number with no meaning. So after `providers.embeddings.model` moves to another model, the notes the previous one embedded drop out of the vector half — they are still this seat's notes, and the recency half still reads them — rather than being ranked on a comparison that means nothing. A row that names no model is an unknown model and is left out the same way: every row written before the column existed, and every row a peer on a build without it carried here through the memory changelog. Nothing re-embeds such a row; a learning vector is made once, by the writer.
...
- **Source:** the `episodes` table in the node's own store, replicated onto the memory changelog so it follows the seat across nodes — one row per completed turn (`agent_handle`, `task_summary`, `plan_summary`, `tool_sequence`, `skills_used`, `review_outcome`, `started_at`, `ended_at`, `duration_ms`, `embedding`, `embedding_windows`, `embedding_model`). Similarity recall compares a query only with the episodes its own embedding model embedded, and leaves out a row that names no model — see [the diary's rule](#2-diary-and-reflect_and_persist-in-flight-personal-memory).
...
- **Cross-agent promotion** is the third path, and its output is deliberately *not* a skill row. Every other skill here is agent-scope (one seat's row, in one seat's prompt) because a skill is a procedure a particular seat follows. A procedure four seats independently arrived at is something the **team** has, which makes it documentation. So when ≥`min_sibling_count` **distinct seats** in one unit converge on a similar tool run, a daily [singleton](seat-ownership.md#singleton-duties) pass distils the cluster into a **draft page in the team knowledge base** under the unit's `Auto-Drafted Skills` parent, for a lead to review.
  ...
  - **The draft is hidden until a person publishes it.** The `## Relevant knowledge` search excludes the auto-drafted subtree (and, as a fail-closed backstop where a backend has no parent chain, the `[Auto-draft] ` title prefix), so an unvetted draft is in no agent's knowledge search. A lead adopts one by moving it out of that parent — on the engine's own knowledge base, with `save_page` and a new `parent` (an empty one moves it to the top of its container); once published it is an ordinary knowledge-base page reachable through the query-time search. A rejection does not stick: the pass re-clusters the same persisted skills every day, so on Confluence a deleted draft is drafted again at the next run for as long as the seats' skills still converge. The engine's own knowledge base has no delete for a page; a draft left under the parent stays out of every search, and while it holds its title the pass does not draft it again.
  - **The knowledge base the search reads, and no other.** The pass writes through a small `learning.PromotionWriter` seam, and the engine hands it the writer that stands beside whichever searcher answers the company's knowledge search (`Engine.KnowledgeServed`, the one decision `knowledge.backend` is read through), so a draft always lands where the exclusion that hides it applies. It asks at every run rather than once, because the Confluence connection is built after the pass is armed. On the engine's own knowledge base — the default — the draft is a markdown page under the unit container's `Auto-Drafted Skills` page; that parent is created quietly if the container has none, and the draft's own creation reaches the container's lead the way any new page in a team's container does. On Confluence, `confluence.PromotionWriter` posts rendered storage-format XHTML under the unit's `space`, creating the parent if the space has none. Either **refuses the draft** rather than filing it at the top of the container if that parent cannot be created, because a page outside the subtree is one every agent's search can return. A node that serves no knowledge base drafts nothing, and logs `skill_promotion_idle` with the knowledge search's own reason — no backend configured, or one this node is not serving. The container is the unit's `space`, never its tracker project: a unit carries both identities, and filing a draft under the tracker's key would create a page in whatever container happened to share the name, or fail against nothing at all.
  - **Cross-tick dedup is the writer's job**, because the pass re-clusters the same persisted rows every tick and would otherwise yield one draft a day forever. Both writers key on the title, which is unique within a Confluence space and within a native container — natively the create itself is arbitrated on the title at the broker, so a write cannot make a second page at that address. The title is `[Auto-draft] ` plus the name the model gives the procedure, so a tick that drafts the same name finds the existing page and stays quiet rather than re-announcing the promotion.
...
- **Never an error for the turn.** With no searcher wired, a gate that says no, or a query-generation call that fails, the block is left out. A search that fails — an unreachable backend, a site that refused the query — answers marked failed, and the block says the search failed and so says nothing about whether a page exists (`FailedKnowledgeHint`), rather than that nothing matched. `Search` is best-effort by the seam's contract: it never raises, and it never passes a failure off as an empty result.
...
- **The gate-path hint** rendered when the block would otherwise go silently empty — the thin-trigger gate skipped the search, the auxiliary model's query ran past the 400-byte limit and was refused rather than cut, or the search ran and returned nothing. Mirrors `personal_memory`'s hint; points the agent at `search_knowledge` as the mid-turn escape hatch.
