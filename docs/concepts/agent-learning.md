...
- **Source:** the `episodes` table in the node's own store, replicated onto the memory changelog — under the seat's **id**, so it follows the seat across nodes and across a rename — one row per completed turn (`agent_handle`, `task_summary`, `plan_summary`, `tool_sequence`, `skills_used`, `review_outcome`, `started_at`, `ended_at`, `duration_ms`, `embedding`).
...
- **Gates, in order.** Each is reported by name, per worker, per turn, because "this company never learns anything" needs an answer that says *which* gate closed:
  ...
  2. **No role** — the turn came from a seat this revision no longer has. Learning about a removed role writes memory under an identity nothing can read back. A renamed one is not in this class: its retired handle still resolves, so the lookup finds it.
