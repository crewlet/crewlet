...
- **A resume waits for room.** A suspended turn is not resumed while its seat or the company is at or past its cap, because its first round would be refused before it was sent. Before it raises a finished job's completion the completion poll reads the budget, counting nothing, and with no room it withholds the completion: nothing is collected, charged, resumed or failed, and the wait publishes one `budget_exhausted` naming the scope, its figures and the run's turn when it starts (logged as `sandbox_completion_waiting_for_budget`). The job's box is still polled, which keeps it running for the length of the wait, and the first poll that finds room — a cap raised in a new revision, or a counter reset — raises the completion and the turn resumes as any other. A budget that cannot be read withholds the completion too, and says so on every poll (`sandbox_completion_budget_unreadable`). The poll is a fleet duty, so a node that takes it over in the middle of a wait says the wait once more; the coordinator on the seat's node reads the same budget before it claims a completion, which covers one raised by a node that does not read it.
- **An answer waits for room too, within its delivery budget.** A person's answer to a parked run is not handed over while the budget has no room, and the coordinator says so once (`sandbox_resume_waiting_for_budget` and one `budget_exhausted`). On a seat busy with another job the answer is parked behind that job with the rest of its mail, and offered again each time it comes back. On a free seat the delivery is negatively acknowledged, so the broker redelivers it with its [usual backoff](event-system.md#delivery-semantics) and each redelivery reads the budget again — logged, like every negative acknowledgement, as the consumer's `batch_handler_failed`. An answer that still finds no room when that delivery budget is spent — 25 deliveries, about ten minutes at the defaults — is dead-lettered like any failing delivery, and the run stays parked on its question until it is answered again.
