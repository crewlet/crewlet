...
- **Call → suspend.** `run_sandbox` provisions the box, starts the coding agent as a background command, persists a durable run record in the [coordination store](coordination.md) (the brief, the task, the conversation key, the trace context, and whether the turn owes anybody an answer), and returns a `tools.Result` with `Suspend` set. The Execute loop leaves that call unanswered and the turn ends. The launch publishes `sandbox_run_started` to the seat's control topic, and the coordinator marks the seat busy from the pending store's own list of the seat's active runs when it handles that event. The handling is asynchronous and nothing orders it before the suspended turn returns, so a delivery the inbox hands out in that window can still start a turn beside the run (see invariant 11 under [Runtime invariants](turn-engine.md#runtime-invariants)). Nothing is parked in memory; everything the resume needs is in the row. `sandbox_run_started` names the run by the turn's task, whole — the text the record holds as `task_description` — so the dashboard, which shows the event's value until its poll returns the record, never relabels a run while somebody is watching it.

  **The record lands in two writes, and the state says which one it is at.** The serialized Execute conversation — the messages, including the dangling `run_sandbox` call — cannot be written by the tool: the runner holds it only until the turn's frame unwinds, which happens *after* the call returns. So the launch opens the row as `launching` and the unwinding turn writes the conversation, flipping it to `running` in the same compare-and-swap. In between, the job is already executing and the row has nothing to resume into — so **nothing polls or claims a `launching` run**. Claimed in that window, a completion would find an empty conversation, and the coordinator, with nothing to re-enter, would fail the run: a coding job that finished before the turn unwound would destroy the agent's whole turn, permanently and with no retry. The window is milliseconds on an idle host and hundreds of milliseconds on a loaded one, and a trivial run is exactly the one that fits inside it.

  **A conversation too large for the record keeps its whole in parts.** The record is one message on the transport, rewritten whole by every step of the run's lifecycle — the claim that resumes it, its park on a question — so the conversation goes on the record only while the record holding it stays within half the transport's ceiling, 4 MiB of the 8 MiB, and the other half is left for those writes. A longer conversation — a turn that read a great deal before it called `run_sandbox` — is split byte for byte into part records filed under the run's launch in the `crewlet_bridge_calls` bucket, *first*, and the record then holds a reference in its place (`execute_state` is `{"suspension_in_parts": {"bytes": …, "parts": …}}`), written in the same compare-and-swap that flips it to `running`, so no reader ever sees a record naming parts that are not there. The node logs `sandbox_suspension_kept_in_parts`.
  - **The resume reads it back whole**: every part present and in order, adding up to exactly the length the reference names. Parts that do not make that whole end the run as lost (`sandbox_run_failed` with reason `no_execute_state`, logged as `sandbox_resume_suspension_unreadable`) rather than re-entering a turn without the call it suspended on; a store that cannot be read hands the claim back for the completion's retry instead. The parts are read while the run still holds its seat, so either way the run gives up its own hold on the seat and no other: a seat still running another job stays busy.
  - **A NATS server set below the ceiling** refuses a record the ceiling admits — a node does not boot against one, but a reconnect can reach one — and that refusal sends the conversation to parts too (`sandbox_suspension_refused_within_ceiling`, naming `max_payload`), split to what that server takes by the rule a bridged call's parts follow: a refused part is retried at half its size, or at 64 KiB when half would be smaller, and a refused part of 64 KiB or less ends the split. A conversation that cannot be kept at all fails the run as `suspension_unrecorded` (see [Failure modes](#failure-modes)): the event's `detail` names `max_payload` and the value to raise it to on every server, and the refusal, with the limit that server announces, is in the `error` of the node's `sandbox_suspension_unwritable` line.
  - **The parts end with the run**: purged when it finishes and when a second launch replaces its launch, and by the maintenance sweep's `bridge_calls` job for a launch no run names.
  - **A node of an older build** that collects such a run reads the reference as a conversation in a format it does not know, and hands the completion back for a node that can resume it.
- **Busy while it runs.** While the seat is busy, every delivery to its inbox is **parked**: the dispatcher requeues it onto the inbox and acks the original, so nothing is held against a broker ack window during an hours-long job and no new turn starts. The one delivery it answers instead is a person's reply to a question one of its runs is parked on. The agent runs **one coding job at a time**: the first call that suspends ends the round. It is freed while a run is parked waiting on a human answer (see [Mid-run clarification](#mid-run-clarification-crewlet-ask)).
...
- **Completion → resume.** The completion event is claimed **at-most-once** (an atomic status flip on the row) and **only for the job it reports**: every launch gets a new id on the row and the completion carries it, so a duplicate that arrives after its job parked on a question, or after the resumed executor started the next job, claims nothing. The result is collected, tokens are post-accounted once per run (see [Budgets](#budgets)), and the suspended loop is rebuilt from the row: the saved messages, the activated tools and loaded skills replayed, and the coding agent's findings appended as the `run_sandbox` call's reply, framed as "the sandbox did the code work; don't redo it; report / act now". The same `turn_id` continues, so the whole sequence renders as one turn on the dashboard, and its rounds number on from the rounds the turn closed before it suspended: the round that re-enters is the round that suspended, since a suspend does not close its round, and every round the turn closes, before the suspend and after it, counts toward its iteration cap. The re-entered round runs even when the cap was lowered below it while the run was out, because the coding run's result would otherwise go unread. **The round remembers what it wrote before it suspended.** A native tracker write is named after the writes before it in its turn, so a redelivered turn names its writes as its first attempt did, and the calls the re-entered round made before the suspend, the coding run's launch among them, are the earlier calls of every tool it calls after the resume: "update ENG-1; `run_sandbox`; resume; update ENG-1" names the second update as the second write to ENG-1, where named as the first it would be answered as already applied and dropped. The resumed turn's writes are stamped with the instant its turn could first have minted one, which the launch writes onto the run's record (`triggered_at`), rather than with the instant of its first launch, because a write the turn named before that launch and names again after the resume was minted before it; a record an older build wrote carries no such instant, and the first launch stands in for it. A failed resume dispatch hands the claim back, to the status it was taken from, so the redelivered completion can retry, and the suspended loop is never lost. That includes a resume a drain interrupts: the hand-back does not inherit the drain’s cancellation, so the seat’s next owner resumes the turn rather than reaping it as abandoned. The one exception is a resumed turn whose own record proves it had written outside the engine before it broke (commented on an issue, asked a colleague): a retry would repeat those writes, so the run is settled instead, its box reclaimed and its record deleted, and the seat is freed. A resumed turn that breaks — in the round that re-entered, in that round's review or in a round after it — is judged on the calls made since it re-entered. The calls made before the suspend, the coding run's launch among them, are **carried**: the round's record still holds them as the whole phase's, because the reviewer and the delivery check read them, but they are answered in the conversation a retry re-enters, so none of them is made again and none of them makes the turn one that wrote outside the engine. So a re-entry that finished without writing anything and then lost its provider in review is handed back for a retry, and one that posted after re-entering and then broke is settled. Otherwise it hands back only the claim it took: when the resumed executor has already started another job in the turn, or the seat's next owner has reaped the run as abandoned, the row is left as it stands, because the relaunch replaced the conversation a retry would re-enter and the reap has already ended the run.
- **Reuse across calls.** The box is a per-Execute-phase resource. On collect it is **paused** (snapshotted), not destroyed; if the resumed executor calls `run_sandbox` again — a follow-up fix, a clarification answer to incorporate — the tool reattaches to the same paused box and checkout (clearing the previous run's result markers, keeping the disk state) and suspends again. When the resumed Execute finishes *without* another call, the box is torn down. The two are told apart by the launch id, not the row's status: a new job that finishes, or parks on a question of its own, before the resumed turn has returned keeps its box and its tail. **A turn starts at most `turn_engine.max_tool_rounds` coding runs.** Each resume re-enters the executor with a fresh tool loop, so a round that called `run_sandbox` again on every resume would be bounded by nothing the loop counts; the run's record counts its launches instead (`launches`, raised by every launch and reset by none), and the call past the cap is refused before any box is provisioned, as a failed `run_sandbox` call naming the setting, so the executor finishes with its own tools or reports what is left.
...
- **A record carries what a newer build wrote.** Every step of a run's lifecycle rewrites its whole record, and nodes of two builds share it through a rolling upgrade, so a member of the record a node's build does not know is carried through each write it makes rather than dropped: a charge record or a held answer a newer node wrote survives an older node's claim, park or relaunch. A fact about one job names that job's launch, so one carried through a relaunch by a node that does not know it is read as nobody's. A node of a build that does not carry the members it does not know erases them on its first write to the record, so while one shares the fleet a held answer or a charge record can be lost to it.
...
**A box is named for itself, and the name is not shortened.** Each box gets a fresh 32-character identifier — a whole UUID in hex with its dashes removed — and that one value is the `sandbox_id` on the run's record and in every log line, the directory `<state_dir>/boxes/<id>`, and in `container` mode the container `crewlet-sbx-<id>`. So `docker ps`, the state directory and the dashboard all line up without consulting anything. Nothing about it is abbreviated for readability: under `container` a collision would be refused by the runtime, but under `direct` the id is only a directory name and `mkdir -p` succeeds on one that exists — two runs would share one workspace, one seeded credential set and one job record, and the first teardown would delete the other's checkout, with nothing reporting any of it.
...
**A loud command keeps both of its ends.** Everything the backend runs on the engine host — a `docker` invocation, the mount proof, a setup command — is captured with a 256 KiB ceiling, because a runtime streaming a pull progress bar, or a provisioning step printing every file it unpacks, would otherwise be the engine's own memory. Past that ceiling the capture keeps the first 64 KiB and the **last** 192 KiB and drops the middle, marking the gap in the text itself (`… output truncated: 8412391 bytes dropped here (kept the first 65536 and the last 196608) …`). Both ends are kept because the message lives at either one: a container runtime refuses on its first line, while an install or a build prints megabytes of progress and *then* the error — and a failed step's stderr is the entire account of the failure that the operator and the coding agent get. What falls in the gap is not retained anywhere, which is why the mark counts it: "truncated" alone cannot tell a hundred dropped bytes from a hundred megabytes. Neither edge of the gap ever lands inside a character, so a command that printed accented text, box-drawing or a non-Latin path reads back as itself rather than as mojibake — and bytes a command printed broken on its own account are passed through untouched rather than tidied away, because the engine only undoes cuts it made. The coding agent's own output never passes through this at all — it is redirected to files inside the box and read back whole.
...
### Runs are uncapped; the findings are the result
...
Because the brief instructs the coding agent that it is *not* the last step, it writes a structured **findings report** to a known file before stopping; `collect` prefers that file for the result text (it survives an agent that finishes but never exits, and a tool-only run that streamed no final message). The resumed executor is handed the result text — the findings, when the agent wrote them — or the error when there is no text, with the delivered refs.

**When a run left nothing else, its own output is the account.** A run that wrote no findings and whose output parsed to no answer comes back as an error made of its exit status and its **stderr**, and a Claude Code run whose stdout does not parse at all has that stdout as its text. Nothing bounds either stream, so each is **tailed**: its last 64 KiB, after a `…[earlier output truncated]…` marker, starting on a whole character. The tail because a crash explains itself at the bottom; 64 KiB because the text becomes a tool answer a model reads, and that is the ceiling the engine sets on one builtin tool answer. Both are **secret-redacted** before they leave the runner. What the cut drops is kept only in the box — `.crewlet/err.log` and `.crewlet/result.json` under its home — and only until the resumed turn is over: the box is then reclaimed, and a run that turn launches into the same box truncates both files before its agent starts.
...
**Every call is written down where a resume can read it — all of them.** A native tool loop keeps its calls in memory and the turn writes them at the end; a bridged run's calls are made by a process outside the engine, minutes apart, possibly across a restart. So each one becomes **its own record** in the fleet's coordination store (the `crewlet_bridge_calls` bucket), filed under the run and the launch that made it and numbered in the order it arrived, and the resume reads every record back: its submission is replayed from them, the delivery check counts them, and the reviewer is shown them. Nothing drops a call from that log, however long the run — a delivery a run made in its middle is one the resume sees, rather than one it misses and sends the turn round to make again.

- **A call too large for one record keeps its whole in parts filed under that record.** A record is one message on the transport, so it holds at most the 8 MiB ceiling, less 64 KiB for the message's headers. A call past that has its whole — the record exactly as it would have been stored — split byte for byte into part records filed under the call's own record, and filed *first*, so a record that names parts is never visible without them. The record then holds the call **fitted** to the ceiling, beside a reference to the parts: `whole_bytes`, the length of the whole, and `whole_parts`, how many parts hold it. The fit decides the **arguments** first, whole or not at all, because a cut through JSON is text nothing can parse: they are kept unless the record would be past the ceiling even with an output of nothing but `…`, and only then replaced by a one-member JSON object, keyed `…`, saying how many bytes of arguments there were and that they were filed whole in parts under the record. The **output** is then fitted to what is left, from the whole of it: cut on a character boundary and ending in `…` when it does not fit, untouched when it does. Each mark is in the field that was cut, and only there. Once the record has landed the node logs `sandbox_bridge_call_fitted`, with the whole lengths as `output_bytes` and `args_bytes`.
- **The resume reads every call whole.** The read a resume rebuilds its phase from reassembles each such call from its parts, so the resumed phase's own record — which outlives the run — carries the whole output the coding agent was handed and the whole arguments the tool was called with. The parts are checked against the record's reference before they are believed: every part present and in order, adding up to exactly `whole_bytes`, and decoding to this very call. Parts that do not — one missing, one short, one holding another call — are never passed off as the call. The resume gets the record's cut form instead, with its `whole_bytes` and `whole_parts` cleared, arguments the record set aside replaced by the `…` object saying the parts they were filed in could not be read back, and a note at the end of the output saying why and how many bytes in how many parts it was; the node logs `sandbox_bridge_call_whole_unreadable`.
- **A whole that could not be kept says so where the call was cut.** When the parts cannot all be written — the store cannot be reached, or a server refuses a part too small to split again — the record is filed in its fitted form, or in its least form when that server refuses the fitted record too, with no reference, and says so in the field that was cut: arguments set aside become the `…` object saying they were not kept, and a cut output ends in a note after its `…`. The coding agent in the box was handed the whole output and the tool took the whole arguments, but no copy the engine can read back exists. Once that record has landed the node logs `sandbox_bridge_call_whole_not_kept` at `ERROR`, with how many parts were written before the failure; those parts are named by nothing, and go with the launch's purge.
- **A NATS server set below the ceiling** refuses a record that fits it — a node does not boot against one, but a reconnect can reach one. The node logs `sandbox_bridge_call_refused_within_ceiling`, naming `max_payload`, and the call's whole goes to parts split to what that server takes: a refused part is retried at half its size, or at 64 KiB when half would be smaller, and only a refused part of 64 KiB or less ends the split. The call's record is then its **least form** — its name and outcome, each text no longer than its mark kept as it was, and each longer one set aside: the arguments for a `…` object saying they were set aside because a server refused the call's record, the output for `…` — beside the reference, so the resume still reads every byte of the call; when even a 64 KiB part is refused, the least form is filed with no reference and says its whole was not kept, as above. That holds whichever record the server refused: the whole of a call within the ceiling, or the fitted record of one past it. The refusal is logged once for the call, whatever number its record lands at, and says only what the server refused and what to change; once the least form has landed the node logs `sandbox_bridge_call_least_form_filed`, naming the whole's length and its parts.
- **The run's row still carries a bounded list, for older builds.** A rolling upgrade puts builds that read a bridged run's calls from its row on the same store, so every call is also appended to that row's list — the first and last 100 kept and the middle counted in `bridge_calls_elided`. The list is also kept small enough that the row stays within half the transport's ceiling, dropping from the middle and counting the same way, because the row is what the run's own lifecycle writes — its claim, its park on a question — and a row the list had filled would refuse those for every build. It is written *after* the record, and a failure to write it loses the call from that view only: the append still succeeds, and the node logs `sandbox_bridge_row_view_append_failed`.
- **A run an older build recorded is read from its row, gap and all.** A launch with no records at all is one an older build recorded, and this build reads that launch from the row. That list is the one log that can be missing calls — past 200 the older build kept only the first and last 100 — and the calls it dropped are kept nowhere, so the resume is handed their count and their place rather than the survivors as though they were every call: the resumed phase's record says how many are missing and after which call, the reviewer's tool log says so where they fell, and the node logs `sandbox_bridge_calls_not_kept`. A delivery among them is one the delivery check cannot see.
- **A run's calls end with the run.** Its calls, and every part filed under them, are purged when the run finishes and when a second launch replaces the first. A run that finishes after a resume has already carried every call, whole, into the resumed phase's record; a run that ends with no resume collecting it — one that failed or was abandoned, a launch that could not start, a seat removed from the company — has its calls purged unread. The maintenance sweep's `bridge_calls` job purges the calls of any launch no run names: what a node that died between a run's end and its purge leaves behind, and what a late call files after one. The bucket has no age of its own: a run parked on a person's answer can wait days, and its calls are what its resume will be judged on.
- **`sandbox_runs` carries both ends of the log, and counts the middle where it falls.** Each run's `bridge_calls` holds the log's first calls and then its newest — how the run set about the work and how it ended, which for a bridged run is its submission — with `bridge_calls_total` saying how many the log holds, `bridge_calls_elided` how many the list leaves out, and `bridge_calls_elided_after` how many of its entries come before them. `bridge_calls_next` is the cursor to read those calls from: ask again with `turn_id` and `calls_after` set to it, and read on until you have the counted number; `calls_limit` sets the page size (default 100, at most 500), and the first answer carries up to that many of the first calls and as many of the newest. Every page of the records, and each end of the first, is also bounded by what its calls weigh — never more than the ceiling one record may reach, so none is heavier than the heaviest single call — and can carry fewer calls than asked for; the cursor, not the count, says whether there is more. A run an older build recorded is the one list its row holds, with no cursor: what that build dropped is counted and placed the same way, and no page reaches it. Each call is shown as its record holds it — a call too large for one record fitted, and one a server below the ceiling refused in its least form, each marked where it was cut and carrying `whole_bytes` and `whole_parts` when its whole is kept in parts under the record — and no answer carries a part or counts one.

Without that log, the reviewer of a resumed run judges a turn whose entire tool log is gone, which the delivery check reads as a turn that acted on nothing.
...
2. The run is parked (`awaiting_clarification`) with the conversation key; the suspended Execute state is preserved untouched; the box is left **paused** so the checkout survives. Unlike a running job, **the agent goes free**: a human reply can take days, so its inbox flows and it can do other work. The run is parked only once the question is out, and the agent freed only once it is parked, because a parked run is no longer the completion’s to retry: if either step fails, the completion is handed back and its retry asks again. The question is kept on the run's record, redacted as its announcement is, so it has to fit there: one longer than `sandbox.MaxQuestionBytes` (344 064 bytes, 336 KiB — a sixth of half the room the record keeps for its lifecycle's writes, since JSON can write a byte of text as six, and the other half is for the reply held beside it while the budget has no room) is never asked, and the run is ended as `question_unrecorded` instead (see [Failure modes](#failure-modes)).
3. The next inbound message on that conversation is treated as the answer. Because the agent is free, that message reaches its dispatcher like any other, which offers every delivery on a conversation to the coordinator before the delivery can become a turn — and, on a seat still busy with another job, before parking it. The coordinator finds the run through an index of runs parked on an answer, kept beside the run records and keyed by the seat and the conversation, so recognising an answer is one keyed read rather than a read of every run the fleet holds. An entry is filed before the run parks and is confirmed against the run's own record on every read, so one that outlived its run, or names a run already claimed by its answer, answers nothing. A run parked by a node of a build without the index has no entry until a node of this build lists it — on a tick of the completion poll while it holds that duty, or when it takes the seat, before the seat's mailbox opens — and an answer that arrives before then is handled as an ordinary message. The coordinator claims the parked run (at-most-once, and only while the row still holds the job that asked) and **resumes the same suspended Execute loop**, splicing in the question, the answer, and the WIP branch as the `run_sandbox` call's reply, with instructions to call `run_sandbox` again incorporating the answer, which reattaches to the paused box and checkout. No fresh turn and no re-investigation. An answer handed over is recorded as worked, so a copy redelivered after a lost acknowledgement is never offered again, where it would be taken as the reply to whatever the run asks next, and is dropped once the seat is free; an answer the coordinator could not hand over — its claim or its resume failed, or the runs parked on the conversation could not be read, so whether it answers one is unknown — is redelivered to be offered again, never run as a new turn. An answer the seat's budget has no room for is held on the run's record and handled (see [Budgets](#budgets)); from then the question has its answer, and a later message on the conversation is an ordinary one.

The durable state is always **git plus the persisted row** — the WIP branch is pushed *before* the agent asks, so the paused box is a fast-resume optimization, never the store. Clarification rounds chain within the same turn. A suspend does not close its round, so an answer followed by another `run_sandbox` call in the round that re-entered stays in that round: the turn's iteration cap bounds the rounds the turn closes, not the asks, answers and relaunches inside one of them. The relaunches are bounded by the turn's launch count (at most `turn_engine.max_tool_rounds` coding runs a turn, see [How a coding task runs](#how-a-coding-task-runs)), and by the token budget where one is set: the pre-flight floor below refuses a launch once the seat's remaining budget is under it.
...
- **Pre-flight floor.** A launch is refused when the agent's remaining token budget is below `turn_engine.sandbox_min_budget_tokens` (default 2000), because a coding run costs a box, a clone and a toolchain install before it produces a token; the `run_sandbox` call returns a normal failure the executor can act on, without suspending. A budget read that FAILED is refused too — launching on an unknown budget is how a company discovers its ceiling by spending past it.
- **Self-limits.** `role.sandbox.max_turns` becomes the coding agent's own cap — Claude Code's `--max-turns` — so the run self-limits.
- **Post-accounting.** On collect, the run's reported token usage is charged to the budget cascade, and the resumed phase's record carries what the run reported it cost (`cost_usd`), which the Tokens view prices alongside native phases. The run's tokens themselves reach the budget counter and no phase record, so the Tokens view's token figures leave them out. Mid-run enforcement is best-effort by design, the price of the capability; OpenCode's unreported usage stays at zero rather than being invented.
- **Charged once per run.** A run's tokens reach the seat's counter and the company's once, however many times its completion is delivered. A resume that fails (the seat moved between the publish and the receive, say) sends the completion back, and the retry collects the same finished job again; the run's coordination record says its spend is already counted, so that retry charges nothing, whether it lands on the same node, on the seat's next owner, or after a restart. The record is written by the same compare-and-swap that hands the completion back, so no retry can find the run reopened without it. A second `run_sandbox` call in the same turn is a second job and is charged on its own. The charge is never refused, because the tokens are already spent: they are added to both counters whatever the caps say, a run that takes a counter past its cap is logged as `sandbox_spend_over_budget`, and the turn continues — the next round the seat or the company asks for is then refused before it is sent. A charge the counter never answered (`sandbox_accounting_failed`) is offered again if the completion is retried; if it had in fact landed, the retry counts it twice, which trips a cap early rather than late.
- **The carried spend is counted once across a resume's attempts.** The resumed phase's record is the only record of the whole phase, so it carries the tokens the phase billed before it suspended and what the collected run reported it cost. A resume that fails and is retried publishes a record for each attempt, and only the first of them counts that carried spend, success or failure: the run's coordination record says so (`carried_counted`), written by the same compare-and-swap that hands the run back for the retry, and every later attempt counts only what it billed itself. A node of an older build knows nothing of that flag and drops it when it writes the record, so while one shares the fleet a retry can count the carried spend again.
- **A resume waits for room, with its answer held.** A suspended turn is not resumed while its seat or the company is at or past its cap, because its first round would be refused before it was sent. The answer its `run_sandbox` call would be resumed with is held on the run's record instead, and the run keeps the status it had: running, holding its seat, for a job's result; waiting on its question, with its seat free, for a person's reply. The completion poll reads the budget on every tick for each run holding an answer — counting nothing, and without polling the run's box — and the first tick that finds room, a cap raised in a new revision or a counter reset, signals the seat's node (`sandbox_answer_ready`, on the seat's control topic alone), which reads the budget again, claims the held answer at most once and resumes the turn with it. With the room gone by then, or a budget that cannot be read, the answer stays held and the next tick signals again. A resume of a held answer that fails hands its claim back with the answer still held, for the retry. The wait is said once, when the answer is held: one `budget_exhausted` naming the scope, its figures and the run's turn, beside `sandbox_resume_waiting_for_budget` in the log; a budget that could not be read is said as `sandbox_resume_budget_unreadable` instead, with no `budget_exhausted`, since no scope was read at its cap. That makes the coordinator holding the answer the one producer of the wait's `budget_exhausted`, once per held answer; the poll produces none.
  - **A finished job is collected whatever the budget.** Its completion is claimed and collected, which pauses its box, and charged; the budget is read after the charge, because the run's own spend can take a scope to its cap. With no room the result is held in the same write that hands the claim back with its charge record, so the job's box waits paused, not running and billed, and a duplicate of the completion claims nothing. A result whose record would be past `sandbox.MaxHeldAnswerBytes` (2 064 384 bytes — half of the record's lifecycle room, measured encoded) is held as a reference to the box instead (`sandbox_result_held_in_box`), where the collect left it, and the resume collects it again, charging nothing a second time. A job that stopped to ask a question is parked whatever the budget: a park resumes nothing, and its answer reads the budget when it arrives.
  - **A person's answer is held and acknowledged.** An answer to a parked run is held on its record, its delivery acknowledged and recorded as worked like one handed over, so it is neither redelivered nor dead-lettered while the budget stays spent. One answer per question: the first is held, and a later message on the conversation is an ordinary one. An answer that, held with the event that delivered it, would be past `sandbox.MaxHeldAnswerBytes` is refused naming that bound and left with its delivery, since it is kept nowhere else; its redelivery hands it over whole once the budget has room.
...
- **`… output truncated: N bytes dropped here …`** in a setup-step failure or a timed-out control command — the command printed more than the 256 KiB the backend captures, so what you are reading is its first 64 KiB and its **last** 192 KiB with N bytes missing between them. The line is the engine's, not the command's, and it is not a failure of its own: the error a command ends with is inside the part that was kept. Quiet the step down (`--silent`, `>/dev/null`) if what it prints in the middle is what you needed.
...
- **Collect failure**: the box was reachable but the result couldn't be read; the box is torn down, the run ended and announced (`collect_unreachable`), and the agent freed. The run's result is lost — whatever it pushed is on its branch — and the `sandbox_collect_failed` log line says why the read failed. A backend refusing a file it will not read back is one cause (E2B refuses one past 32 MiB); stderr is read only for a run that left nothing else, so a large one never costs a run its report.
- **`sandbox_suspension_*` errors** (`sandbox_run_failed` with reason `suspension_unrecorded`): the turn suspended but the conversation a resume re-enters never reached the run's record, because the runner recorded none, it would not serialize, the coordination store refused the write, or the run was no longer launching. The coding job was already executing, so a run still launching has its box reclaimed at once and is ended, rather than left to run to its TTL with nothing able to collect it. A run that is no longer launching is left alone and not announced: a write reported as refused can have landed, which makes it an ordinary suspended run the completion poll resumes, and a run already ended was announced by whoever ended it. A refusal **as too large** is a NATS server whose `max_payload` is below 8 MiB refusing the conversation's parts down to 64 KiB: the event's `detail` says so and names the value to raise `max_payload` to on every server, and the `error` of `sandbox_suspension_unwritable` names the limit that server announces. Any other store refusal points at the coordination store, whose reason is on that same line; the other three are a defect in the engine worth reporting.
- **`sandbox_run_failed` with reason `question_unrecorded`**: the coding agent stopped to ask a person a question the run's record could not hold, so the run could not wait for an answer. The refusal is permanent — the same question is refused on every attempt — so the run is ended, its box reclaimed, rather than handed back to ask again. The event's `detail` names the question's size and the limit it passed: `sandbox.MaxQuestionBytes` when the question alone was too long, which is refused before anybody is asked it, or the record's own ceiling when the store refused the question beside what the record already carried, by which point the question had been announced. The question itself is whole in the log of the node that collected the run (`sandbox_question_unrecorded`). Brief the agent to ask a shorter question — whatever it pushed is on its branch.
...
- **A run that "finishes" with no report** — the coding agent neither wrote its findings file nor streamed a final message. `collect` surfaces the exit code and the last 64 KiB of stderr as the error; the resumed executor sees a failed run rather than an empty success.
