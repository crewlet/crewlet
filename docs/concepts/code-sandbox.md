...
Runs headless — `claude -p "<brief>" --output-format json --permission-mode bypassPermissions` — and its JSON output maps directly onto the result (final text, session id, token usage, cost). The run's input is its **whole** prompt: the vendor's `input_tokens` counts only the uncached remainder, so the engine adds `cache_read_input_tokens` and `cache_creation_input_tokens` to it and carries both as the cache's share, the same way its own Anthropic provider does. Read alone, the uncached figure put a fraction of every coding run's prompt on the budget counter.
...
### Runs are uncapped; each run is published as a phase
...
Because the brief instructs the coding agent that it is *not* the last step, it writes a structured **findings report** to a known file before stopping; `collect` prefers that file for the result text (it survives an agent that finishes but never exits, and a tool-only run that streamed no final message). The runner also reconstructs an **activity transcript** — for OpenCode from its line-flushed `--format json` event stream (`[tool] bash: <command>`, errors flagged), for Claude Code from captured stderr.

**Each collected run is a phase of its turn.** When the completion is claimed and the result read back, the coordinator publishes one `agent_phase_completed` with `phase: sandbox` and `backend: sandbox` — the run's own record, beside the executor's:

| Field | What it holds |
|---|---|
| `turn_id`, `iteration`, `launch_id` | The identity. `iteration` is the executor iteration that launched the run, written onto the run's record with the suspended conversation; a turn can launch twice in one iteration, so `launch_id` tells the runs apart |
| `started_at`, `duration_ms` | From the launch, dated by the store when it named the job, to the collection on the node that read the result — so the completion poll's interval is inside it |
| `input_tokens`, `output_tokens`, `total_tokens`, `cache_read_tokens`, `cache_write_tokens` | What the run spent, as its CLI reported it (zero for OpenCode, see above) |
| `cost_usd` | What the run's CLI said it billed, where it says so. On this record only — it is the run's fact, and it used to ride the resumed executor's record, which a run that parked on a question never gets |
| `model`, `coding_agent`, `sandbox_id` | The model the launch pointed the agent at (empty when it named none), the CLI and the box |
| `response`, `delivered_refs`, `activity_transcript` | The findings report, what it delivered, and the transcript |
| `failed`, `error` | Set when the run did not succeed. A run that stopped to ask a question is **not** failed; its `notes` carry the question |
| `work_item`, `work_key`, `conversation_key` | As on every phase record |

It is published **once per run**, however often the tail is retried: a resume that fails hands the completion back and the retry collects the same job again, so the run's coordination record says the phase already went out, in the same write that hands the claim back — exactly as the [charge](#budgets) is recorded. A publish the broker refused is not recorded and is offered again by a retry, if one comes; telemetry never fails the resume. Because every spend reader folds these records — the spend rollup, each node's daily usage and the turn list — a coding run's tokens are counted where every other phase's are; before this record existed they reached the budget counter and nothing a person reads.

The report, the transcript and a crash's detail are each bounded at **256 KiB** (`MaxTranscriptBytes`), in bytes, because what they ride is an event and the broker refuses one over 8 MiB whole: 256 KiB is at most 1.5 MiB once JSON has escaped it, and three of them still fit. The transcript and the crash detail keep their **end** (the conclusion is what a reader wants; the clone and the install are the least interesting part to lose), the report keeps its **start** (it is read from the top), and every cut lands on a character boundary with `…` where it was made. Everything on the record is **secret-redacted** at the publish as well as at collection.

The dashboard renders the run as its own card after the executor that launched it: the report, and the transcript as a closed **Activity** fold. A turn with a coding run therefore reads as the executor (its reasoning, the launch and — once resumed — what it did with the result) followed by one card per run.
...
Once it is in the code, the coding agent may hit something only a person can answer — an ambiguous spec, a design decision above its remit. Headless coding agents can't pause to ask interactively, so every box gets a shim, **`crewlet-ask`** (`internal/sandbox/codingagent/ask.go`), and the brief instructs: *don't guess — commit and push your WIP branch, run `crewlet-ask "<question>" --to <audience>`, and stop.* The audience is `requester`, `team`, `manager`, or a named teammate — the brief carries the seat's unit roster and its manager, by handle, so the agent can name a real person and the park can resolve the name exactly. (That instruction had no caller until the answer-by-turn change: the shim was installed in every box and no brief mentioned it, so a run blocked on a decision had no way to park on a question at all.)
...
### Who a question is put to

The audience a coding agent names is its own words, so the park resolves it — once, against the org chart, in the same write as the question — into the seats the question waits on (`audience_handles` on the run's record and on [`GET /sandbox-runs`](../reference/api-endpoints.md#get-sandbox-runs)):

| Audience | Put to |
|---|---|
| `requester` | The seat whose message, notice or ask woke the turn that launched the run. The launch records it on the run (`requester`), because the park runs when the job finishes — days later, perhaps on another node — and sees no trigger. |
| `manager` | The seat's manager. |
| `team` | The lead of the seat's unit and the **people** in it; the unit's other agents are left out, since the seat could ask them itself. |
| a name | The one colleague it names exactly — a handle, an id on a chat surface, or a role name, case aside. A partial or approximate match is not a person: a guessed colleague is somebody else's question in their inbox. |

When the audience names nobody the chart has — no requester was recorded, the seat has no manager, the name matches nobody — the question is put to the seat's **lead chain** instead (its managers, nearest first, or, for a seat nobody manages, the leads of the units that hold it) and `audience_fallback` says so. A question is never parked waiting on nobody while somebody is answerable for the seat, and a person can tell a question put to them from one that reached them because nobody else could be named. `?audience=<handle>` on the board is "what is waiting on me".

### Answering a parked run

A reply on the question's conversation (step 3 above) is one way to answer. The other is to **name the run by its turn**, with the operator tool [`answer_run`](../reference/api-endpoints.md#answering-a-parked-coding-run) — from your own assistant over `/operator/mcp`, or from the dashboard. It is the only way for a run started by a **schedule, a task assignment or a colleague's ask**: those turns have no conversation, so the run's record names none and nothing a person says in chat can reach it. Until this existed such a run's question waited out its pause TTL however promptly somebody knew the answer.

`answer_run{turn_id, answer}` checks the run is waiting (`not_running` if it is not), checks that the node holding the seat runs a build that can route the answer (`peer_upgrading` if it cannot — see [what a node says it can do](coordination.md)), and puts a `sandbox_answer_given` event on **the seat's own inbox**, answering `pending`. The node holding the seat takes it off the inbox **before its inbox screening** — a seat busy with another coding job does not park it, because the run it answers holds nothing — and hands it to the coordinator, which claims the run exactly as a chat reply does and resumes the same suspended Execute loop, the answer attributed to the person who gave it. It is **never a turn**: whatever it finds, the delivery is spent there, and only an answer the run is still owed (the store could not be read, the resume failed and the claim went back) is handed back to the broker for its spaced retry. Because it travels on the inbox, an answer given while the seat is paused waits for the resume, and one given while the holder restarts is still there afterwards. Two answers to one question — a chat reply and an answer by turn, or two people — contend for the one claim, and the run is resumed once.

**Both routes leave a record.** `sandbox_run_answered` names the run, the route (`chat` or `operator`), who answered (the chat sender, or the operator credential and the person it is bound to) and what the answer became: `resumed`, `not_awaiting` (another answer had it first, or the run is running a job) or `gone` (the run had ended). The chat route used to leave nothing at all. A chat message that matched no parked run was never an answer and records nothing; an answer still owed a retry has not become anything yet and records nothing until it has.
