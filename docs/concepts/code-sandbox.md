...
**Every bridged call acts as the run's seat.** Because the token is the credential, the caller is the seat the run belongs to, and the bridge states that on every call — the same identity a native turn carries, with the same four grants — so the [authority table](identity-and-access.md#the-authority-table-one-function-decides) decides a bridged `list_work_items` exactly as it decides the native one. It is set by the bridge itself rather than read off the request, because `/mcp/` is exempt from the credential guard: the guard resolves a box that presents nothing as anonymous, and a development build started with `-dev-principal` resolves it as that principal. Neither may decide a seat's tool call, so the bridge replaces the request's answer with the seat's on this route and nowhere else. A session whose tool surface is bound to no seat is refused at `Open` rather than served.
...
The token is **signed and self-describing** rather than a key into a store, because minting and verifying can happen on different nodes: the node running the seat mints when it launches a run, and whichever node the box exports to verifies. Every node derives its signing keys from the Tier A keyring, so a fleet agrees without sharing state, and with no keyring configured the key is per-process, which works on one node and is warned about loudly on more. Expiry rides in the token, so nothing is reaped and a restart does not invalidate a live run's endpoint. The token also **names the key that signed it**, so a keyring rotation does not invalidate a run already in flight: the key stays on the ring until the operator drops it, and a token naming a key a node no longer holds is refused rather than retried against the active one. The runbook is in [Secret store](secret-store.md).
