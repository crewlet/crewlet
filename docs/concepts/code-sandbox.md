...
- **`the catalogue offers more than one place to do it`** — a sandbox-enabled seat (or an agent-mode entry) names no cell and the catalogue names no `default_run_in`. Name the cell where the message points, or name the default. A catalogue is never refused on its own for being ambiguous — only a seat or entry that would actually fall to the missing default is.
