...
- **A call too large for one record is cut, never dropped.** A record is one message on the transport, so a call whose output would take it past the 8 MiB ceiling (less 64 KiB for the message's headers) keeps the head of its output, ending in `…`, with `output_bytes` saying how long the whole was; its arguments are kept whole, or — only if the output alone could not make room — not at all, with `args_bytes` saying so. The coding agent in the box was handed the whole output; the record is the engine's only copy, so the cut is marked on it. The node logs `sandbox_bridge_call_fitted` when it happens.
- **The run's row still carries a bounded list, for older builds.** A rolling upgrade puts builds that read a bridged run's calls from its row on the same store, so every call is also appended to that row's list — the first and last 100 kept and the middle counted in `bridge_calls_elided`. The list is also kept small enough that the row stays within half the transport's ceiling, dropping from the middle and counting the same way, because the row is what the run's own lifecycle writes — its claim, its park on a question — and a row the list had filled would refuse those for every build. It is written *after* the record, and a failure to write it loses the call from that view only: the append still succeeds, and the node logs `sandbox_bridge_row_view_append_failed`. A launch with no records at all is one an older build recorded, and this build reads that launch from the row.
...
- **The run board pages them.** `sandbox_runs` carries each run's first page of calls with `bridge_calls_total` and, when the log goes on, a `bridge_calls_next` cursor; ask again with `turn_id` and `calls_after` set to that cursor for the next page, and `calls_limit` for the page size (default 100, at most 500). A page is also bounded by what its calls weigh — together, never more than the ceiling one record may reach, so the heaviest page is no heavier than the heaviest single call — and can carry fewer calls than asked for; the cursor, not the count, says whether there is more.
