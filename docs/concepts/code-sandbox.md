...
**On a fleet, the box calls the node that opened its session.** A session is a live tool surface in that node's process, so each node mints its endpoint from its own `CREWLET_MCP_BRIDGE_URL`, and that URL has to address the node itself rather than a load balancer in front of several (see [agent mode](subscription-llm-backends.md#agent-mode)). The token is still signed rather than stored: every node derives the signing key from Tier A `secrets.keys`, under a domain that separates bridge tokens from telemetry tokens, so a peer a box reaches by mistake can tell a misrouted call from a forged one and log `mcp_bridge_unresolved` naming the setting. With no keyring the key is per-process, which is fine on one node and logged loudly otherwise: a peer then reads a perfectly good token as forged rather than as a call that reached the wrong node.
