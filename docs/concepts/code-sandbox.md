...
Execute loop                     Engine                         Sandbox
    ...
    │  (busy, mail is requeued)    │  SandboxWaiter polls ─────────┤ keepalive +
...
- **Call → suspend.** `run_sandbox` provisions the box, starts the coding agent as a background command, persists a durable run record in the [coordination store](coordination.md) (the brief, the task, the conversation key, the trace context, and whether the turn owes anybody an answer), and returns a result with `Suspend` set. The Execute loop leaves that call unanswered and the turn ends. The seat goes busy when the coordinator handles the run's `SandboxRunStarted`, which the launch publishes on the seat's control topic before the call returns. That handling is asynchronous, so a delivery the inbox hands out before it lands can still start a turn beside the run (see invariant 11 under [Runtime invariants](turn-engine.md#runtime-invariants)). Nothing is parked in memory; everything the resume needs is in the row.
...
- **Busy while it runs.** Every delivery the agent's inbox hands it for the duration of the run is requeued and acked, so nothing is held against a broker ack window during an hours-long job. Nothing pauses the topic, so those deliveries cycle through requeue and ack until the run settles (see the known gap under [Runtime invariants](turn-engine.md#runtime-invariants)). The agent runs **one coding job at a time**: only the first suspend in a loop is honored, and a busy agent starts no new turns. It is freed only while a run is parked waiting on a human answer (see [Mid-run clarification](#mid-run-clarification-crewlet-ask)).

  That busy state is **this subsystem's to set and this subsystem's to clear**. So every exit that is finished with the box frees the seat, including the ones where cleanup failed: a store write that errors while tearing a box down is a bad minute, and a seat that is owned, attached and permanently deaf is a bad week. The one path that deliberately keeps it is a resumed executor calling `run_sandbox` again, where a new run owns the box and the seat is legitimately still busy.
- **Completion → resume.** The completion event is claimed **at-most-once** (an atomic status flip on the row), the result is collected, tokens are post-accounted, and the suspended loop is rebuilt from the row: the saved messages, the activated tools and loaded skills replayed, and the coding agent's findings appended as the `run_sandbox` call's reply, framed as "the sandbox did the code work; don't redo it; report / act now". The same `turn_id` continues, so the whole sequence renders as one turn on the dashboard. A failed resume dispatch reverts the claim so the redelivered completion can retry, so the suspended loop is never lost. The exception is a resumed turn that broke after writing outside the engine, or panicked: its claim is kept, so the completion is never resumed a second time, and the run is settled like one that finished.
