...
- **A call too large for one record is cut, never dropped.** A record is one message on the transport, so each call is fitted to the 8 MiB ceiling (less 64 KiB for the message's headers). Its **arguments** are decided first, whole or not at all, because a cut through JSON is text nothing can parse: they are kept unless the record would be past the ceiling even with an output of nothing but `…`, and only then replaced by a one-member JSON object, keyed `…`, saying how many bytes of arguments were not kept — which the resumed phase, the reviewer and the run board all show where the arguments would be. The **output** is then fitted to what is left, from the whole of it: cut on a character boundary and ending in `…` when it does not fit, untouched when it does. The coding agent in the box was handed the whole output, and the tool the whole arguments; the record is the engine's only copy, so the cut is marked on it. The node logs `sandbox_bridge_call_fitted`, with the whole lengths as `output_bytes` and `args_bytes`. A NATS server configured below that ceiling refuses a record that fits it — a node does not boot against one, but a reconnect can reach one — and the call is then kept in its least form, its name and outcome with both texts replaced by their marks, while the node logs `sandbox_bridge_call_refused_within_ceiling`, naming `max_payload`.
