...
| `object-map` | Keeps the [object store's](object-store.md) placement map in step with the live data nodes: adds a node at the weight it offers, and takes one out after ten minutes gone. Writes by compare-and-set | Two writers would each move the epoch for one change, and every data node repairs on an epoch change |
