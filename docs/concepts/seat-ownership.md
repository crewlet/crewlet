...
A turn the fence stops is **deferred**, not failed: the delivery is healthy and the seat's new owner is entitled to it, so it goes straight back rather than waiting out the backoff a failure earns, and this node stops fetching. It is not free — on the shipped broker a deferral is a Nak and spends one of the delivery budget's twenty-five, which is why that budget is sized for handoffs as well as failures (see [Event System](event-system.md#delivery-semantics)). The exceptions are a turn that panicked and a turn whose own record proves it already wrote outside the engine — each is acked and recorded instead, because a successor would reach the same defect or repeat the write.
...
| `maintenance` | Two kinds of job on one tick. The **fleet** jobs run only on the node holding the duty: both halves of the A2A channel sweep (the idle-close of an ask no turn ever answered, and the delete of one closed long enough); the retirement of a [removed seat's mailbox](#the-removed-seat) and its coding runs once the seat has been absent for 24 hours; `bridge_calls`, which purges a coding run's bridged calls once no run names their launch; and the native tracker's repairs of work a crash left half-done. The channel, mailbox and bridged-call records are the *shared* things swept here, and the [coordination store](coordination.md#retention-is-a-buckets-age) says why: their buckets have no age, because an age cannot tell an open channel from a closed one, a present seat from a removed one, or a parked run's calls from a finished run's. The **node-local** jobs run on every node on the same tick, holding the duty or not: the retention sweeps for the short-horizon tables each node keeps its own copy of (`events`, `scheduled_runs`, `conversation_sessions`, the expired short-term `agent_diary` rows, `counterparty_profiles`, the tracker inbox, and each state-log domain's operation ledger) | The fleet jobs publish records or delete shared state, so a second node running them would race the first or repeat its work. The mailbox retirement is not idempotent, so it takes its own compare-and-set on every record and claims the seat's own lease while it deletes, rather than relying on the duty's lease. The node-local sweeps are deliberately not held to one node: each node's copy is swept by that node alone, and a sweep that ran only where the duty is held would tidy one copy and let every other grow |
| `retention` | The state log's trim: reads each domain's terms from the live fleet, purges from the ordered log what they all permit, and publishes what it concluded, so a node not holding the duty can still say why a log is not shrinking | Two nodes trimming at once would race on the published floor: one conclusion would overwrite the other, and the record of how long the trim has been held back would reset on every tick. A fleet whose nodes all run without `workers` does not trim |
| `embeddings` | Embeds the company's pages and work items once for the fleet, publishing each vector on the vector domain's log for every node's applier to write | An embedding is a provider call the company is billed for, and every node needs the same vector: run per node, it would be one bill per node for one value |
...
A fleet sets `coordination.type: embedded-kv` and gives the nodes one stream to share, either a clustered embedded server or an external NATS. The two go together by construction: the coordination KV rides the stream's own broker, so refusing the mixed pairing keeps the halves from drifting apart. See [Running a Fleet](../guides/fleet.md#what-a-fleet-needs).
