...
Each sits behind a `worker:{duty}` lease, **claimed per tick rather than held**, so a node that dies mid-duty releases it by lapsing and a peer picks it up on its next tick with no handoff protocol. A duty's TTL is sized from its own tick (three ticks, so one slow claim never moves it), which makes it anything from 30 seconds to three hours, and duty leases live in a [bucket of their own](coordination.md#duties-have-a-bucket-of-their-own) for that reason: the seat lease TTL does not bound them. During a rolling upgrade from a build that kept duties with the seat leases, newer nodes run no duties until the last older node has stopped. There are five:
