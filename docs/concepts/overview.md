...
| Vector search | The store's vector distance functions | The per-agent diary and the episodic store, in the node's own estate beside the rest of its records. The *arithmetic* is the database's; there is no ANN index reachable from the Go driver yet, so recall is a scan behind the per-agent index |
...
| Coordination | TTL leases with a fencing epoch | Seat ownership and the fleet's shared counters, in a KV riding the stream's own NATS connection — never the store's files, and never a second connection that could fail on its own |
