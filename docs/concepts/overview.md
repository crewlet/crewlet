...
Alongside them sit the durable runtime tables a turn leaves behind — `crewlet_events`, `pending_sandbox_run`, `scheduled_runs`, `chat_thread_follows`, `a2a_channels`, `token_usage` — and the config plane's `company_config` payloads. The full migration list is in `internal/store/schema/`.
