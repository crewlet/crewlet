...
**A node's own active revision follows the fleet.** It is what the node's `GET /config` serves, what the node boots on, and what the node offers the fleet at its next start whenever it is newer than the pointer, so once a node runs the fleet's epoch it has to be the fleet's revision. The reconciler checks that on every tick and corrects a copy that disagrees: a write's local activation that failed after the fleet took it, one that landed a moment after the node adopted a newer revision, or an offline import the fleet had already superseded when the node started.
...
`applied_subsystems` is the ordered list of what this node had already rebuilt when it stopped, out of `secrets`, `company`, `tools`, `learning`, `sandbox`, `parties`, `integrations`, `epoch`, `seat_tools`, `mailboxes`, `scheduler` (the example above refused the revision's `providers.sandbox` after rebuilding its learning workers). That is the difference between "refused before anything changed" and "torn down halfway", which is precisely what decides whether the node needs a restart. An `error` with an **empty** list never reached the apply at all: the revision could not be read, opened or parsed.
