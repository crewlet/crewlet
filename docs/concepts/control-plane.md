...
`error` is the failure text as the node reported it, up to 64 KiB and marked where that bound cuts it; the whole text is in that node's own log, at WARN, as `reconcile_tick_failed` (or `initial_reconcile_failed` for the tick it ran at boot).
