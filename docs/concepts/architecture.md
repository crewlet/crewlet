...
| **`chart_units`** · `chart_seats` · `chart_manages` · `chart_leads` · `chart_history` · `chart_import_ledger` | The company's own org chart, derived from `CREWLET_CHART_LOG`: the units and seats a founder authored, the two edges the document states at one end (`manages:` on the seat, `lead:` on the unit) and the record of every change to any of it. The *organisation* a turn reads is derived from these rows rather than stored beside them — see [The Org Chart Domain](chart-domain.md) |
...
| **`CREWLET_CHART_LOG`** | `crewlet.chart.log.>`, **the ordered log the replicated estate's org chart is derived from**, and the state log's fourth domain. It is the one whose subjects do NOT all mean the same thing: a unit's and a seat's own CONTENT contend per object like a task or a page, while the whole STRUCTURE — who sits under whom, who leads what, what the chart no longer names — contends on `…log.tree`, one subject for the company. Containment in an org chart is the object rather than a field on the thing contained, so two moves that are each locally valid can jointly make a cycle that neither writer's subject would have shown them. Serialising the rarest write this company makes is what buys that |
| **`CREWLET_IAM_LOG`** | `crewlet.iam.log.>`, **the ordered log the replicated estate's identity tables are derived from**, and the state log's fifth domain. It is the one where the SUBJECT IS THE CONSTRAINT: the replicated estate forbids a `UNIQUE` index outside a primary key — a constraint violation inside an apply transaction aborts it deterministically on every node at once, which here would be every login in the company — so there is no uniqueness check anywhere in the schema, and what keeps two people from taking one address is that each claim arbitrates on ITSELF, create-only at an expectation of zero: `…log.email.<blind>`, `…log.login.<login>`, `…log.seat.<id>`, `…log.session.<lineage>`. No two claims share a subject, because a shared one never contends and the duplicate identity nobody refused is discovered by somebody signing in as somebody else. A session writes one record when it opens and one when it closes and nothing in between — a rotation id is derived rather than recorded — so this log grows with sign-ins rather than with requests |
