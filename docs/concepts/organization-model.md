...
Each unit (and the organisation root) is expected to publish a page titled exactly **`Onboarding`** in its own container of the knowledge base: the unit's `space`, and for the root `knowledge.root_space` (`HOME` by default). On an agent seat's first turn for its current org chain, a dedicated onboarding pass runs before the executor, with a short `## First-turn onboarding` block listing the unit chain (org → ancestor units → own unit). The pass starts with only `reflect_and_persist`, `mark_onboarded` and the tool-discovery pair active, and its header tells the agent to activate its knowledge base's own search and page-read tools with `activate_tool` and read each `Onboarding` page with them: on the native backend they are `search_knowledge` and `get_page` in its tool catalogue (`get_page` also takes `CONTAINER/Title`, such as `ENG/Onboarding`); on a vendor wiki they are that wiki's MCP server's page-search and get-page tools — `confluence_search` and `confluence_get_page` on Confluence — found with `list_mcp_server_tools` on the team's knowledge-base server. The agent then captures the conventions that matter via `reflect_and_persist` and calls `mark_onboarded` when done. After that, the hint disappears from subsequent prompts.
