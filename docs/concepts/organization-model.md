...
Each unit (and the organisation root) is expected to publish a page titled exactly **`Onboarding`** in its own container of the knowledge base: the unit's `space`, and for the root `knowledge.root_space` (`HOME` by default). On an agent seat's first turn for its current org chain, a dedicated onboarding pass runs before the executor, with a short `## First-turn onboarding` block listing the unit chain (org → ancestor units → own unit). The agent finds and reads each `Onboarding` page with its knowledge base's own search and page-read tools — `search_knowledge` and `get_page` on the native backend (`get_page` also takes `CONTAINER/Title`, such as `ENG/Onboarding`), the Confluence MCP server's `confluence_search` and `confluence_get_page` on Confluence — captures the conventions that matter via `reflect_and_persist`, and calls `mark_onboarded` when done. After that, the hint disappears from subsequent prompts.
