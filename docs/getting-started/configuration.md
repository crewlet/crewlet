      ...
      - name: Backend                   # required
        ...
        roles:
          - name: Tech Lead             # required — unique agent identity
            ...
            integrations:               # optional — this seat's own external identities
              mattermost:
              ...
              atlassian:                # optional — which Atlassian products this
                products: [jira]        #   seat is licensed for; a licence is billable
...
| `integrations.atlassian.products` | list[string] | no | Which Atlassian products this seat holds a licence for — `jira` and/or `confluence`. Read by [`crewlet atlassian provision`](../integrations/atlassian.md), and reported by [`GET /integrations`](../reference/api-endpoints.md#get-integrations), which runs that command's own planner over the org chart to name the seats a run would mint an account for; no routing, parser, transport or delivery decision reads it. Three states, all real: the **block absent** means every product the company configures, an **explicit empty list** means none (how a chat-only seat stays out of a provisioning run without deleting its `mcp_env`), and a **list** means exactly those. Two of those spellings are nearly identical, so a **bare `atlassian: {}` with no `products` key is refused**: it decodes to the same thing as the empty list, and a block an author wrote to say something about the seat would silently take away every licence it would otherwise have had. Write `products: []` for none, or remove the block for all. It is exposed rather than collapsed into "every configured product" because a product licence is **billable** and the free service-account allowance without Atlassian Guard is small — `products: [confluence]` is how a writer agent consumes one licence and holds no credential that can move a sprint. Authored on a **role**, never a unit: a licence is per seat. Naming a product twice, or one outside `jira | confluence`, fails validation |
...
integrations:
  ...
  atlassian:                             # the ORGANIZATION the two sites above live in;
                                         #   read by `crewlet atlassian provision` and
                                         #   reported by GET /integrations — never by a
                                         #   routing decision
    org_id: "${ATLASSIAN_ORG_ID}"         # required — the Settings page at admin.atlassian.com
    site_url: "https://example.atlassian.net"  # optional — permission-report link base;
                                               #   a ${VAR} resolving to one is fine too
    display_name_prefix: "Crewlet"        # optional — default "Crewlet"; never empty
...
- **`atlassian`** — the **organization** the `jira` and `confluence` sites live in, and the one block here that nothing ever *routes* on: no parser, transport or delivery decision reads it, because nothing it carries is an inbound routing input, which is why the organization has no webhook route of its own while both of its products do. What reads it is [`crewlet atlassian provision`](../integrations/atlassian.md) — and the engine's own company view, since a block an operator wrote that no surface acknowledges is indistinguishable from one the build does not know about: [`GET /integrations`](../reference/api-endpoints.md#get-integrations) reports the organization id and site, no secret (there is none to hold), and runs the provisioner's planner to name the seats a run would mint an account for. `org_id` is **required** and is on the Settings page at admin.atlassian.com. There is deliberately **no `api_key` field**: the organization key carries the right to create billable accounts across the whole organization, so it stays the *operator's* and arrives from `-admin-token`, then `$ATLASSIAN_ORG_API_KEY`, then `$ATLASSIAN_ADMIN_TOKEN` — never the secret store, which is replicated to every node holding the keyring. The block names the organization and nothing else: the site a licence is granted on is the `cloud_id` on `jira` or `confluence`, which is also how an organization running its two products on two sites works rather than by accident. `display_name_prefix` (default `Crewlet`) can never be empty — it is the only thing marking an account as this company's, so it is what a re-run adopts by and what `-decommission` sweeps by, and an empty prefix would match a person. `token_expiry_days` is refused outside 1–365 (default 300, deliberately short of Atlassian's cap: nothing renews a credential on a schedule). Two cross-block shapes fail validation rather than a mid-run API call — an `atlassian` block with neither `jira` nor `confluence` has no site to license a seat into, and a company where one product names a `cloud_id` and the other does not is half Cloud and half Data Center. Which products a given seat is licensed for is `role.integrations.atlassian.products` (below). See [Atlassian](../integrations/atlassian.md).
...
**There is no `transports` key.** Outbound delivery is *derived*, never declared: enabling `mattermost` starts its transport from the same block, GitLab's comes from `gitlab`, and Slack's is per seat, built from each role's own app credentials rather than from anything org-level. Jira and Confluence have no transport at all, by design — an agent's write to an issue or a page goes through its own MCP tools under its own credential, never through the engine. This matters more than a missing feature would: `integrations` refuses unknown keys, so a config carrying a `transports:` list is rejected **whole**, not partly ignored.
