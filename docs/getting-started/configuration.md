...
```yaml
integrations:
  # REFUSED BY THIS BUILD: confluence, slack, github. They are shown here
  # for their shape only — `crewlet validate` rejects a config that sets any
  # of them. See the notes under this block.
...
- **`confluence`** — **refused by this build.** There is no Confluence searcher and no parser, so the block named a surface the engine never read: `integrations.confluence` is how an operator says where the company's knowledge lives, and the answer was an empty "Relevant knowledge" section on every Plan phase. `crewlet validate` rejects it by name, along with `knowledge.confluence_spaces`; the knowledge base is `plane`. Agents still reach Confluence through the `atlassian` MCP server, which is a separate surface. See [Confluence](../integrations/confluence.md).
