...
A stdio server's **stderr is never passed through raw**. Every line the child process writes (startup banners, tracebacks, that probe warning) becomes a structured `server_stderr` DEBUG event attributed to the server, instead of foreign log lines interleaving with the engine's own stream. When a server fails to start, the last lines it wrote are surfaced with the failure as a single `server_stderr_tail` ERROR event — that tail usually names the real cause (bad token, missing binary, import error). Run with `-debug` to watch a server's stderr live, line by line.

**The tail is bounded twice, and both bounds say so.** One line is kept up to 8 KiB, and the 50 most recent lines are kept per server.

A longer LINE is cut on a character boundary — so the text stays valid UTF-8 whatever alphabet the server writes in, rather than ending in half a character that a JSON log handler rewrites to a replacement character (`U+FFFD`) — and the marker ` …[truncated]` is appended, so a severed line never reads as the whole of what was written.

The 50-line WINDOW is the second cut, and the `server_stderr_tail` event reports it rather than leaving you to infer it: beside `lines` the event carries `dropped_lines`, the number of earlier lines the window pushed out to keep the ones you are reading. It is present always, including when it is `0` — a field that appeared only sometimes is a field a log query misses, and `0` is itself the answer that matters, because it says the tail is every line the server wrote rather than the end of something longer. Fifty lines from a server that wrote fifty and fifty from one that wrote five thousand are the same slice, and opposite diagnoses; `dropped_lines` is what tells them apart.

Where the dropped lines are: in the engine's own log, on one condition. Every line the window forgets was already emitted as its own `server_stderr` DEBUG event by the same pump that fills the window — so a node running at debug level (`-debug`, or `logging.level: debug`) holds the whole sequence, and the tail is a convenience copy of its end. The condition is that level: above debug the log holds none of those events and `dropped_lines` is all that survives of them. Each of those events is one line under the same 8 KiB bound and the same marker, so what this route recovers is every LINE, never more of any one of them.

What the 8 KiB bound drops is **gone**, and this paragraph says so rather than leaving you to assume a copy exists: the child's stderr is a private pipe with no second reader, so there is no store row, event payload or retrieval tool holding the rest.

That is a decision about COST, not a missing route. The whole of an over-long line could be recovered without ever buffering it — the same pump already writes one `server_stderr` event per line, so each successive 8 KiB window of a monstrous line could go out as its own event, and the engine would hold no more at once than it holds now. What refuses that is the input the bound exists for in the first place: a server emitting a gigabyte with no newline becomes 131,072 debug events, each through the engine's log handler and whatever your deployment ships that log to. The recovery route would BE the outage, and it would fire hardest exactly when a server is already misbehaving. So the line is bounded and marked instead — and when you need a server's untruncated output, run that server's own `command` in a terminal, where its stderr is your shell's and nobody's heap.
...
| **Say it cut** | a coding run's unparseable output and its crash error (the last 64 KiB — a crash explains itself at the bottom); an MCP server's stderr tail; a `cli-agent` subprocess's output; error text on an event (`events.ClipDiagnostic`, 64 KiB, head kept); a prior round's produced text and a failed call's result as *rendered* into the prior-work ledger; a conversation's older entries, dropped whole and counted; a delegated worker's error as its parent reads it (start and end kept, whole on the worker's phase event); a config diff; a knowledge snippet; an episode's tool sequence | The full text genuinely cannot travel — an unbounded subprocess, a per-tick fleet read, or an event the queue would REFUSE, which costs the operator the whole record rather than its tail — so the excerpt carries a marker and, where a reader can go and get the rest, says how |
