...
- **History** is the fifty most recent changes, and `history_truncated` says
  when there are older ones. `task_activity` is what pages properly, with a
  cursor that survives a reanchor. The flag matters because the cut was silent:
  a task with five hundred changes and a task with fifty answered identically,
  so a reader asking "has anybody touched this" was told the whole story either
  way and could not tell which it had.
