...
| **parent**, **subtasks** | a tree, with a depth cap, inside ONE project: a subtask is filed in its parent's project, and a create or a re-parent that would put it anywhere else is refused, because a board draws a project's roots with their subtrees and a subtask filed elsewhere would be on neither board. A parent in the trash takes no new subtask. A query filters ROOTS by default and lets their subtrees ride along; `subtasks=separate` filters every task on its own. |
