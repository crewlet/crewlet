...
| **parent**, **subtasks** | a tree, with a depth cap, inside ONE project: a subtask is filed in its parent's project, and a create or a re-parent that would put it anywhere else is refused, because a board draws a project's roots with their subtrees and a subtask filed elsewhere would be on neither board. A parent in the trash takes no new subtask, a purged one is refused as gone for good, and a task is never filed under itself or one of its own subtasks, which would make a cycle with no root to draw it from. A query filters ROOTS by default and lets their subtrees ride along; `subtasks=separate` filters every task on its own. |
...
| `create_work_item` | file a task or a subtask. A subtask is filed in its **parent's** project when the call names none — the caller's own team is the default only for an item with no parent. `fields` sets custom fields by **slug**, and the create is refused naming any the project requires and this call leaves out. It also takes the four **scheduling** arguments below |
...
| `merge_work_item` | fold a duplicate into the item that survives: the duplicate is linked to it, its **subtasks are re-parented onto it** (`move_subtasks`, true unless you say otherwise), and the duplicate is closed as `cancelled`. Nothing is destroyed and both histories stay readable. Closing a duplicate by hand instead leaves its subtasks under a closed parent, where nobody finds them. Across two projects the subtasks cannot follow — a subtask lives in its parent's project — so such a merge is refused before it writes anything unless `move_subtasks` is false, and so is one moving the subtasks onto one of them. The item that survives must be live: a merge into an item in the **trash** is refused whichever way the subtasks go (restore it first), and a subtask in the trash is frozen, so it stays under the duplicate. A merge interrupted part-way is finished by the tracker duty, and one only a person can unblock waits, named on every tick with what releases it, without holding up the rest: a survivor that went to the trash since (`tracker_merges_waiting_on_the_trash`) until it is restored, a duplicate that did (`tracker_merges_of_items_in_the_trash`) until it is restored — the trash takes no write, so nothing moves on its behalf meanwhile — and a survivor filed under one of the duplicate's subtasks since (`tracker_merges_waiting_on_a_cycle`) until it is filed elsewhere. One whose survivor was purged has nothing left to merge into, so its mark is cleared and the duplicate stays open |
