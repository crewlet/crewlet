...
| `statelog_reanchor_started`, `statelog_reanchored` | `WARN` | A generation transition of ONE domain. Both name the domain (`domain`), the one stream it moved (`stream`), the new generation and the stream's live creation instant (`stream_created_at`); the start also names the instant the rows were keyed to before (`keyed_to`), and the completion gives the new checkpoint (`cursor`) and the stream's high-water mark before the reanchor (`prev_last_seq_seen`). No other domain's checkpoint moves, and the domain's applier resumes without a restart. |
