...
4. **Project lead fallback** — if steps 1-3 produced no *meaningful* recipient (the creator is auto-added by Jira as a watcher, so a delivery to only the creator counts as no recipient), the lead of the unit that owns the project (its `integrations.jira.project`) gets a copy with `routed_via = "project_lead_fallback"`. The mapping is built once at engine start from each unit's `integrations.jira.project` (runtime `org.Unit.JiraProject`) and the effective lead.
