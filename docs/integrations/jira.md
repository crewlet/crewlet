...
The project identity is set once on the unit's `integrations.jira.project`. It is integration identity (webhook routing + write home), not a tool credential, and it does not scope knowledge reads. An agent seat's `mcp_env.atlassian` is layered over its unit's, variable by variable, with the seat's own values winning, so each agent still authenticates as itself. Only the unit's direct agent seats inherit the unit's block; a human seat inherits none.
