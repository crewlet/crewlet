...
The `## Relevant knowledge` block carries no pages — logged, never an error — whenever the search cannot run. With no searcher wired (an org token that did not resolve, an integration that failed to start) the block is left out, and a seat's `search_knowledge` answers that the company's knowledge base is not served on this node — naming `integrations.confluence.token` when the token is the cause, and the node's own log line otherwise; with a search that failed (an unreachable instance, a query the site refused) the block carries the sentence that nothing surfaced at turn start and that a focused search is the next step. **A turn must not die because a wiki was slow**, so every failure path is an empty result.
