...
(The Nimbus example ships a `confluence:` block, so `examples/nimbus.company.yaml` works as the positional argument as-is; `examples/nimbus-docs/` is the set of pages it publishes. The smaller `examples/nimbus-claude-cli.company.yaml` has no wiki — add the block at the top of this page to it first.)
