...
- **`trigger:` present ⇒ a [Tool Skill](../concepts/tool-skills.md)** — published into the [Tool Skills project](#the-tool-skills-project) with a leading YAML code block the engine parses back out. Its directory is ignored. Target project: `--project` > `$CREWLET_TOOL_SKILLS_PROJECT` > `TS`. The engine reads `integrations.plane.skills_project` (same default), so point the two at the same project — they are the publishing side and the reading side of one convention.
