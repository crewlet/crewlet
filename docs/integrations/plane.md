...
Registration runs at **engine startup**, on live **`integrations.plane` config changes** (any config write whose diff touches that block re-resolves every seat), and when **roles are added live** (only the just-added seats are resolved, and that role-add refresh is additionally gated on a running MCP bridge). Rotating an *existing* seat's `mcp_env.plane` token in place is **not** re-resolved live — an engine restart or a config write touching `integrations.plane` is required to pick up the new identity.
...
- **Two result exclusions**: pages in the [Tool Skills project](#the-tool-skills-project) are dropped wholesale (skill content reaches agents through the skill catalogue / `load_tool_skill`, and a skill page's leading YAML block would dominate any snippet), and unreviewed [auto-drafts](#skill-promotion-on-plane) are hidden by the `[Auto-draft] ` **title prefix**. Plane pages carry no parent chain here, so unlike Confluence there is no ancestor to judge by — and the prefix is the better half of that pair anyway: it needs no lookup, so there is no query whose failure could let an unreviewed draft surface. It is applied only when a caller asked for drafts to be hidden, which is every agent-facing search.
...
- The draft's target container is the unit's **`integrations.plane.project`** identity; a unit without one soft-skips with the hint naming that field.
- **Cross-tick dedup by external identity, not by title.** The pass re-clusters the same persisted rows every tick, and Plane enforces no title uniqueness — so the writer stamps every draft with `external_source="crewlet"` + `external_id="draft:<title>"`, walks the project's page index before creating, and **returns the existing page instead of creating another**. The identity survives a lead renaming the draft while reviewing it, which a name-keyed check would not: that would create a second copy beside the one they are editing. A row carrying the same id but another tool's `external_source` is *not* adopted. One converging cluster therefore produces **one** draft, ever, and a tick that finds it stays quiet rather than re-announcing the promotion.
- **The page walk is all-or-nothing.** A failed or truncated enumeration makes an existing draft look absent, so the writer **refuses the draft** for that tick rather than creating a duplicate; the next tick retries.
- **Drafts are hidden by title prefix, not by parent.** Plane pages in this build carry no parent chain — see [query-time knowledge search](#query-time-knowledge-search) — so `[Auto-draft] ` on the title is the mechanism rather than a backstop, and it is strictly the safer of the two: there is no lookup to fail, and therefore no window in which an unreviewed draft becomes visible because a parent query came back empty. Bodies are rendered markdown; every page is created `access` public.
- **Publish gesture** — a unit lead reviews the draft and **drops the `[Auto-draft] ` prefix from the title**. That is the whole gesture on Plane: renaming is what makes the page reachable by the knowledge search. (On Confluence the equivalent gesture is moving the page out of the `Auto-Drafted Skills` parent, because there the ancestor chain is the mechanism.)
