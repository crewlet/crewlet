...
- **[The Org Builder](guides/org-builder.md)**: editing the organization, and creating the company, from the dashboard's Org chart screen: what the lens shows and why, creating a company from a template, how every draft is checked by the engine, reviewing and saving (including a save whose answer never arrives), following the revision until every node applies it, keeping a `company.yaml` in step with `-import-company` and Copy as YAML, keeping a draft across a reload, updating a draft when somebody else saves first, and undo, redo and the keyboard
