...
- **[Integration Reconcile](concepts/integration-reconcile.md)** — What checks that an integration still works: the finding vocabulary every vendor reports in, why the excess-access advisory is ranked last, a cadence derived from who has to act rather than from what failed, the fleet singleton that runs it, and what the loop deliberately will not do (tear down, rotate a working credential, or provision unattended)
