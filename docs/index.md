...
- **[Deployment](guides/deployment.md)** — The single host, the compose profiles, the stream beyond one host (an embedded cluster or an external NATS one, with its auth and TLS), the event store, tracing, logging (levels, the three formats, colour, and a rotating log file beside stderr)
