...
- **[Chat](concepts/chat.md)** — The third backend axis (`chat.backend: native | vendor | none`): the engine's own chat as the log's fourth domain — channels, unit rooms the org chart creates and is the sole writer of, derived direct-message identity, one-level threads, edits, tombstoned deletes and reactions — who a message wakes and which of them owes a reply, how a person is bound to a seat by an API token, read state in coordination rather than on the log, its own keyword index with no semantic half, the message horizon, and the three things it deliberately does not do
...
- **[The Work Tracker](guides/work-tracker.md)** — The product guide to the engine's own tracker: projects and keys, everything a task carries, goals and how their progress is computed rather than stored, the catalogue of types and fields a company declares, the five view shapes and the six every container has without anybody saving one, why the trash is a query rather than a shape, the timeline's bars, bands and dependency arrows and what it refuses to place, who is carrying how much and the two things the workload will not do, manual board order and what a re-spread is, a person's own inbox, queue and pins and the three different authorities over them, the tags any seat may declare and the rules that keep them one grouping, the fourteen tools a seat has and the four that count as a delivery, the two wakes that are about something other than a task, why a handle is checked before it is stored, what a person reaches through the dashboard and their own assistant, and the difference between removing, deleting and purging
