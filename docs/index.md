...
- **[Integration Reconcile](concepts/integration-reconcile.md)** — What checks that an integration still works: the finding vocabulary every third-party app reports in, what a refused call contributes to it (what the app *said*, never the page it sent), why the excess-access advisory is ranked last, a cadence derived from who has to act rather than from what failed, the fleet singleton that runs it, and what the loop deliberately will not do (tear anything down, or rotate a credential that works) — it does register the webhooks and create the accounts, because connecting an integration is the permission
