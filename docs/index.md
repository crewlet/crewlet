...
- **[The Org Chart Domain](concepts/chart-domain.md)** — the chart as a replicated state machine: why the structure arbitrates on one subject while each unit's and each seat's content arbitrates on its own, what `stream.chart_log_max_bytes` is for and why it is the one log ceiling that is not derived from your disk, why a node behind on the chart is not admitted to run seats, what a key change keeps working and what it does not, and the tables the engine derives from the log
