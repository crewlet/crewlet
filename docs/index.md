...
- **[Chat](concepts/chat.md)** — The third backend axis (`chat.backend: native | vendor | none`): the engine's own chat as the log's fourth domain — channels, unit rooms with managed membership as a floor, derived direct-message identity, one-level threads, edits, tombstoned deletes and reactions — who a message wakes and which of them owes a reply, how a person is bound to a seat by an API token, read state in coordination rather than on the log, its own keyword index with no semantic half, the message horizon, and the three things it deliberately does not do
