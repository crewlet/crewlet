...
- **[Choosing Your Stack](getting-started/choosing-your-stack.md)**: the decision guide for every external dependency (the LLM provider, the tracker and knowledge base, the code host, chat and the code sandbox), what each path sets up for you, and what you must create manually
...
- **[Architecture](concepts/architecture.md)** — The whole engine at six zoom levels: what sits outside the process boundary and what each dependency is for, what runs inside one node under `ingress` / `seats` / `workers`, the hop-by-hop path a trigger takes from a third-party app webhook to an agent's reply across a fleet, what a turn does and how one survives its own process, which of the four estates every table, bucket and stream belongs to, and what changes when a second node appears
...
- **[Identity and Access](concepts/identity-and-access.md)** — Who is acting and what they may do: the four kinds of principal, why a person bound to a seat acts as themselves and an unbound one acts as the credential, the ten grants and which surface each opens, the enrolment stages, why a name's shape decides which namespace it belongs to so a login, a machine handle and a seat handle can never collide in one audit column, and the difference between the engine knowing you are nobody and the engine not being able to tell — plus where identity is KEPT: the state log's fifth domain, its seven tables, why a claim's uniqueness is arbitrated at the broker rather than checked by an index, what is sealed under a person's own key and what is deliberately in the clear, and why a stalled identity log does not stop your agents
...
- **[Coordination](concepts/coordination.md)**: The fleet's shared store and the line between it and the node's own database: the three-valued answer every question here returns, which direction each contract fails in when the store cannot be reached, the slots a fleet is discovered from, why every retention is a bucket's age rather than a per-write TTL, why fleet duties keep their leases in a bucket apart from the seats' and what a rolling upgrade across that costs, and what stays node-local or per-process on purpose
...
- **[The Org Chart Domain](concepts/chart-domain.md)** — the chart as a replicated state machine: why the structure arbitrates on one subject while each unit's and each seat's content arbitrates on its own, what `stream.chart_log_max_bytes` is for and why it is one of the two log ceilings that are not derived from your disk, why a node behind on the chart is not admitted to run seats, what a key change keeps working and what it does not, and the tables the engine derives from the log
...
- **[Agent Runtime](concepts/agent-runtime.md)**: how a configured role becomes a running seat, the states the dashboard shows, what each phase's prompt is built from (including the chat thread a turn is handed rather than told to fetch), the built-in tools, and graceful shutdown
...
- **[Event System](concepts/event-system.md)**: EventQueue, topics, routing, inbox batching, listing the durable subscriptions a broker holds, distributed tracing
- **[Integration Reconcile](concepts/integration-reconcile.md)** — What checks that an integration still works: the finding vocabulary every third-party app reports in, why the excess-access advisory is ranked last, a cadence derived from who has to act rather than from what failed, the fleet singleton that runs it, what a rename does and does not move at a third-party app, and what the loop deliberately will not do (tear anything down, or rotate a credential that works) — it does register the webhooks and create the accounts, because connecting an integration is the permission
- **[The Tracker](concepts/task-engine.md)** — the two shapes a company's work can take: the engine's own tracker (`tracker.backend: native`, the default — every change a record on an ordered log the fleet shares, applied into identical SQL copies on every node, with a board, seat tools and an MCP surface), or an external PM tool the engine deliberately mirrors none of
- **[Scheduling](concepts/scheduling.md)** — Role/unit-scoped cron-style recurring work (standups, audits, nightly jobs)
- **[Knowledge System](concepts/knowledge-system.md)** — Shared knowledge behind the `knowledge.Searcher` seam — exactly one backend per company, either the engine's own pages (keyword, semantic or both fused) or a live Confluence search — plus the private `agent_diary`, and `crewlet search eval` for measuring the semantic half on your own vectors
...
- **[GitHub](integrations/github.md)** — github.com or Enterprise Server: one GitHub App per agent (create it, then install it) with three access tiers, webhook routing by review request / assignment / review verdict / mention, derived seat logins, participant fan-out, organization or per-repository hooks, and `crewlet github provision`
- **[Datadog](integrations/datadog.md)** — Monitor alerts as inbound events through the Webhooks integration: routing by monitor tag with a required fallback seat, a trigger and its recovery as one conversation, and shared-token verification because Datadog cannot sign a body
...
- **[The Org Builder](guides/org-builder.md)**: editing the organization, and creating the company, from the dashboard's Company screen: what the lens shows and why, creating a company from a template, reading the organization on the canvas, the reporting chart and the outline (their keys, the lead chip and reordering), how every draft is checked by the engine, what each editor can and cannot change and why, the integration fields a seat carries, what moving, deleting and changing a seat's kind do, reviewing and saving (including a save whose answer never arrives), following the revision until every node applies it, keeping a `company.yaml` in step with `-import-company` and Copy as YAML, keeping a draft across a reload and what asks before work is left behind, updating a draft when somebody else saves first, and undo, redo and the keyboard
- **[Configure via API](guides/configure-via-api.md)** — End-to-end curl recipes for bootstrapping a company through `/config/*` and evolving its org chart through `/chart/*`
- **[The Work Tracker](guides/work-tracker.md)** — The product guide to the engine's own tracker: projects and keys, everything a task carries, the catalogue of types and fields a company declares, the five view shapes and the six every container has without anybody saving one, why the trash is a query rather than a shape, the timeline's bars, bands and dependency arrows and what it refuses to place, who is carrying how much and the two things the workload will not do, manual board order and what a re-spread is, a person's own inbox, queue and pins and the three different authorities over them, the tags any seat may declare and the rules that keep them one grouping, the thirteen tools a seat has and the four that count as a delivery, the one wake that is about something other than a task, why a handle is checked before it is stored, what a person reaches through the dashboard and their own assistant, and the difference between removing, deleting and purging
- **[Replication](guides/replication.md)** — How the fleet's copies of the company's work stay in step: the three numbers a reader must not fold together, the three outcomes a write has and what to do with `pending`, what an acknowledged write has actually reached at each topology, the two regimes above and below the trim floor, replication lag as two positions, what a bulk gesture costs every peer, and what the design does not promise
- **[Search](guides/search.md)** — How a knowledge search is answered and what to do when it is slow: the two rankers and the bucket both of them carry, when a fleet divides the scan and the floor below which it does not, why the merge is by score and only then fused, why BM25 stays comparable across a divided scan, and what a partial answer names rather than silently truncates
- **[Read Consistency](guides/consistency.md)** — Picking how fresh an answer has to be: the four levels and what each costs, how `linearizable` works as a barrier append rather than a field check, the twelve refusal codes with a remedy each, completeness as a fact separate from freshness, and the three things no level can strengthen
- **[Retention](guides/retention.md)** — What the log keeps and why it sometimes will not shrink: the one number to watch, the six trim terms in plain words, why a company that never backs up never trims, snapshots and the join runbook, eviction and readmission, what a full log refuses, the storage forecast, and the recovery-operations profile

## Reference

- **[CLI](reference/cli.md)** — Command reference
- **[API Endpoints](reference/api-endpoints.md)** — REST API routes and schemas, the WebSocket protocol, the tracker and page surface over the native backends, and `/operator/mcp` — the company's own tracker and knowledge base served to *your* AI assistant over MCP
- **[Dashboard Design System](reference/dashboard-design.md)** — The dashboard's information architecture and its visual system: the two levels of navigation and the grammar that keeps them apart (a rail row is a workspace, a sidebar row is a destination with its own path, a tab is a query on the path you are on — and nothing is two of those), the route table and the gate that holds it against the router's own tables, the frame every screen wears, why the Inbox is the landing screen and what the wake reason on each of its rows is, the one rule (colour carries state, never identity), a palette whose every contrast and separation claim is recomputed from the shipped stylesheet in both themes, the four rules that keep a running turn's transcript from moving under the reader, why the built bundle is committed, and the rules a change has to keep
